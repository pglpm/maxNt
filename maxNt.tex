...
%% Last-Updated: 2017-05-15T14:30:55+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
\mynote{Add formula $\av{\underbrace{\yX\dotsm\yX}_{\text{$m$ factors}}} = \binom{N\yXf}{m}/\binom{N}{m}$.}
...
\yL_2\, \frac{\yRf\,(N \yRf-1)}{N-1} + \yL_1\, \yRf\biggr].
\end{equation}
Applying \me\ at the sample level, using the third term of
\eqn~\eqref{eq:expe_products}, we find
\begin{equation}
...
We call this state of knowledge $\yHb$. For example, with $N=5\,000$,
$n=100$, $c_1=0.45$, $c_2=0.35$, we find $\yL_1 = -13\,321.9$,
$\yL_2 = 13\,321.5$, $\yl_1=-269.3$, $\yl_2 = 268.9$, and appropriate
normalization constants. The resulting distributions
$\p(\yxs = \yrs \| \yHa)$, $\p(\yxs = \yrs \| \yHb)$ are shown in
\fig~\ref{fig:diff_maxent_pop_sample}; they are clearly different.
\begin{figure}[!t]
\centering
...
similar results in some cases of interest, especially with numerically
small constraints. One could thus be used as an approximation of the other
depending on the circumstances -- \eg\ if probability tails are
unimportant. See \fig~\ref{fig:diff_maxent_pop_sample_realdata} for an
example.
\begin{figure}[!t]
\centering
...
\mynote{We don't give an answer. Different purposes. Bright side: no real
  contradiction because we are expressing two states of knowledge, not
  physical situations. Also, in some case numerically very similar results.

  The distribution obtained from the population-level application has lower
  entropy; this usually means that in includes additional assumptions than
  the other. Intuitively, this additional assumption is that the $n$
  neurons are part of a larger population, of which they are
  representative.

  Same phenomenon in statistical mechanics \cite{maesetal1999}. Somehow
  similar quandary for equilibrium state at the end of non-equilibrium
  process: Gibbs state vs initial Gibbs state + Liouville
  evolution \cite[\sect~4]{jaynes1985d_r1993}; but in this case both
  distributions are immensely sharp, and this doesn't affect predictions}
...
symmetry assumption~\eqref{eq:homog_math}. Now consider the subnetwork of the
...
\begin{equation}
\label{eq:conditional_prob_explained}
\begin{split}
      \p(\yxs =a \| \yXf=A, \yH)
      &=
...
\end{split}
\end{equation}
  which is the conditional probability~\eqref{eq:conditional_prob}. Note
  ...
  marginalization,
  \begin{equation}
    \label{eq:subpop_average_explained}
    \begin{split}
      \p(\yxs =a \| \yH)
    ...
    \end{split}
  \end{equation}
  which proves the subnetwork-average formula~\eqref{eq:subpop_average}.
  This formula, combined with \eqns~\eqref{eq:aver_subpop} and
  \eqref{eq:joint_plaus_N_homog}, leads to the conditional
  probability~\eqref{eq:conditional_total_state}.
