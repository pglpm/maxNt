...
%% Last-Updated: 2017-05-17T21:23:29+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
methods, but its discussion of \enquote{random sampling} formulae may be
useful to all neuroscientists involved in understanding the physical and
dynamical characteristics of brain areas via neuronal recording.

Recent electrophysiological techniques in fact allow experimenters to
record samples of even a couple hundreds neurons from specific brain areas.
We don't elaborate on the various important purposes of such recordings
here, but stress that these sample sizes can be considered \enquote{large},
because their statistical analysis requires considerable computational
power.
...
neuronal activity. Maximum-entropy methods make this kind of compression:
they associate a unique probability distribution with few experimental
quantities. This distribution is usually said to be \enquote{maximally
  noncommittal} \cite{jaynes1963}. 


We intend the quoted expression only as
an umbrella term, also because it means very little without further
clarifications.
...
\citep{jaynes1994_r2003}. We intend \enquote{probability} as the degree of
belief that \enquote{would be agreed by all rational men if there were any
...
%  &
%    \sav{\yx \yx} &\defd\tbinom{n}{2}^{-1} (x_1 x_2 + x_1 x_3  + \dotsb +  x_{n-1} x_n),
\\[2\jot]
\av{\yX\yX\yX} &\defd
                  \tbinom{N}{3}^{-1}
...
$\ypp(\yrs\|\yRf)$, typical of \enquote{drawing without replacement}
problems. The combinatorial proof of the formulae above is in fact the same
as for this class of problems
\cites[\chap~3]{jaynes1994_r2003}[\sect~4.8.3]{ross1976_r2010}[\sect~II.6]{feller1950_r1968}.

The conditional probability $\ypp(\yrs,\yRf)$ connects the spaces of the
sample average $\yXf\in\set{0,\dotsc,N}$ and of the population average
$\yxs\in\set{0,\dotsc,n}$. It is a coarsening projector of any probability
$p$ for $\yXf$ onto a marginal probability $p_*$ for $\yxs$:
\begin{equation}
...
\end{equation}
Therefore it also a pulls back expectations of functions $f$ of the sample
average $\yxs$ to expectations of functions $f^*$ the population average
$\yXf$:
\begin{equation}
  \label{eq:pullback_P}
...
\begin{figure}[!t]
\centering
%\includegraphics[width=0.75\linewidth]{pop_sample_projection.pdf}\\
\includegraphics[width=0.75\linewidth]{pop_sample_projection2.pdf}%
\caption{Log-plot of the hypergeometric distribution
  $%\p(\yxs = \yrs \|\yXf=\yRf, \yH)=
  \binom{n}{n\yrs}\binom{N-n}{N \yRf-n\yrs}\binom{N}{N \yRf}^{-1} \defs
  \ypp(\yrs\|\yRf)$ for $N=5000$, $n=200$. (Band artifacts may appear in the
  colourbar depending on your \textsc{pdf} viewer.)}
\label{fig:hypergeom_proj}
\end{figure}%hypergeometric_identity.nb
A look at a plot of the hypergeometric distribution $\ypp(\yrs\|\yRf)$, for
example \fig~\ref{fig:hypergeom_proj}, reveals that it is a sort of
\enquote{fuzzy identity matrix} between the $\yXf$-space $\set{0,\dotsc,N}$
and $\yxs$-space $\set{0,\dotsc,n}$. When $n=N$ it is the identity matrix.
We thus have that
\begin{equation}
...
features of the two probability distributions. If the distribution for the
population average $\yXf$ is bimodal, for example, the bimodality can be
lost in the distribution for the sample average $\yxs$, owing to the
coarsening effect of $\ypp(\yrs\|\yRf)$.
...
The approximate equalities above express the fact that \emph{our
  ...
  the population and about other samples}, and vice versa. Upon observation
of a sample average, say $\yxxs=a$, the updated expectations for such
average in the population and in any new sample will usually be shifted
towards the observed value, as follows from Bayes's theorem and the
formulae above.
...
uncertainty about the population, and does not imply any sort of physical
...
among those that have the desired expectations. But we are in a quandary:
...
$f$ to the vector values $c$, that is, $\expeb{f(\yxs)}=c$. Application of
\me\ at the population level, denoted by $\yHa$, gives
\begin{equation}
  \label{eq:app_maxent_pop}
  \p(\yXf = \yRf \| \yHa)  = \yK\,
  \binom{N}{N \yRf}\exp\Bigl[\yL\T
  \sum_{n\yrs=0}^n f(\yrs)\ypp(\yrs\|\yRf)\Bigr],
\end{equation}
...
c = \yK\sum_{n\yrs=0}^n \sum_{N\yRf=0}^N f(\yrs) \ypp(\yrs \|\yRf)
\,
  \binom{N}{N \yRf}\exp\Bigl[\yL\T
  \sum_{n\yrs=0}^n f(\yrs)\ypp(\yrs\|\yRf)\Bigr].
\end{equation}
...
c = \yk\sum_{n\yrs=0}^n  f(\yrs) \binom{n}{n\yrs}
  \exp[\yl\T f(\yrs)].
\end{equation}
...
\eqref{eq:app_maxent_sample} will be approximately equal, by our previous
observation~\eqref{eq:roughly_equal_nN} and also by the fact that they must
satisfy the same expectations for $f$. But the latter may miss important
features present in the former, like additional modes or particular tail
behaviour.
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.75\linewidth]{different_maxent_pop_sample_200_realdata_2mom.pdf}%
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.75\linewidth]{different_maxent_pop_sample_200_realdata_4mom.pdf}
% 
