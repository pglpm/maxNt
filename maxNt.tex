...
%% Last-Updated: 2017-05-17T12:40:23+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
sample variable $\yXf$ and of the population variable $\yxs$. It is a
projector $\ypp$ of probabilities $p$ for $\yXf$ onto marginal
probabilities for $\yxs$:
\begin{equation}
  \label{eq:projection_P}
  \ypp\colon p(\yXf=\dotv) \to p_*(\yxs=\dotv)
  \defd \sum_{N\yRf=0}^N \ypp(\dotv\|\yRf) p(\yRf).
\end{equation}
It is also a pull-back $\ypp^*$ of functions $f$ of $\yxs$ to functions of
$\yXf$:
\begin{equation}
  \label{eq:pullback_P}
  \ypp^*\colon f(\yxs=\dotv) \to f^*(\yXf=\dotv) \defd \sum_{N\yrs=0}^n f(\yrs)\ypp(\yrs\|\dotv),
\end{equation}
with a caveat: if $f$ is a physical quantity and doesn't depend on our
knowledge, $f_*$ is \emph{not} a physical quantity but a conditional
expectation, $f^*(\yRf) \equiv \expe{f(\yxs)\|\yXf=\yRf,\yH}$, and depends
on our knowledge.

\begin{figure}[!t]
\centering
%\includegraphics[width=0.99\linewidth]{pop_sample_projection.pdf}\\
...
A look at a plot of the hypergeometric distribution $\ypp(\yrs\|\yXf)$, for
example \fig~\ref{fig:hypergeom_proj}, reveals that the maps $\ypp$ and
$\ypp^*$ are \enquote{fuzzy identities} between the $\yXf$ space
$\set{0,\dotsc,N}$ and $\yxs$-space $\set{0,\dotsc,n}$. When $n=N$ they are
exact identities. We thus have very roughly that
\begin{equation}
  \label{eq:roughly_equal_nN}
  \p(\yxs=a) \sim \p(\yXf=a),\qquad
\expeb{f(\yxs)} \sim \expeb{f(\yXf)}.
\end{equation}
These formulae express the fact that \emph{the sample is representative of
  the population}, and vice versa. Our initial knowledge $\yH$, which fully
determines $\ypp$, thus assumes that by observing a sample we can make
predictions about the full population, and thence about new samples.
Upon observation of a sample average, say $\yxxs=a$, the updated
expectations for such average in the population and in any new sample will
usually be shifted towards the observed value, as follows from Bayes's
theorem and the formulae above.

Note that the approximate formulae~\eqref{eq:roughly_equal_nN} say more
than the limits $\p(\yxs=a) \to \p(\yXf=a)$,
$\expeb{f(\yxs)} \to \expeb{f(\yXf)}$ as $n\to N$. These limits are
trivially valid because as $n\to N$ the sample becomes the full population.
For conditional probabilities $\p(\yxs = \yrs \|\yXf=\yRf)$ very different
from the fuzzy identity $\ypp(\yrs\|\yRf)$ (see
\fig~\ref{fig:hypergeom_proj}) these limits would hold, but the
probabilities and expectations from a small sample could wildly differ from those
from the population.
...
A special case of the pull-back above is the function
$f(\yxs) \defd \sav{\yx\dots\yx}=\binom{\yxs}{m}/\binom{n}{m}$, for which
\begin{equation}
  \label{eq:pullback_m_expectations}
  \ypp^*\colon \sav{\underbrace{\yx \dotsm \yx}_{\text{$m$ factors}}}
  \mapsto \bigl( \sav{\underbrace{\yx \dotsm \yx}_{\text{$m$ factors}}}\bigr)^*
  \equiv
  \av{\underbrace{\yX \dotsm \yX}_{\text{$m$ factors}}};
\end{equation}
thus, the averages of activity products \emph{are the same for the sample
...
This is true, for example, for the raw moments $\expe{\yxs^m}$ and
$\expe{\yXf^m}$, which can be written in terms of the product expectations
via \eqn~\eqref{eq:expe_products}.
