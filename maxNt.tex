...
We would like to warn about an implicit assumption of the \me\ method,
which can informally be put this way:
\begin{quote}
  \emph{\me\ method applied to $\yN$ units assumes that no other
  units exist.}
\end{quote}
Less informally, the method implicitly assumes that the $\yN$ units are not a
subnetwork sample of a network with $\yM$ units, $\yM>\yN$.
...
precede a \me\ application already involve probability considerations at
the level of the full network. The usual reasoning by enumeration
implicitly assumes a uniform distribution, or a ***
