...
%% Last-Updated: 2017-05-18T01:43:11+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
observations are made. %, no matter what kinds of predictions we are interested in.
This initial assignment will be modified by our experimental
observations.
...
This theorem is intuitive: owing to symmetry, all states with $N\yRf$
active neurons must have equal probabilities.
...
satisfy the same expectations for $\yf$. But the latter may miss important
...
answer. An answer can only be given case by case, depending on the
inferences we are trying to make, on the assumptions needed and those
negligible, and on our computational power. We want to stress, in fact, the
...
The first purpose of this note was to remind our readers that probability
models can have hidden assumptions that we don't want to make or that are
in contradiction with assumptions we want to make. The dilemma generated by
the \me\ method and the assumption of random representative sampling is an
example. The usual application of \me\ at the sample level generates a
different distribution than the application at the population level. Then
the former application must implicitly assume that no larger population
exist; or in other words that the neurons of the sample are isolated from
their environment.
...
one obtained by application at the sample level. The former distribution
says that some sample states $\yx$ should be considered less likely than in
the latter distribution. A lower entropy signals the presence of an
additional assumption. Such assumption, intuitively, is that the sample is
part of a larger population, and our ignorance about the former is
representative of our ignorance about the latter. This assumption may
affect our predictions.

In a neuroscientific context we find this assumption very natural, and
therefore the \me\ method at the population level preferable. Also
\emph{physical} neuronal-network models add indeed an external input to the
neurons, mimicking their embedding in a larger network.
