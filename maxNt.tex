...
%% Last-Updated: 2017-05-15T01:20:54+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
$\p(\yxs=\yrs \| \yH)$; the two applications, however, are
\emph{inequivalent}. They lead to numerically different
$\p(\yxs=\yrs \| \yH)$; see \fig~\ref{fig:diff_maxent_pop_sample} for an
example. Suppose our constraints are $\expe{\yxs \|\yH}$ and
$\expe{\yxxs \|\yH}$. Application of \me\ at the population level leads to
the distribution
\begin{equation}
...
\end{equation}
with such parameters $\yL_1$, $\yL_2$ as to satisfy the constraints.
Marginalization by \eqn~\eqref{eq:marginal} leads to 
\begin{equation}
...
\yL_2\, \frac{\yRf\,(N \yRf-1)}{N-1} + \yL_1\, \yRf\biggr].
\end{equation}
Application of \me\ at the sample level leads to the distribution
\begin{equation}
...
\end{equation}
with such parameters $\yl_1$, $\yl_2$ as to satisfy the constraints.
Expressions \eqref{eq:full_pop_av_example_ME_marg} and
\eqref{eq:sample_av_example_ME} cannot be equal when $n>6$ because, even if
we could choose their four parameters and two normalization constants
freely (and we can't), for their equality we should still satisfy $n+1$ equations,
\ie\ more than six.
