...
%% Last-Updated: 2018-05-14T16:10:02+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
recent electrophysiological techniques \cite{berenyietal2014}. In 1985,
Gerstein, Perkel, \amp\ Dayhoff wrote:
\begin{quote}
  ...
  well as practical. The principal conceptual problems are (1) defining
  cooperativity or functional grouping among neurons and (2) formulating
  quantitative criteria for recognizing and characterizing such
  cooperativity. \cite{gersteinetal1985}
\end{quote}
 More than thirty years later these problems are
still under investigation.

Some investigations in the neuroscientific literature proceed as follows.
We bin in time and binarize in intensity
\cite{caianiello1961,caianiello1986} the recorded activity, transforming it
into a sequence of $0$s (inactive) and $1$s (firing) for each neuron.
Groups of two or more $1$s can appear in any time bin. Then we ask a
question of this kind: can the presence of groups of three or more
simultaneous firings be \enquote{explained} in terms of groups of just two,
or three, or\ldots? This question is very vague until we define what we
mean with \enquote{explain} or similar words. Usually such
\enquote{explanation} has a probabilistic meaning: we wonder whether the
sole knowledge about two, or three, or more simultaneous firings can yield
as good predictions about the whole activity as a more complete knowledge
would do. To make this question concrete we must first choose a
probabilistic model from some well-defined assumptions, and then assess
whether its final probabilities are \enquote{good} according to some
well-defined measure. In the following we consider only the first,
model-building, step.

Maximum-entropy probability models
\cite{jaynes1957,jaynes1963,sivia1996_r2006,meadetal1984} have been used in
neuroscience with this kind of purpose, for recordings from brain areas as
diverse as retina and motor cortex, at least since the 1990s
...
Part of this literature focuses on the range of applicability of \me\
distributions
\cite{tkaciketal2006,roudietal2009,roudietal2009b,barreiroetal2010b,mackeetal2013,rostamietal2016_r2017}.
We assume that our readers are at least vaguely familiar with this kind of model.
In brief, it builds a probability distribution for neuronal activity using
only data about simultaneous firings of two neurons, or of three, and so
on. Let us take the case of the first two moments as an example. The
maximum-entropy distribution can be derived in three main ways:
\begin{enumerate}
\item\label{item:maxent}From the maximum-entropy method
  \cite{jaynes1957,jaynes1963,sivia1996_r2006}. The distribution is chosen
  as the one with the highest Shannon entropy among those having particular
  first and second moments. This distribution is deemed to be
  \enquote{maximally noncommittal} \cite{jaynes1963}, but this adjective
  means little without further technical characterization. Two
  characterizations are given by the next two derivations.

\item\label{item:entropic_prior}As the asymptotic distribution of a Bayesian model called
  \enquote{exchangeable}, with a particular prior
  \cites{jaynes1986d_r1996,skilling1989b,skilling1990,rodriguez1989,rodriguez2002,portamana2017}[see
  also:][\chap~VI]{schroedinger1946_r1948}%[pp.~52--55]
  {jaynes1979b}. This distribution is the limit, when the number of time
  bins is very large, of
  \begin{equation}
    \label{eq:predictive_exchangeability}
    \p(\text{\small activity in one time bin} \|
    \text{\small 1st \amp\ 2nd empirical moments from other bins}%,\text{\small other info}
    ),
  \end{equation}
  calculated with Bayes's theorem from a particular initial joint
  distribution in which different time bins are almost independent. This
  derivation produces a probability just by taking into account those two
  moments, but does not exclude that other information might be relevant.
  It can therefore be compared with a probability that uses additional
  information, or with empirical frequencies.

\item\label{item:sufficiency_model}As the asymptotic distribution of a Bayesian model based on
  sufficient statistics \cite{jaynes1982,%[\sect~I]
    jaynes1982b,portamana2017}. This distribution is the limit, when the
  number of time bins is very large, of
\begin{equation}
  \label{eq:general_prediction}
  \p(\text{\small activity in one time bin} \|
  \text{\small activities in other bins}%,\text{\small other info}
  )
\end{equation}
under the assumption that \emph{only} the values of the first and second
...
\end{multline}
in other words, the assumption that the first and second moments are
sufficient statistics. This limit generally does not depend on the initial
joint distribution. This derivation produces a probability by explicitly
assuming that only the first and second moments are informationally
relevant: any other information would lead to the same conditional
probability. It can therefore only be compared with empirical frequencies.
\end{enumerate}
We refer our readers to the cited references for a deeper discussion of
these derivations. It is remarkable that all three lead to the same
distribution. We shall now see, however, that if we apply these derivations
more carefully they lead to different distributions.

\bigskip

This kind of investigation of recorded samples of neuronal activity would
have little significance if we did not expect its results to generalize
beyond the particular recorded sample. In other words, we expect that the
conclusions drawn with \me\ methods about the recorded neurons should
somehow generalize or extrapolate at least to larger areas from which those
neurons are recorded. We are thus assuming that the recorded sample is a
\emph{representative sample} of some larger area. Whether such
extrapolation makes biological sense is a difficult question, to be
analysed case by case. For the moment let's assume that it does make
biological sense, and that our sample can be assumed to be representative
of a larger area.

Then a double dilemma appears in the way the \me\ method is typically
applied in the neuroscientific literature to recorded or artificial
samples. In this note we want to discuss one part of this dilemma and offer
a solution to it.

We can apply the \me\ method to the sample, choosing some set of moments,
and generate a probability distribution for the state of the sample. But if
we consider our sample to be representative of a larger population, it
makes sense to apply the \me\ method to the larger population, using one of
the three derivations above, and generate a probability distribution for
the larger population. The distribution for the sample can then be found by
marginalization. Here's the double dilemma:
\begin{itemize}
\item \emph{the distribution obtained from the application at the sample
    level differs from those obtained from the application at the
    population level};
\item \emph{at the population level, the
    derivations~\ref{item:entropic_prior} and~\ref{item:sufficiency_model}
    lead to different distributions} (the derivations \ref{item:maxent}
  and~\ref{item:entropic_prior} always lead to the same distribution).
\end{itemize}

\hrule
\mynote{delete this part}
...
In the rest of the paper we mathematically formulate and analyse the first
part of this dilemma, focusing on the derivation~\ref{item:entropic_prior}.
To this purpose we also present some probability relations relevant to
sampling. The relations we present are well-known in survey sampling and in
...
literature, so they may be of interest on their own. We briefly discuss
also two very different meaning of \enquote{representative} that lead to
similar initial probability assignments for sampling.
...
\mynote{DISCUSSION ABOUT THE SECOND STATE OF KNOWLEDGE IS TO BE DISCARDED}
...
the symmetric distribution of $\yH$ must obey
\begin{equation}
...
assign equal probabilities to all states with $N\yRf$ active neurons.
...
\begin{gather}
  ...
  \p(\yXf=\yRf \| \yH),
  \\[2\jot]
  \shortintertext{with the conditional probability}
...
Our initial symmetric knowledge should intuitively also apply to the
...
How is it possible that the very different states of knowledge $\yHp$ and
$\yHi$ lead to the same formulae above? Their difference appears as soon as
we make an experimental observation, say $X_2 = R_2\in\set{0,1}$ and update our
initial probabilities \eqref{eq:joint_plaus_N_homog}:
\begin{equation}
  \label{eq:updated_joint}
    \p(\yX = \yR \| X_2 = R_2, \yH) \equiv
  \p(\yX = \yR \| \yXf = \yRf, X_2 = R_2, \yH)\,
  \p(\yXf=\yRf \| X_2 = R_2,\yH).
\end{equation}
The conditional probability and the probability for the average on the
right side will update in very different ways for $\yHp$ and $\yHi$. The
discussion of this update process in the two cases is unnecessary for the
purposes of this note and outside their scope; we hope to address it in
full in a future work.

\hrule

The conditional probability
$\p(\yxs = \yrs \|\yXf=\yRf, \yH)\equiv\ypp(\yrs\|\yRf)$ relates the spaces
of the sample average $\yXf\in\set{0,\dotsc,N}$ and of the population
average $\yxs\in\set{0,\dotsc,n}$. It is a coarsening projector of any
probability $p$ for $\yXf$ onto a marginal probability, as
\eqn~\eqref{eq:subpop_average} shows. It also allows us to construct a
function of the population average $f^*(\yXf)$, given a function of the
sample average $f(\yxs)$:
\begin{equation}
  \label{eq:pullback_P}
  f^*(\yXf) \defd \sum_{n\yrs=0}^n f(\yrs)\ypp(\yrs\|\yXf),
\end{equation}  
with the property of having the same expectation value as the latter:
\begin{equation}
  \label{eq:same_expect}
  \expeb{f^*(\yXf)} \equiv
...
A look at a plot of the hypergeometric distribution $\ypp(\yrs\|\yRf)$,
...
$f(\yxs) = \sav{\yx\dots\yx}\equiv\binom{n\yxs}{m}/\binom{n}{m}$,
 the expectations at the population and at the sample
levels are exactly the same, as can be seen from \eqn~\eqref{eq:same_expect}:
\begin{equation}
...
The second example uses two additional constraint functions
$\expe{\sav{\yx\yx\yx}}$, $\expe{\sav{\yx\yx\yx\yx}}$. The distribution
obtained at the population level has two modes, replaced by only one in the
distribution obtained at the sample level; the tails are very different
also in this case.

\begin{figure}[!t]
\centering
\includegraphics[width=0.9\linewidth]{newplot.pdf}
% 
\caption{\mynote{to be written} distributions obtained using
  formulae~\eqref{eq:app_maxent_pop_marg_unknown_N} and the other two}
\label{fig:newplot}
\end{figure}%maxent_pop_or_sample.nb
...
usually \enquote{explain} has a probabilistic meaning. We want to see
whether information about moments give reasonable predictions about the
rest. Necessity of making this precise. Three ways:

-- \me\ method,

-- Bayesian with entropic prior,

-- Bayesian sufficiency.

Applied to sample, all three are asymptotically equivalent. However, sample
is representative of population. Need to take full population into
consideration. This leads to different formulae, and also to
non-equivalence of third way. Here we focus on first two.
...
\begin{equation}
 ...
 \pf(\ytheta\| \text{\small other info})\,
  ...
  \text{with \enquote{$\pf(\text{\small activity at time bin } i \|\ytheta)$}
    having \me\ form with multipliers $\ytheta$};
\end{gathered}
\end{equation}
...
\me\ distribution
\cite{jaynes1957,jaynes1963,sivia1996_r2006,meadetal1984} with those
Lagrange multipliers. This kind of distributions has been used in
neuroscience at least since the 1990s to study correlations from diverse
points of view, for diverse purposes, and with data recorded from diverse
brain areas
\cite{mackay1991,martignonetal1995,bohteetal2000,amarietal2003,schneidmanetal2006,shlensetal2006,mackeetal2009b,roudietal2009c,tkaciketal2009,gerwinnetal2009,mackeetal2011,mackeetal2011b,ganmoretal2011,granotatedgietal2013,tkaciketal2014b,moraetal2015,shimazakietal2015}.
Part of this literature focused on the range of applicability of
\me\ distributions
