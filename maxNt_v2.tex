...
%% Last-Updated: 2018-05-14T14:56:53+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
\begin{enumerate}
\item From the maximum-entropy method
...
\item As the asymptotic distribution of a Bayesian model called
...
\item As the asymptotic distribution of a Bayesian model based on
...
beyond the particular recorded sample. In other words, we implicitly or
explicitly expect that the conclusions drawn with \me\ methods about the
recorded neurons should somehow generalize or extrapolate at least to whole
areas those neurons are recorded from, \eg\ retina or motor cortex. We are
thus assuming that the recorded sample is a \emph{representative sample} of
that area. Whether such extrapolation makes biological sense is a difficult
question, to be analysed case by case. For the moment let's assume that it
does make biological sense, and that our sample can be assumed to be
representative of its brain area.

Then a problem appears in the way the \me\ method is typically applied, in
the literature, to recorded or artificial samples. In this note we want to
discuss this problem and offer a solution to it.

The problem is this. We can apply the \me\ method to the sample, choosing
some sufficient statistics, and generate a probability distribution for the
state of the sample. But we consider our sample to be representative of a
larger population, in which it is biologically and functionally embedded.
We can apply the \me\ method to the larger population, using the same kind
of sufficient statistics, and generate a probability distribution for its
...
features. However, \emph{the distributions obtained from these two
  applications differ}. It goes without saying that if one is
\enquote{noncommittal}, the other must be \enquote{committal}. Which choice
is most meaningful?
