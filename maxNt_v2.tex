...
%% Last-Updated: 2018-05-13T23:58:06+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
Some investigations in the literature proceed as follows. We bin in time
and binarize in intensity \cite{caianiello1961,caianiello1986} the recorded
activity, transforming it into a sequence of $0$s (inactive) and $1$s
(firing) for each neuron. Groups of two or more $1$s can appear in any time
bin. Then we ask a question of this kind: can the presence of groups of
three or more simultaneous firings be \enquote{explained} in terms of
groups of just two, or three, or\ldots? This kind of question is very vague
until we define what we mean with \enquote{explain} or similar words.
Usually such \enquote{explanation} has a probabilistic meaning: we wonder
whether the sole knowledge about two, or three, or more simultaneous
firings can yield as good predictions about the whole activity as a more
complete knowledge would do. To make this question concrete we must choose
a probabilistic model.

Maximum-entropy models
\cite{jaynes1957,jaynes1963,sivia1996_r2006,meadetal1984} have been used in
neuroscience with this kind of purpose and for diverse brain areas at least
since the 1990s
...
Part of this literature focused on the range of applicability of \me\
distributions
\cite{tkaciketal2006,roudietal2009,roudietal2009b,barreiroetal2010b,mackeetal2013,rostamietal2016_r2017}.
We assume that our readers are at least vaguely familiar with such models.
In brief, they build a probability distribution for neuronal activity using
only data about simultaneous firings of two neurons, or of three, and so
on. Let us take the case of the first two moments just as an example. The
corresponding maximum-entropy probability distributions can be derived in
three main ways:
\begin{enumerate}
\item From the maximum-entropy method
  \cite{jaynes1957,jaynes1963,sivia1996_r2006}. The distribution is chosen
  as the broadest one, in the Shannon-entropy sense, among those having
  particular first and second moments.
\item As asymptotic distributions of Bayesian models called
  \enquote{exchangeable} with entropic priors
  ...
  {jaynes1979b}. The distribution is the limit, when the number of time
  bins is very large, of
  \begin{equation}
    \label{eq:predictive_exchangeability}
    \p(\text{\small activity at some time bin} \|
    \text{\small 1st \amp\ 2nd empirical moments from observed bins},\text{\small other info});
  \end{equation}
  obtained from Bayes's theorem \emph{given} the observed empirical moments
  and a particular initial joint distribution.
\item As asymptotic distributions of Bayesian models based on sufficient
  statistics
  \cite{jaynes1982,%[\sect~I]
jaynes1982b,portamana2017}. The distribution is the limit, when the number of time
bins is very large, of
\begin{equation}
...
\end{equation}
under the assumption that only the values of the first and second empirical
moments of the activities are \emph{relevant} for the probability above, \ie\
\begin{multline}
...
\emph{sufficient statistics}. This limit generally does not depend on the initial
joint distribution.
\end{enumerate}
...
\me\ methods about the recorded neurons should somehow generalize or
extrapolate at least to whole areas those neurons are recorded from, \eg\
retina or motor cortex. We are thus assuming that the recorded sample is a
\emph{representative sample} of that area. Whether such extrapolation makes
biological sense is a difficult question, to be analysed case by case.

\bigskip

But a problem remains even if we assume that the sample is representative
of its brain area. The problem lies in the way the \me\ methods
is typically used in the literature. In this note we want to discuss this
problem and offer a solution to it.
