...
%% Last-Updated: 2018-05-15T22:50:14+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
\newcommand*{\pdftitle}{Representative samples\\ and maximum-entropy distributions:\\ a dilemma}
...
\begin{abstract}
  \mynote{to be rewritten} This note shows that the \me\ method can be
  applied to a representative sample from a neuronal population along two
  different routes: (1) apply to the sample; or (2) apply to the population
  and marginalize to the sample. These two routes give inequivalent
  results. Which route should be chosen? Some arguments are presented in
  favour of the second. The note also touches upon probability formulae of
  representative sampling and discusses their possible meanings, a
  discussion that may be useful for sampling problems in neuroscience.
...
Gerstein, Perkel, \amp\ Dayhoff \cite{gersteinetal1985} posed this question
very tersely (our emphasis):
\begin{quote}
  ...
  cooperativity.
\end{quote}
These questions have a long history, of course; see \eg\ the 1966 review by
Moore \etal\ \cite{mooreetal1966}. % This scenario is a concrete possibility
...
\begin{enumerate}
\item The activity of each neuron, a continuous signal, is binned in time
  and binarized in intensity, and thus transformed into a sequence of
  digits \enquote{$0$}s (inactive) and \enquote{$1$}s
  \cite[\cf][]{caianiello1961,caianiello1986}.

  Let the variable $\yvx_i(t)\in\set{0,1}$ denote the activity of the $i$th
  sampled neuron at time bin $t$. The average activity at that bin is
  therefore $\yxs(t) \defd \sum_i \yvx_i(t)/n$. If we count the number of
  distinct pairs of active neurons at that bin we combinatorially find
  $\tbinom{n\yxs(t)}{2}\equiv n\yxs(t)\,[n\yxs(t)-1]/2$. There can be at
  most $\tbinom{n}{2}$ simultaneously active pairs, so the average pair
  activity is
  $\av{\yx \yx}(t) \defd \tbinom{n}{2}^{-1}\tbinom{n\yxs(t)}{2}$. With some
  combinatorics we see that the average activity of $m$-tuples of neurons
  is
  \begin{equation}
    \label{eq:products_intermsof_average}
    \av{\underbrace{\yx\dotsm\yx}_{\text{$m$ terms}}}
    = \binom{n}{m}^{-1}\binom{n\yxs(t)}{m}.
  \end{equation}
  For brevity let us agree to simply call \enquote{activity} the average
  $\yxs$, \enquote{pair-activity} the average $\av{\yx \yx}$, and so on.

\item Construct a sequence of relative-\me\ distributions for the activity
  $\yxs$, using this sequence of constraints:
  \begin{itemize}
  \item the time average of the activities $\yxs(t)$;
  \item the time averages of the activities $\yxs(t)$ and pair-activities
    $\av{\yx \yx}(t)$;
  \item \ldots and so on, including the time averages of $\yxs(t)$ up to
    $\av{\yx\dotsm\yx}(t)$ with $k$ terms.
  \end{itemize}
  Call the resulting distributions $p_1(\yxs), p_2(\yxs),\dotsc,p_k(\yxs)$.
...
\enquote{hidden} neurons. The problem stems from the fact that the recorded
neurons are a \emph{sample} from a larger, unrecorded population.
...
device; such population can amount to tens of thousands of neurons
...
Here is the problem. We can apply the \me\ method to the sample, as
described in the above protocol, to generate probability distributions for
the activity of the sample. But, if our sample is representative of a
larger population, we can apply the \me\ method to the larger (unrecorded)
population, using the constraints obtained from the recorded sample, and
generate a probability distribution for the larger population. The
distribution for the sample is then obtained by marginalization. Either
application seems to have some commendable features. However, \emph{the
  distributions obtained from these two applications differ}. Which choice
is most meaningful?

In this paper we develop the second way of applying the \me, at the level
of the larger population, and show that its results differ from the
...
so they may be of interest on their own.

\iffalse In the final discussion we present some arguments in favour of one choice
...
The notation and terminology in this note follows ISO and ANSI standards
...
\section{Notation: population, sample, probabilities}
\label{sec:setup}

We have a population of $\yvN$ binary neurons. We assume that they can be
...
$(\yvX_1,\dotsc,\yvX_\yvN) \defs \yX$ having fixed but unknown binary values
$(\yvR_1,\dotsc,\yvR_\yvN)\defs \yR \in \set{0,1}^\yvN$; the \enquote{$\yvX$} denote the
variables, and \enquote{$\yvR$} their values. A particular sample of $n$ neurons
from this population has joint state $(\yvx_1, \dotsc, \yvx_n) \defs \yx$ having
fixed binary values $(\yvr_1, \dotsc, \yvr_n)\defs\yr \in \set{0,1}^n$. We will
consider various averages of the population and the sample. For this
purpose we introduce a population-averaging operator $\sav{\dotv}$ defined
by
\begin{equation}
  \begin{gathered}
    \av{\yX} \defd \tfrac{1}{\yvN} (\yvX_1 + \yvX_2 + \dotsb + \yvX_\yvN),
    % & \sav{\yx} &\defd \tfrac{1}{n} (x_1 + x_2 + \dotsb + x_n),
   \qquad% \\[2\jot]
    \av{\yX \yX} \defd \tbinom{\yvN}{2}^{-1} (\yvX_1 \yvX_2 + \yvX_1 \yvX_3 + \dotsb +
    \yvX_{\yvN-1} \yvX_\yvN),
    % & \sav{\yx \yx} &\defd\tbinom{n}{2}^{-1} (x_1 x_2 + x_1 x_3 + \dotsb
    % + x_{n-1} x_n),
    \\[2\jot]
    \av{\yX\yX\yX} \defd \tbinom{\yvN}{3}^{-1} (\yvX_1 \yvX_2 \yvX_3 +% \yvX_1 \yvX_2 \yvX_4
    \dotsb + \yvX_{\yvN-2} \yvX_{\yvN-1}\yvX_\yvN),
    \qquad\text{and so on.}
...
the fraction of simultaneously active pairs out of all $\binom{\yvN}{2}$
pairs, $\av{\yX\yX\yX}$ the fraction of simultaneously active triplets, and
so on. Products of states like $\yvX_i \dotsm \yvX_j$ also have values in
$\set{0,1}$; from this we can combinatorially  prove that
\begin{equation}
  \label{eq:products_intermsof_average}
  \av{\underbrace{\yX\dotsm\yX}_{\text{$m$ factors}}}
  = \binom{\yvN}{m}^{-1}\binom{\yvN\yXf}{m}.
\end{equation}
...
\begin{simplenote}
  ...
  $\set{\textrm{X}_1, \textrm{X}_2}$ is random because we do not know the
  ...
  two samples are different: the former is obtained from marginalization, the
...
\begin{equation}
  ...
  \p(\yvX_1=\yvR_1, \yvX_2=\yvR_2, \dotsc, \yvX_\yvN=\yvR_\yvN \| \yHc) \quad\text{or}\quad
\p(\yX =\yR \| \yHc), \quad \yR \in \set{0,1}^\yvN,
\end{equation}
...
\begin{equation}
  ...
  \p(\yvx_1=\yvr_1, \yvx_2=\yvr_2, \dotsc, \yvx_n=\yvr_n \| \yHc) \quad\text{or}\quad
...
We need to make an initial probability assignment for the state of the
population before any experimental observations are
...
which can involve only a sample of the population. Our initial probability
assignment should reflect that samples are somehow representative of the
whole population. We consider here a state of knowledge that express
...
some biologic or physical identity of the neurons.


The \emph{representation theorem for finite exchangeability} states that
the symmetric distribution in the state of knowledge $\yH$ must obey
\begin{equation}
  \label{eq:joint_plaus_N_homog}
  \p(\yX = \yR \|  \yH) \equiv
  \p(\yX = \yR \| \yXf = \yRf, \yH)\,
  \p(\yXf=\yRf \| \yH) =
  \binom{\yvN}{\yvN\yRf}^{-1} \p(\yXf=\yRf \| \yH),
\end{equation}
the latter being the probability for the population average $\yX$. A sum is
only apparently missing in the central term: its summands $\yXf=A$ are all
zero except for $A=\yRf$. Proof of this theorem and generalizations to
...
\cite{heathetal1976}. This theorem is intuitive: owing to symmetry, we must
assign equal probabilities to all states with $\yvN\yRf$ active neurons.
...
\begin{gather}
  ...
  \p(\yxs=\yrs \| \yH) = \sum_{\yvN \yRf =0}^{\yvN}
    \p(\yxs = \yrs \|\yXf=\yRf, \yH)\,
  \p(\yXf=\yRf \| \yH),
  \\[2\jot]
  \label{eq:conditional_hypergeometric}
  \p(\yxs = \yrs \|\yXf=\yRf, \yH)=
  \binom{n}{n\yrs}\binom{\yvN-n}{\yvN \yRf-n\yrs}\binom{\yvN}{\yvN \yRf}^{-1}
  \defs \ypp(\yrs\|\yRf).
\end{gather}
The conditional probability in the last formula is a hypergeometric
distribution $\ypp(\yrs\|\yRf)$, typical of \enquote{drawing without
  replacement} problems. The combinatorial proof of the formulae above is
in fact the same as for this class of problems
...
sample; the probability for the state of the
sample~\eqref{eq:subpop_average} indeed satisfies the representation
theorem~\eqref{eq:joint_plaus_N_homog} as well.

The conditional probability
$\p(\yxs = \yrs \|\yXf=\yRf, \yH) \equiv \ypp(\yrs,\yRf)$ relates the
spaces of the sample average $\yXf\in\set{0,\dotsc,\yvN}$ and of the
population average $\yxs\in\set{0,\dotsc,n}$. It is a coarsening projector
of any probability for $\yXf$ onto a marginal probability for $\yxs$, as
\eqn~\eqref{eq:subpop_average} shows. For each function $f$ of the sample
average $\yxs$, the  hypergeometric distribution 
$\ypp(\yrs,\yRf)$ also allows us to construct a function $f^*$ of the
population average $\yXf$
\begin{equation}
  \label{eq:pullback_P}
...
%  f(\yrs)\ypp(\yrs\|\dotv),
\end{equation}
that has the same expectation as $f$:
\begin{equation}
  \expeb{f^*(\yXf)}
  \equiv
  \sum_{\yvN\yRf=0}^\yvN f^*(\yRf)\,\p(\yXf=\yRf \| \yH)
%  =
%  \sum_{N\yRf=0}^N \sum_{n\yrs=0}^n f(\yrs)\ypp(\yrs\|\yRf)\,\p(\yXf=\yRf \| \yH)
  ...
  \equiv  \expeb{f(\yxs)}.
\end{equation}
The middle equality uses the combination of formulae~\eqref{eq:pullback_P}
and~\eqref{eq:subpop_average}.
...
\caption{Log-plot of the hypergeometric distribution
  $%\p(\yxs = \yrs \|\yXf=\yRf, \yH)=
\ypp(\yrs\|\yRf) \defd  \raisebox{0pt}[0pt][1ex]{}\binom{n}{n\yrs}\binom{\yvN-n}{\yvN \yRf-n\yrs}\binom{\yvN}{\yvN \yRf}^{-1}$ for $\yvN=5000$, $n=200$. (Band artifacts may appear in the
...
\fig~\ref{fig:hypergeom_proj}, reveals that it is a sort of \enquote{fuzzy
  identity matrix} between the $\yXf$-space $\set{0,\dotsc,\yvN}$ and
$\yxs$-space $\set{0,\dotsc,n}$. When $n=\yvN$ it is the identity matrix. We
thus have that
\begin{equation}
...
The approximate equalities above, on the other hand, express the fact that
\emph{our uncertainty about the sample is representative of our uncertainty
  about the population and about other samples}, and vice versa. These
approximate equalities are not generally valid for other probability
distributions for the population. They should be contrasted with the limits
$\p(\yxs=a) \to \p(\yXf=a)$ and $\expeb{f(\yxs)} \to \expeb{f(\yXf)}$, as
$n\to \yvN$, do: these limits are trivially, universally valid because the
sample becomes the full population as $n\to \yvN$. In particular, these limits
...
%   \qquad\text{or}\qquad
%   \sum_{n\yrs=0}^n f(\yrs)\,p_*(\yxs=\yrs) = \sum_{\yvN\yRf=0}^\yvN f^*(\yRf)\,p(\yXf=\yRf).
% \end{equation}
For averaged products, $\sav{\underbrace{\yx\dots\yx}_{\text{$m$
      factors}}}\equiv\binom{n\yxs}{m}/\binom{n}{m}$,
formulae~\eqref{eq:pullback_P} yield the useful equality
\begin{equation}
...
\equiv
    ...
    = \binom{\yvN}{m}^{-1}
    % \frac{(N-m)!}{N!}
    \sum_{\mathclap{\yvN \yRf=0}}^\yvN %m!\,
    \binom{\yvN \yRf}{m}\, \p(\yXf=\yRf \| \yH)
\equiv    \expe{\av{\underbrace{\yX \dotsm \yX}_{\text{$m$ factors}}} \| \yH}.
\end{equation}
The proof of the middle equality uses the expression for the $m$th
factorial moment of the hypergeometric distribution \cite{potts1953}. Thus,
in the state of knowledge $\yH$ the expectations of the averages of
activity products \emph{are initially the same for the sample and for the
  full population}. Similar relations can be found for the raw moments
...
\section{Maximum-entropy at the population level with constraints at the
  sample level}
...
constrains $\yvN+1$ unknowns by only $n+1$ equations.
...
is, $\expeb{\yf(\yxs)}=\yc$. Application of \me\
\cite{sivia1996_r2006,meadetal1984} at the population level, denoted by
$\yHa$, gives
\begin{equation}
  \label{eq:app_maxent_pop}
  \p(\yXf = \yRf \| \yHa)  = \yK\,
  \binom{\yvN}{\yvN \yRf}\,\exp\Bigl[\yL\T
  \sum_{n\yrs=0}^n \yf(\yrs)\ypp(\yrs\|\yRf)\Bigr],
\end{equation}
and then by marginalization~\eqref{eq:marginal}
\begin{equation}
  \label{eq:app_maxent_pop_marg}
  \p(\yxs = \yrs \| \yHa)  = \yK\, 
  \sum_{\yvN\yRf=0}^\yvN \ypp(\yrs \|\yRf)
\,
  \binom{\yvN}{\yvN \yRf}\,\exp\Bigl[\yL\T
  \sum_{n\yrs=0}^n \yf(\yrs)\ypp(\yrs\|\yRf)\Bigr],
\end{equation}
...
\yc = \yK\sum_{n\yrs=0}^n \sum_{\yvN\yRf=0}^\yvN \yf(\yrs) \ypp(\yrs \|\yRf)
\,
  \binom{\yvN}{\yvN \yRf}\,\exp\Bigl[\yL\T
  \sum_{n\yrs=0}^n \yf(\yrs)\ypp(\yrs\|\yRf)\Bigr].
\end{equation}
...
Lagrange multipliers $\yL$ and $\yl$ to satisfy the constraint equations
\eqref{eq:app_maxent_pop_constraints},
\eqref{eq:app_maxent_sample_constraints}, and also
$\p(\yxs = \yrs \| \yHa) = \p(\yxs = \yrs \| \yHb)$; that is, $2m+n$
equations (one normalization is taken care of) in $2m$ unknowns. A solution can
exist, if at all, only for very special choices of constraints functions
$\yf$ and values $\yc$.
...
\caption{Linear and log-plots of $\p(\yxs = \yrs)$ constructed by \me\ at the population
  ...
  circles), \eqn~\eqref{eq:app_maxent_sample}, with $\yvN=5000$,
...
and~\ref{fig:diff_maxent_pop_sample_realdata}, for $\yvN=5000$, $n=200$, and
...
\iffalse The first purpose of this note is indeed to remind ourselves that
probability models require careful scrutiny, because they can rest on
hidden assumptions that we do not want to make or that contradict others we
...
population: we do not know whether $\yvN=n$ or how large $\yvN$ is otherwise.
Call this state of ignorance $\yHd$. In the probability calculus this
ignorance about $\yvN$ is expressed by assigning a probability distribution
$\p(\yvN \| \yHd)$ that vanishes if $\yvN<n$, since we know that $\yvN\ge n$; see
...
assume a specific value of $\yvN$ are then obtained via multiplication of all
$\yvN$-dependent probabilities by $\p(\yvN \| \yHd)$ and subsequent
marginalization over $\yvN$. Technically speaking, $\yvN$ becomes a
...
\begin{equation}
  ...
  \p(\yxs = \yrs \| \yHd)  = \sum_\yvN \biggl\{\yK_\yvN\, 
  \sum_{\yvN\yRf=0}^\yvN \ypp_\yvN(\yrs \|\yRf)
\,
  \binom{\yvN}{\yvN \yRf}\,\exp\Bigl[{\yL_\yvN}\T
  \sum_{n\yrs=0}^n \yf(\yrs)\ypp_\yvN(\yrs\|\yRf)\Bigr]\biggr\}
  \,\p(\yvN \| \yHd),
\end{equation}
where $\yvN$-dependencies have been made explicit. This is a formidable
...
\eqref{eq:app_maxent_sample} equal for some choice of $\p(\yvN \| \yHd)$, with
$\p(\yvN \| \yHd)\ne 0$ for $\yvN>n$?}. We leave this mathematical problem for
future work. Note, however, that this equality is satisfied if
$\p(\yvN=1 \| \yHd)=1$, which means that the usual \me\ model can also be
...
the standard \me\ model. The question remains of how to specify $\yvN$,
though. We have tacitly intended $\yvN$ as the size of the largest
...
for example the primary visual cortex, for which $\yvN\sim 10^8$
\cite{leubaetal1994}. For large $\yvN$ -- unfortunately we are not yet able to
translate this \enquote{large} into a numeric order of magnitude -- the
final distribution becomes independent of $\yvN$, and continuous
...
\caption{Example of discrepancy in $\p(\yxs = \yrs \| \yH)$ constructed by
  ...
  $\expe{\yxxs \|\yH}=0.0025$, and $\yvN=5000$, $n=200$.}
\label{fig:diff_maxent_pop_sample_realdata}
\end{figure}%maxent_pop_or_sample.nb
The second is that \fi
...
for example $\expe{\yvx_2}$ or $\expe{\yvx_1x_8x_9}$. A short computation shows
...
% \mynote{Discussion of case when individual $\yvX_i\dotsm \yvX_j$ are constrained.
...
\begin{align}
  \expe{\yvx_{i_1} \dotsm \yvx_{i_m} \| \yH}
&=
\expe{\sav{\underbrace{\yx \dotsm \yx}_{\text{$m$ factors}}} \| \yH}
=
\expe{\av{\underbrace{\yX \dotsm \yX}_{\text{$m$ factors}}} \| \yH},
...
make statements about the whole population of $\yvN$ units and about a
...
are denoted by lowercase letters: $(\yvx_1, \dotsc, \yvx_n)\equiv\yx$ and
$(\yvr_1, \dotsc, \yvr_n)\equiv\yr$; but note that $\yvx_i \equiv \yvX_{j_i}$ and
$\yvr_i \equiv \yvR_{j_i}$ for some distinct $j_1,\dotsc,j_n$. We shall also make
statements about the population-averaged state, or \emph{population
  average}:
\begin{align}
  \label{noeq:population-av}
   \yXf &\defd (\yvX_1 + \dotsb + \yvX_\yvN)/\yvN,
\\
\intertext{and the subpopulation-averaged state, or \emph{subpopulation average}:}
\label{noeq:subpopulation-av}
\yxs &\defd (\yvx_1 + \dotsc + \yvx_n)/n.
\end{align}
The quantities $\yvN\yXf$ and $n\yxs$ represent the total number of active
...
$\tbinom{\yvN}{m}$ products of $m$ states; \eg,
\begin{align}
\av{\yX \yX} &\defd
\tbinom{\yvN}{2}^{-1} (\yvX_1 \yvX_2 + \yvX_1 \yvX_3  + \dotsb +  \yvX_{\yvN-1} \yvX_\yvN),
\\[2\jot]
\sav{\yx\yx\yx} &\defd
\tbinom{n}{3}^{-1} (\yvx_1 \yvx_2 \yvx_3 + \yvx_1 \yvx_2 \yvx_4 + \dotsb + \yvx_{n-2} \yvx_{n-1} \yvx_n),
\end{align}
and so on.
...
\begin{equation}
  ...
  \p(\yvX_1=\yvR_1, \yvX_2=\yvR_2, \dotsc, \yvX_\yvN=\yvR_\yvN \| \yH) \quad\text{or}\quad
\p(\yX =\yR \| \yH).
\end{equation}
...
\begin{multline}
  ...
  \p(\yvX_1=\yvR_1, \yvX_2=\yvR_2, \dotsc, \yvX_\yvN=\yvR_\yvN \| \yH) ={}
\\ 
\shoveright{\p(\yvX_1=\yvR_{\pi(1)}, \yvX_2=\yvR_{\pi(2)}, \dotsc, \yvX_\yvN=\yvR_{\pi(\yvN)} \| \yH)}
\\
...
=  Q(A),
\qquad
A \in \set*{0,\tfrac{1}{\yvN},\tfrac{2}{\yvN},\dotsc,1}.
\end{equation}
...
\begin{equation}
  ...
  \p(\yX = \yR \| H) = \binom{\yvN}{\yvN\yRf}^{-1} Q(\yRf).
\end{equation}
...
\begin{equation}
  \label{noeq:marginal}
    \p(\yx = \yr \| \yH) =
\sum_{\yvN A =0}^{\yvN} 
\binom{\yvN-n}{\yvN A-n\yrs}\binom{\yvN}{\yvN A}^{-1}\,
Q(A).
\end{equation}
Note that the only summands contributing to this sum are those for which
$n\yrs \le NA \le \yvN$; the others are zero because by definition
...
\begin{multline}
 ...
 \sum_{\yvN A=0}^{\yvN}
\binom{\yvN-n}{\yvN A-n a}\binom{\yvN}{\yvN A}^{-1}\,
Q(A),
\\
...
\begin{equation}
  ...
  \p(\yxs = a \| \yXf = A, \yH)
=
\binom{n}{n a}\binom{\yvN-n}{\yvN A-n a}\binom{\yvN}{\yvN A}^{-1}.
\end{equation}
...
\item\label{noitem:moments}The product of the states of any $m$ distinct
  ...
  \begin{equation*}
    \yvx_{i_1} \yvx_{i_2} \dotsm \yvx_{i_m},
...
=
  \tfrac{(\yvN-m)!}{\yvN!}\expeb*{\frac{(\yvN\yXf)!}{(\yvN\yXf-m)!} \| \yH}.
\end{equation}
...
\begin{subequations}\label{noeq:moments}
  \begin{align}
    ...
    \expe{\yxs^2 \| \yH} &= \expe{\yXf \| \yH}
                                  \frac{\yvN-n}{(\yvN-1)n}
 +{}
\\&\quad\quad \expe{\yXf^2 \| \yH}\frac{\yvN\,(n-1)}{(\yvN-1)n},
\end{split}
    \\[2\jot]
    \begin{split}
    \expe{\yxs^3 \| \yH} &=
                              \expe{\yXf \| \yH}\frac{(\yvN-n)(\yvN-2n)}{(\yvN-1)(\yvN-2)n^2}+{}
                                    \\
                                    &\quad\quad\expe{\yXf^2 \| \yH}
                                    \frac{3N\,(\yvN-n)(n-1)}{(\yvN-1)(\yvN-2)n^2} +{}
                                    \\
                                    &\quad\quad\quad\expe{\yXf^3 \| \yH}
                                    \frac{\yvN^2\,(n-1)(n-2)}{(\yvN-1)(\yvN-2)n^2}.
                                  \end{split}
                                    % \\[2\jot]
                                    % \expe{\yxs^4 \| \varEta} &=
...
\begin{equation}
  ...
  \expe{\yxs^m \| \yH} = \sum_{l=1}^m M_{ml}(n,\yvN) \expe{\yXf^l \| \yH},
\end{equation}
with a universal, lower-triangular transformation matrix $M_{ml}(n,\yvN)$ that
depends only on $n$, $\yvN$, and the condition of symmetry~\eqref{noeq:homog_math}.

As intuition suggests, we have 
%$\expe{\yxs^m \| \yH} \xrightarrow{n\to \yvN} \expe{\yXf^m \| \yH}$.
\begin{equation}
  \label{noeq:limit_moments}
  \expe{\yxs^m \| \yH} \xrightarrow{n\to \yvN} 
  \expe{\yXf^m \| \yH},
\qquad
  \expe{\yxs^m \| \yH} \xrightarrow{n\to 1} 
  \expe{\yXf \| \yH},
\end{equation}
the latter because ${\yvx_i}^m=\yvx_i$, since states are $\set{0,1}$-valued.
...
\cites[\chap~3]{jaynes1994_r2003}[\sect~4.8.3]{ross1976_r2010}[\sect~II.6]{feller1950_r1968},
with parameters $\yvN,\yvN\yXf,n$, weighted by the probabilities
...
\binom{\yvN}{\yvN A}\exp[
\yl_2\, \yvN A\,(\yvN A-1)/2 + \yl_1\, \yvN A].
\end{equation}
...
$Q_0$ defined by $Q_0(A) = 2^{-\yvN}\binom{\yvN}{\yvN A}$, corresponding to a
...
which $\yvN = 1000$ units, $\lambda_1=-2.55$, $\lambda_2=0.005$, and
...
$\expe{\yvx_{i_1}\dotsm \yvx_{i_m} \| \yH}$, proportional to the factorial
...
$\RR^{\yvN+1}$ to $\RR^{n+1}$, with $\yvN$ larger than $n$, is onto but not into.
...
\item the expectations of the single and pair averages $\yxs$ and
  ...
  \begin{equation}
    ...
    \expe{\yxs \| \yHa} = c_1, \qquad \expe{\sav{\yvx_i \yvx_j}\|\yHa} = c_2;
  \end{equation}
...
\begin{multline}
  ...
  \pf(\yX =\yR \|\yHa) =
\varKappa\exp[\yL_2 \yvN\yRf\,(\yvN\yRf-1)/2 + \yL_1 \yvN\yRf]
\\
\!\begin{aligned}[b]
&\text{with $\varKappa, \yL_m$, such that the distribution is normalized and}
\\
&\varKappa\sum_{\yvN A=0}^\yvN %m!\, 
\binom{\yvN-m}{\yvN A-m}
\exp[\yL_2 \yvN A\,(\yvN A-1)/2 + \yL_1 \yvN A]
= c_m, \quad m=1,2.
\end{aligned}
\end{multline}
...
%\binom{N}{N A}^{-1}
\binom{n}{n a}
\binom{\yvN-n}{\yvN A-n a}\,
\exp\Biggl[
\sum_{n a=0}^n \yL_{ a}
%\binom{N}{N A}^{-1}
\binom{n}{n a}
\binom{\yvN-n}{\yvN A-n a}\Biggr]
=q( a)
\end{aligned}
\end{multline}
...
$2^{\yvN}$ states of $\yvN$ units we are not excluding that these might be
marginals of $2^{M}$ states of $M$ units. Each one has the same
multiplicity $2^{M-\yvN}$, but this constant multiplicity factor disappears
by normalization. So the method applies just as in the case of $\yvN$ units
only, right?
...
population of two units, $M=2$, of which we consider one unit, $\yvN=1$.
\begin{itemize}
...
\begin{enumerate}
\item have a population with $\yvN$ units, $2^{\yvN}$ possible states
...
\item use maximum-entropy to choose a probability distribution for the
  states of the $\yvN$ units conforming to our expectations.
\end{enumerate}

We:
\begin{enumerate}
\item have a population with $M$ units, $2^{M}$ possible states
\item expect that any subpopulation of $\yvN$ units has $\yxs$ active
...
\end{enumerate}
  ...
  is $\yvN\yXf$, then $\yvN\yXf$ out of $\yvN$ units are active, and there are
  $\tbinom{\yvN}{\yvN\yXf}$ possible states for which this can be true; therefore
  \begin{equation}
    \labelbis{eq:joint_plaus_\yvN_homog}
    \p(\yX = \yR \| H) = \binom{\yvN}{\yvN\yRf}^{-1}Q(\yRf).
  \end{equation}
...
There are $\tbinom{\yvN}{\yvN\yXf}$ possible network states if the network
average is $\yXf$, \ie\ if $\yvN\yXf$ units are in state $1$; the conditional
probability of each is therefore $1/\tbinom{\yvN}{\yvN\yXf}$, owing to the
...
which $\yvN\yXf-n\yxs$ of the remaining $\yvN-n$ units are in state $1$; there are
$\tbinom{\yvN-n}{\yvN\yXf-n\yxs}$ such states, all equally probable. Finally,
...
\end{equation}
  ...
  blue balls in $n$ drawings without replacement from an urn with $\yvN$
  ...
  \begin{equation}
    ...
    \begin{split}
      \p(\yxs =a \| \yH)
      &= \sum_{\yvN A=0}^\yvN\p(\yxs = a \| \yXf = A, \yH)\,
      \p(\yXf = A \| \yH),
      \\
      &=\sum_{\yvN A=0}^\yvN
      \binom{n}{n a}\binom{\yvN-n}{\yvN A-n a}\binom{\yvN}{\yvN A}^{-1}
  ...
  \begin{multline}
    ...
    \sum_{\yx} \yvx_1\dotsm \yvx_m \p(\yx =\yr \| \yH)
      =\sum_{\yr,\yR}  \yvr_1\dotsm \yvr_m \p(\yx=\yr \| \yX=\yR,\yH)
      \, \p(\yX=\yR \| \yH),
      \\
      \begin{aligned}
      &=\sum_{\yr,\yR} \yvr_1\dotsm \yvr_m\,\delt(\yvR_1-\yvr_1)\dotsm\delt(\yvR_m-\yvR_m)
      \p(\yX = \yR \| \yH),
      \\
      &=\sum_{\yR} \yvR_1\dotsm \yvR_m \p(\yX =\yR \| \yH).
    \end{aligned}
  \end{multline}
  ...
  the subnetwork:
  \begin{equation*}
    \yvx_1 \yvx_2 + \yvx_1 \yvx_3 + \dotsb + \yvx_{n-1} \yvx_n.
  \end{equation*}
  ...
  \begin{equation}
    \sav{\yvx_{i_1} \dotsm \yvx_{i_m}} = \binom{n\yxs}{m} \binom{n}{m}^{-1}.
  \end{equation}
  Then, using \eqn~\eqref{noeq:subpop_average},
  \begin{multline}
    \label{noeq:expect_product_m}
    \expe*{\sav{\yvx_{i_1} \dotsm \yvx_{i_m}} \| \yH}
    \\
    \begin{aligned}[b]
      &=\tfrac{(n-m)!}{n!}
      ...
      \\
      &=\frac{(n-m)!}{n!} \sum_{\yvN A=0}^\yvN
      Q( A)
      \Biggl[ 
      \sum_{n a=0}^n 
      m!\,\binom{n a}{m}\, \binom{n}{n a}\binom{\yvN-n}{\yvN A-n a}\binom{\yvN}{\yvN A}^{-1} 
      \Biggr].
    \end{aligned}
  \end{multline}
...
%file: factorial_moments_etc.nb
  \begin{equation}
    \sum_{n a=0}^n m!\,\binom{n a}{m} \binom{n}{n a}\binom{\yvN-n}{\yvN\yXf-n a}
    \binom{\yvN}{\yvN\yXf}^{-1} 
    =
    m!\,\binom{n}{m} \binom{n a}{m}\binom{\yvN}{m},
  \end{equation}
  ...
  \begin{equation}
    ...
    \binom{\yvN}{M}\binom{M}{m}=\binom{\yvN}{m}\binom{\yvN-m}{M-m},
  \end{equation}
  ...
  \begin{equation}
    ...
    \quad
    \expe{\yXf^m \| \yH} \defd
    \sum_{\yvN A=0}^{\yvN} A^m \p(\yXf = A  \| H),
  \end{equation}
...
\begin{equation}
  ...
  \p(\yXf = \yRf \| \yHa)  = \yL\,
\binom{\yvN}{\yvN \yRf}\exp\biggl[
\yL_2 \yRf \,\frac{\yvN \yRf-1}{\yvN-1} + \yL_1\, \yRf\biggr]
\end{equation}
...
\begin{equation}
  ...
  \p(\yxs = \yrs \| \yHa)  = \yL
  \sum_{\yvN\yRf=0}^\yvN
  \binom{n}{n\yrs} \binom{\yvN-n}{\yvN\yRf-n\yrs} \exp\biggl[
\yL_2 \yRf \,\frac{\yvN \yRf-1}{\yvN-1} + \yL_1\, \yRf\biggr].
\end{equation}
...
Figure~\ref{fig:diff_maxent_pop_sample} shows one example, for $\yvN=5000$,
...
\fig~\ref{fig:diff_maxent_pop_sample_realdata}, for $\yvN=5000$, $n=200$,
