...
%% Last-Updated: 2018-05-16T11:37:42+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
\item Construct a sequence of relative-\me\ distributions for the activity
  ...
  \item \ldots
  \item the time averages of the activity, of the pair-activity, \etc\ up
    to the $k$-activity.
  \end{itemize}
...
bin averaged over the larger population, \etc.
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    \\
    &\text{\enquote{$\yxs = \yrs$} 
      stands for \enquote{the (population-averaged) activity of the sample neurons is $\yrv_i$}},
  \end{aligned}
\end{equation}
and similarly for other quantities and for the activities of the neurons of
the larger population.

\iffalse We have a population of $\yNv$ binary neurons. We assume that they can be
...
$(\yXv_1,\dotsc,\yXv_\yNv) \defs \yX$ having fixed but unknown binary values
$(\yRv_1,\dotsc,\yRv_\yNv)\defs \yR \in \set{0,1}^\yNv$; the \enquote{$\yXv$} denote the
variables, and \enquote{$\yRv$} their values. A particular sample of $n$ neurons
from this population has joint state $(\yxv_1, \dotsc, \yxv_n) \defs \yx$ having
fixed binary values $(\yrv_1, \dotsc, \yrv_n)\defs\yr \in \set{0,1}^n$. We will
...
\begin{equation}
  \begin{gathered}
    \av{\yX} \defd \tfrac{1}{\yNv} (\yXv_1 + \yXv_2 + \dotsb + \yXv_\yNv),
    % & \sav{\yx} &\defd \tfrac{1}{n} (x_1 + x_2 + \dotsb + x_n),
   \qquad% \\[2\jot]
    \av{\yX \yX} \defd \tbinom{\yNv}{2}^{-1} (\yXv_1 \yXv_2 + \yXv_1 \yXv_3 + \dotsb +
    \yXv_{\yNv-1} \yXv_\yNv),
    % & \sav{\yx \yx} &\defd\tbinom{n}{2}^{-1} (x_1 x_2 + x_1 x_3 + \dotsb
    % + x_{n-1} x_n),
    \\[2\jot]
    \av{\yX\yX\yX} \defd \tbinom{\yNv}{3}^{-1} (\yXv_1 \yXv_2 \yXv_3 +% \yXv_1 \yXv_2 \yXv_4
    \dotsb + \yXv_{\yNv-2} \yXv_{\yNv-1}\yXv_\yNv),
    \qquad\text{and so on.}
...
the fraction of simultaneously active pairs out of all $\binom{\yNv}{2}$
pairs, $\av{\yX\yX\yX}$ the fraction of simultaneously active triplets, and
so on. Products of states like $\yXv_i \dotsm \yXv_j$ also have values in
$\set{0,1}$; from this we can combinatorially  prove that
\begin{equation}
  \label{eq:products_intermsof_average}
  \av{\underbrace{\yX\dotsm\yX}_{\text{$m$ factors}}}
  = \binom{\yNv}{m}^{-1}\binom{\yNv\yXf}{m}.
\end{equation}
...
\begin{simplenote}
  ...
  two samples are different: the former is obtained from marginalization,
  the latter by symmetrization and marginalization.
\end{simplenote}
\fi

If $\yHc$ denotes our state of knowledge, \ie\ the evidence and assumptions
...
assign equal probabilities to all states with $\yNv\yRf$ active neurons.
...
\begin{gather}
  ...
  \p(\yxs=\yrs \| \yH) = \sum_{\yNv \yRf =0}^{\yNv}
...
spaces of the sample average $\yXf\in\set{0,\dotsc,\yNv}$ and of the
...
\begin{equation}
  \expeb{f^*(\yXf)}
  \equiv
  \sum_{\yNv\yRf=0}^\yNv f^*(\yRf)\,\p(\yXf=\yRf \| \yH)
%  =
...
\fig~\ref{fig:hypergeom_proj}, reveals that it is a sort of \enquote{fuzzy
  identity matrix} between the $\yXf$-space $\set{0,\dotsc,\yNv}$ and
$\yxs$-space $\set{0,\dotsc,n}$. When $n=\yNv$ it is the identity matrix. We
thus have that
\begin{equation}
...
%   \qquad\text{or}\qquad
%   \sum_{n\yrs=0}^n f(\yrs)\,p_*(\yxs=\yrs) = \sum_{\yNv\yRf=0}^\yNv f^*(\yRf)\,p(\yXf=\yRf).
% \end{equation}
...
constrains $\yNv+1$ unknowns by only $n+1$ equations.
...
\begin{equation}
  ...
  \p(\yxs = \yrs \| \yHa)  = \yK\, 
  \sum_{\yNv\yRf=0}^\yNv \ypp(\yrs \|\yRf)
\,
  \binom{\yNv}{\yNv \yRf}\,\exp\Bigl[\yL\T
  \sum_{n\yrs=0}^n \yf(\yrs)\ypp(\yrs\|\yRf)\Bigr],
\end{equation}
...
\yc = \yK\sum_{n\yrs=0}^n \sum_{\yNv\yRf=0}^\yNv \yf(\yrs) \ypp(\yrs \|\yRf)
\,
  \binom{\yNv}{\yNv \yRf}\,\exp\Bigl[\yL\T
  \sum_{n\yrs=0}^n \yf(\yrs)\ypp(\yrs\|\yRf)\Bigr].
\end{equation}
...
and~\ref{fig:diff_maxent_pop_sample_realdata}, for $\yNv=5000$, $n=200$, and
...
population: we do not know whether $\yNv=n$ or how large $\yNv$ is otherwise.
Call this state of ignorance $\yHd$. In the probability calculus this
ignorance about $\yNv$ is expressed by assigning a probability distribution
$\p(\yNv \| \yHd)$ that vanishes if $\yNv<n$, since we know that $\yNv\ge n$; see
...
assume a specific value of $\yNv$ are then obtained via multiplication of all
$\yNv$-dependent probabilities by $\p(\yNv \| \yHd)$ and subsequent
marginalization over $\yNv$. Technically speaking, $\yNv$ becomes a
...
\begin{equation}
  ...
  \p(\yxs = \yrs \| \yHd)  = \sum_\yNv \biggl\{\yK_\yNv\, 
  \sum_{\yNv\yRf=0}^\yNv \ypp_\yNv(\yrs \|\yRf)
\,
  \binom{\yNv}{\yNv \yRf}\,\exp\Bigl[{\yL_\yNv}\T
  \sum_{n\yrs=0}^n \yf(\yrs)\ypp_\yNv(\yrs\|\yRf)\Bigr]\biggr\}
  \,\p(\yNv \| \yHd),
\end{equation}
where $\yNv$-dependencies have been made explicit. This is a formidable
...
\eqref{eq:app_maxent_sample} equal for some choice of $\p(\yNv \| \yHd)$, with
$\p(\yNv \| \yHd)\ne 0$ for $\yNv>n$?}. We leave this mathematical problem for
future work. Note, however, that this equality is satisfied if
$\p(\yNv=1 \| \yHd)=1$, which means that the usual \me\ model can also be
...
the standard \me\ model. The question remains of how to specify $\yNv$,
though. We have tacitly intended $\yNv$ as the size of the largest
...
for example the primary visual cortex, for which $\yNv\sim 10^8$
\cite{leubaetal1994}. For large $\yNv$ -- unfortunately we are not yet able to
translate this \enquote{large} into a numeric order of magnitude -- the
final distribution becomes independent of $\yNv$, and continuous
...
\caption{Example of discrepancy in $\p(\yxs = \yrs \| \yH)$ constructed by
  ...
  $\expe{\yxxs \|\yH}=0.0025$, and $\yNv=5000$, $n=200$.}
\label{fig:diff_maxent_pop_sample_realdata}
\end{figure}%maxent_pop_or_sample.nb
...
for example $\expe{\yxv_2}$ or $\expe{\yxv_1x_8x_9}$. A short computation shows
...
\begin{align}
  \expe{\yxv_{i_1} \dotsm \yxv_{i_m} \| \yH}
&=
\expe{\sav{\underbrace{\yx \dotsm \yx}_{\text{$m$ factors}}} \| \yH}
=
\expe{\av{\underbrace{\yX \dotsm \yX}_{\text{$m$ factors}}} \| \yH},
...
make statements about the whole population of $\yNv$ units and about a
...
are denoted by lowercase letters: $(\yxv_1, \dotsc, \yxv_n)\equiv\yx$ and
$(\yrv_1, \dotsc, \yrv_n)\equiv\yr$; but note that $\yxv_i \equiv \yXv_{j_i}$ and
$\yrv_i \equiv \yRv_{j_i}$ for some distinct $j_1,\dotsc,j_n$. We shall also make
statements about the population-averaged state, or \emph{population
  average}:
\begin{align}
  \label{noeq:population-av}
   \yXf &\defd (\yXv_1 + \dotsb + \yXv_\yNv)/\yNv,
\\
\intertext{and the subpopulation-averaged state, or \emph{subpopulation average}:}
\label{noeq:subpopulation-av}
\yxs &\defd (\yxv_1 + \dotsc + \yxv_n)/n.
\end{align}
The quantities $\yNv\yXf$ and $n\yxs$ represent the total number of active
...
$\tbinom{\yNv}{m}$ products of $m$ states; \eg,
\begin{align}
\av{\yX \yX} &\defd
\tbinom{\yNv}{2}^{-1} (\yXv_1 \yXv_2 + \yXv_1 \yXv_3  + \dotsb +  \yXv_{\yNv-1} \yXv_\yNv),
\\[2\jot]
\sav{\yx\yx\yx} &\defd
\tbinom{n}{3}^{-1} (\yxv_1 \yxv_2 \yxv_3 + \yxv_1 \yxv_2 \yxv_4 + \dotsb + \yxv_{n-2} \yxv_{n-1} \yxv_n),
\end{align}
and so on.
...
\begin{equation}
  ...
  \p(\yXv_1=\yRv_1, \yXv_2=\yRv_2, \dotsc, \yXv_\yNv=\yRv_\yNv \| \yH) \quad\text{or}\quad
\p(\yX =\yR \| \yH).
\end{equation}
...
\begin{multline}
  ...
  \p(\yXv_1=\yRv_1, \yXv_2=\yRv_2, \dotsc, \yXv_\yNv=\yRv_\yNv \| \yH) ={}
\\ 
\shoveright{\p(\yXv_1=\yRv_{\pi(1)}, \yXv_2=\yRv_{\pi(2)}, \dotsc, \yXv_\yNv=\yRv_{\pi(\yNv)} \| \yH)}
\\
...
=  Q(A),
\qquad
A \in \set*{0,\tfrac{1}{\yNv},\tfrac{2}{\yNv},\dotsc,1}.
\end{equation}
...
\begin{equation}
  ...
  \p(\yX = \yR \| H) = \binom{\yNv}{\yNv\yRf}^{-1} Q(\yRf).
\end{equation}
...
\begin{equation}
  \label{noeq:marginal}
    \p(\yx = \yr \| \yH) =
\sum_{\yNv A =0}^{\yNv} 
\binom{\yNv-n}{\yNv A-n\yrs}\binom{\yNv}{\yNv A}^{-1}\,
Q(A).
\end{equation}
Note that the only summands contributing to this sum are those for which
$n\yrs \le NA \le \yNv$; the others are zero because by definition
...
\begin{multline}
 ...
 \sum_{\yNv A=0}^{\yNv}
\binom{\yNv-n}{\yNv A-n a}\binom{\yNv}{\yNv A}^{-1}\,
Q(A),
\\
...
\begin{equation}
  ...
  \p(\yxs = a \| \yXf = A, \yH)
=
\binom{n}{n a}\binom{\yNv-n}{\yNv A-n a}\binom{\yNv}{\yNv A}^{-1}.
\end{equation}
...
\item\label{noitem:moments}The product of the states of any $m$ distinct
  ...
  \begin{equation*}
    \yxv_{i_1} \yxv_{i_2} \dotsm \yxv_{i_m},
...
=
  \tfrac{(\yNv-m)!}{\yNv!}\expeb*{\frac{(\yNv\yXf)!}{(\yNv\yXf-m)!} \| \yH}.
\end{equation}
...
\begin{subequations}\label{noeq:moments}
  \begin{align}
    ...
    \expe{\yxs^2 \| \yH} &= \expe{\yXf \| \yH}
                                  \frac{\yNv-n}{(\yNv-1)n}
 +{}
\\&\quad\quad \expe{\yXf^2 \| \yH}\frac{\yNv\,(n-1)}{(\yNv-1)n},
\end{split}
    \\[2\jot]
    \begin{split}
    \expe{\yxs^3 \| \yH} &=
                              \expe{\yXf \| \yH}\frac{(\yNv-n)(\yNv-2n)}{(\yNv-1)(\yNv-2)n^2}+{}
                                    \\
                                    &\quad\quad\expe{\yXf^2 \| \yH}
                                    \frac{3N\,(\yNv-n)(n-1)}{(\yNv-1)(\yNv-2)n^2} +{}
                                    \\
                                    &\quad\quad\quad\expe{\yXf^3 \| \yH}
                                    \frac{\yNv^2\,(n-1)(n-2)}{(\yNv-1)(\yNv-2)n^2}.
                                  \end{split}
                                    % \\[2\jot]
                                    % \expe{\yxs^4 \| \varEta} &=
...
\begin{equation}
  ...
  \expe{\yxs^m \| \yH} = \sum_{l=1}^m M_{ml}(n,\yNv) \expe{\yXf^l \| \yH},
\end{equation}
with a universal, lower-triangular transformation matrix $M_{ml}(n,\yNv)$ that
depends only on $n$, $\yNv$, and the condition of symmetry~\eqref{noeq:homog_math}.

As intuition suggests, we have 
%$\expe{\yxs^m \| \yH} \xrightarrow{n\to \yNv} \expe{\yXf^m \| \yH}$.
\begin{equation}
  \label{noeq:limit_moments}
  \expe{\yxs^m \| \yH} \xrightarrow{n\to \yNv} 
  \expe{\yXf^m \| \yH},
\qquad
  \expe{\yxs^m \| \yH} \xrightarrow{n\to 1} 
  \expe{\yXf \| \yH},
\end{equation}
the latter because ${\yxv_i}^m=\yxv_i$, since states are $\set{0,1}$-valued.
...
\cites[\chap~3]{jaynes1994_r2003}[\sect~4.8.3]{ross1976_r2010}[\sect~II.6]{feller1950_r1968},
with parameters $\yNv,\yNv\yXf,n$, weighted by the probabilities
...
\binom{\yNv}{\yNv A}\exp[
\yl_2\, \yNv A\,(\yNv A-1)/2 + \yl_1\, \yNv A].
\end{equation}
...
$Q_0$ defined by $Q_0(A) = 2^{-\yNv}\binom{\yNv}{\yNv A}$, corresponding to a
...
which $\yNv = 1000$ units, $\lambda_1=-2.55$, $\lambda_2=0.005$, and
...
$\expe{\yxv_{i_1}\dotsm \yxv_{i_m} \| \yH}$, proportional to the factorial
...
$\RR^{\yNv+1}$ to $\RR^{n+1}$, with $\yNv$ larger than $n$, is onto but not into.
...
\item the expectations of the single and pair averages $\yxs$ and
  ...
  \begin{equation}
    ...
    \expe{\yxs \| \yHa} = c_1, \qquad \expe{\sav{\yxv_i \yxv_j}\|\yHa} = c_2;
  \end{equation}
...
\begin{multline}
  ...
  \pf(\yX =\yR \|\yHa) =
\varKappa\exp[\yL_2 \yNv\yRf\,(\yNv\yRf-1)/2 + \yL_1 \yNv\yRf]
\\
\!\begin{aligned}[b]
&\text{with $\varKappa, \yL_m$, such that the distribution is normalized and}
\\
&\varKappa\sum_{\yNv A=0}^\yNv %m!\, 
\binom{\yNv-m}{\yNv A-m}
\exp[\yL_2 \yNv A\,(\yNv A-1)/2 + \yL_1 \yNv A]
= c_m, \quad m=1,2.
\end{aligned}
\end{multline}
...
%\binom{N}{N A}^{-1}
\binom{n}{n a}
\binom{\yNv-n}{\yNv A-n a}\,
\exp\Biggl[
\sum_{n a=0}^n \yL_{ a}
%\binom{N}{N A}^{-1}
\binom{n}{n a}
\binom{\yNv-n}{\yNv A-n a}\Biggr]
=q( a)
\end{aligned}
\end{multline}
...
$2^{\yNv}$ states of $\yNv$ units we are not excluding that these might be
marginals of $2^{M}$ states of $M$ units. Each one has the same
multiplicity $2^{M-\yNv}$, but this constant multiplicity factor disappears
by normalization. So the method applies just as in the case of $\yNv$ units
only, right?
...
population of two units, $M=2$, of which we consider one unit, $\yNv=1$.
\begin{itemize}
...
\begin{enumerate}
\item have a population with $\yNv$ units, $2^{\yNv}$ possible states
...
\item use maximum-entropy to choose a probability distribution for the
  states of the $\yNv$ units conforming to our expectations.
\end{enumerate}

We:
\begin{enumerate}
\item have a population with $M$ units, $2^{M}$ possible states
\item expect that any subpopulation of $\yNv$ units has $\yxs$ active
...
\end{enumerate}
  ...
  is $\yNv\yXf$, then $\yNv\yXf$ out of $\yNv$ units are active, and there are
  $\tbinom{\yNv}{\yNv\yXf}$ possible states for which this can be true; therefore
  \begin{equation}
    \labelbis{eq:joint_plaus_\yNv_homog}
    \p(\yX = \yR \| H) = \binom{\yNv}{\yNv\yRf}^{-1}Q(\yRf).
  \end{equation}
...
There are $\tbinom{\yNv}{\yNv\yXf}$ possible network states if the network
average is $\yXf$, \ie\ if $\yNv\yXf$ units are in state $1$; the conditional
probability of each is therefore $1/\tbinom{\yNv}{\yNv\yXf}$, owing to the
...
which $\yNv\yXf-n\yxs$ of the remaining $\yNv-n$ units are in state $1$; there are
$\tbinom{\yNv-n}{\yNv\yXf-n\yxs}$ such states, all equally probable. Finally,
...
\end{equation}
  ...
  blue balls in $n$ drawings without replacement from an urn with $\yNv$
  ...
  \begin{equation}
    ...
    \begin{split}
      \p(\yxs =a \| \yH)
      &= \sum_{\yNv A=0}^\yNv\p(\yxs = a \| \yXf = A, \yH)\,
      \p(\yXf = A \| \yH),
      \\
      &=\sum_{\yNv A=0}^\yNv
      \binom{n}{n a}\binom{\yNv-n}{\yNv A-n a}\binom{\yNv}{\yNv A}^{-1}
  ...
  \begin{multline}
    ...
    \sum_{\yx} \yxv_1\dotsm \yxv_m \p(\yx =\yr \| \yH)
      =\sum_{\yr,\yR}  \yrv_1\dotsm \yrv_m \p(\yx=\yr \| \yX=\yR,\yH)
      \, \p(\yX=\yR \| \yH),
      \\
      \begin{aligned}
      &=\sum_{\yr,\yR} \yrv_1\dotsm \yrv_m\,\delt(\yRv_1-\yrv_1)\dotsm\delt(\yRv_m-\yRv_m)
      \p(\yX = \yR \| \yH),
      \\
      &=\sum_{\yR} \yRv_1\dotsm \yRv_m \p(\yX =\yR \| \yH).
    \end{aligned}
  \end{multline}
  ...
  the subnetwork:
  \begin{equation*}
    \yxv_1 \yxv_2 + \yxv_1 \yxv_3 + \dotsb + \yxv_{n-1} \yxv_n.
  \end{equation*}
  ...
  \begin{equation}
    \sav{\yxv_{i_1} \dotsm \yxv_{i_m}} = \binom{n\yxs}{m} \binom{n}{m}^{-1}.
  \end{equation}
  Then, using \eqn~\eqref{noeq:subpop_average},
  \begin{multline}
    \label{noeq:expect_product_m}
    \expe*{\sav{\yxv_{i_1} \dotsm \yxv_{i_m}} \| \yH}
    \\
    \begin{aligned}[b]
      &=\tfrac{(n-m)!}{n!}
      ...
      \\
      &=\frac{(n-m)!}{n!} \sum_{\yNv A=0}^\yNv
      Q( A)
      \Biggl[ 
      \sum_{n a=0}^n 
      m!\,\binom{n a}{m}\, \binom{n}{n a}\binom{\yNv-n}{\yNv A-n a}\binom{\yNv}{\yNv A}^{-1} 
      \Biggr].
    \end{aligned}
  \end{multline}
...
%file: factorial_moments_etc.nb
  \begin{equation}
    \sum_{n a=0}^n m!\,\binom{n a}{m} \binom{n}{n a}\binom{\yNv-n}{\yNv\yXf-n a}
    \binom{\yNv}{\yNv\yXf}^{-1} 
    =
    m!\,\binom{n}{m} \binom{n a}{m}\binom{\yNv}{m},
  \end{equation}
  ...
  \begin{equation}
    ...
    \binom{\yNv}{M}\binom{M}{m}=\binom{\yNv}{m}\binom{\yNv-m}{M-m},
  \end{equation}
  ...
  \begin{equation}
    ...
    \quad
    \expe{\yXf^m \| \yH} \defd
    \sum_{\yNv A=0}^{\yNv} A^m \p(\yXf = A  \| H),
  \end{equation}
...
\begin{equation}
  ...
  \p(\yXf = \yRf \| \yHa)  = \yL\,
\binom{\yNv}{\yNv \yRf}\exp\biggl[
\yL_2 \yRf \,\frac{\yNv \yRf-1}{\yNv-1} + \yL_1\, \yRf\biggr]
\end{equation}
...
\begin{equation}
  ...
  \p(\yxs = \yrs \| \yHa)  = \yL
  \sum_{\yNv\yRf=0}^\yNv
  \binom{n}{n\yrs} \binom{\yNv-n}{\yNv\yRf-n\yrs} \exp\biggl[
\yL_2 \yRf \,\frac{\yNv \yRf-1}{\yNv-1} + \yL_1\, \yRf\biggr].
\end{equation}
...
Figure~\ref{fig:diff_maxent_pop_sample} shows one example, for $\yNv=5000$,
...
\fig~\ref{fig:diff_maxent_pop_sample_realdata}, for $\yNv=5000$, $n=200$,
