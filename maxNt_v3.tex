...
%% Last-Updated: 2018-05-17T18:47:41+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
expression shows that the expectation of the function $g$, whose argument
is the sample activity $\yrs$, is equal to the expectation of the function
$g^*$, whose argument is the full-population activity $\yRf$, defined as
...
in $n$ ways (one active neuron out of $n$), \etc\ This term is analogous to
the \enquote{density of states} in front of a Boltzmann factor in
...
\caption{Linear and log-plots of $\p(\yxs = \yrs)$ for a sample of $n=200$
  and constraints as in \eqn~\eqref{eq:constraints}, constructed by: (red
  circles) \me\ at the sample level, \eqn~\eqref{eq:app_maxent_sample};
  (blue triangles) \me\ at the population level,
  \eqn~\eqref{eq:app_maxent_pop_marg} with $\yNv=10\,000$, followed by
  sample marginalization; (yellow squares) \me\ at the population level
  with unknown population size,
  \eqn~\eqref{eq:app_maxent_pop_marg_unknown_N}, according to the
  distribution~\eqref{eq:pdf_popsize}.}
\label{fig:all_three}
...
\item Do the standard and new applications lead to different or contrasting
  ...
  sets?
\item How to extend the new application when we want to study the 
  activities of individual neurons, of individual pairs, triplets, and so
  on?
...
\iffalse Our analysis touched only constraints of the sample average,
$\expeb{g(\yxs)}$. The corresponding models are usually called
