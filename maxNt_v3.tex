...
%% Last-Updated: 2018-05-15T13:51:45+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Report-no: ***
...
\item The activity of each neuron, ideally a continuous signal, is
  transformed into a sequence of \enquote{$0$}s (inactive) and
...
Yet, it is quantitative and more precise than an undefined notion of
...
activities of individual neurons, or even protocols that take into account
time dependence.

The purpose of the present work is to show that there is a problem in the
way the \me\ method as sketched above, or more complex variations of it, is
applied to the activity of the recorded neurons. The problem stems from the
fact that the recorded neurons are a sample from a larger population.

Suppose that the neurons were recorded with electrodes covering an area of
some square millimetres. This recording is a sample of the activity of the
population covered by the electrode; we can consider it as a sample of an
even more extended brain area. The characterization of the cooperativity of
the recorded sample would have little meaning if we did not expect its
results to generalize to a larger area, at least that covered by the
electrodes. In other words, we expect that the conclusions drawn with the
\me\ methods about the sampled neurons should somehow generalize or
extrapolate at least to larger areas from which those neurons were sampled.
We are thus assuming that the recorded sample is a \emph{representative
  sample} of some larger area. Probability theory tells us how to make
inferences from a sample to the larger population from which it is sampled
\parentext{see references below}.
...
$(R_1,\dotsc,R_N)\defs \yR \in \set{0,1}^N$; the \enquote{$X$} denote the
variables, and \enquote{$R$} their values. A particular sample of $n$ neurons
from this population has joint state $(x_1, \dotsc, x_n) \defs \yx$ having
fixed binary values $(r_1, \dotsc, r_n)\defs\yr \in \set{0,1}^n$. We will
...
spaces of the sample average $\yXf\in\set{0,\dotsc,N}$ and of the
