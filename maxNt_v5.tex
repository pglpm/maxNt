...
%% Last-Updated: 2019-06-19T15:25:54+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
interesting inference is that the most probable frequency distribution for
...
for short. Obviously $\ya_{t} \in\set{0,1/n,\dotsc,(n-1)/n,1}$; if
$\ya_{t}=0$, no neuron spikes during bin $t$; if $\ya_{t}=1$, all spike at
some point during bin $t$. For brevity, let's say \enquote{at $t$} for
\enquote{during time bin $t$}. From the activities $\set{\ya_{t}}$ we can
count how many times the activity levels $\ya=0$, $\ya=1/n$, and so on
appeared during the recording, obtaining the measured relative-frequency
distribution $(\yff_{\ya}) \defs \yf$. We can also consider the sample
activity at time bins \emph{outside} of the recorded range. Such activity
is unknown to us, of course.
...
recorded within the region are not -- and at present cannot be --
...
experimental sessions. We assume that there's a region, comprising a
...
inferences about the full-network activity. First of all, this seems to be
...
We first calculate the maximum-entropy distribution for the full network by
using six moments. This number already provides as much information as the
full frequency distribution of the sample, and at the same time illustrates
the use of the approach in questions of statistic sufficiency (typically
limited to two or three moments). Figure~\ref{fig:plots_6mom} shows the
resulting densities (distribution/activity-step, that is,
distribution${}\times N$) for full-network sizes $N=n$, $N=1\,000$,
$N=5\,000$, $N=10\,000$\mynote{motivate?}. The case $N=n$ corresponds to
...
\begin{figure}[!p]
\centering
\includegraphics[width=\linewidth]{scripts/paper_stensola_6mom_distributionsN_prior2.pdf}%
\\[3em]%
\includegraphics[width=\linewidth]{scripts/paper_riehle_6mom_distributionsN_prior2.pdf}%
\caption{***}
...
shoulder on the right of the mode, suggestive of two activity regimes.
...
probability calculus to make forecasts about the full network thus reveals
interesting and useful features.

\bigskip

As an illustration of our approach in questions of sufficient statistics,
\fig~\ref{fig:plots_2_4mom} shows the results for two constrained moments
(equivalent to constraining means and correlations only) and for four
constrained moments, using $N=10\,000$. In either data set we obtain very
distinct distributions for the two kinds of constraints. In particular, for
the first data set with four moments we obtain a bimodal distribution, and
with two moments a unimodal one, showing that two moments would be clearly
insufficient. The inset in the plot for the first data set shows the
distributions obtained applying maximum-entropy at the sample level: their
differences aren't so glaring as in the full-network application.


\begin{figure}[!p]
\centering
\includegraphics[width=\linewidth]{scripts/paper2_stensola_twomom_distributions10000_prior2.pdf}%
\\[3em]%
\includegraphics[width=\linewidth]{scripts/paper_riehle_twomom_distributions10000_prior2.pdf}%
\caption{***}
