...
%% Last-Updated: 2019-06-18T15:05:04+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
frequency distribution in time during the recording? How much total
activity should we expect at a time bin during the recording? And at a new
time bin? We cannot answer these questions with certainty; we can only give
distributions of probability over their possible answers. Our approach
gives such probability distributions.
...
this method that are seldom made in the literature. First, the
\enquote{distribution} given by this method, which we denote by $\px(\yA)$,
is the zeroth-order approximation of four different distributions for the
full network:
\begin{itemize}
...
completely different inferences. Note that although the four distributions
above numerically coincide in this approximation, they differ in
higher-order approximations. We discuss their differences in \sect***.
Second, the maximum-entropy method based on the Shannon entropy implicitly
makes specific assumptions about the probabilities for the long-run
...
factorial moment of a distribution $\px(\ya)$ for the activity of the
...
=    \sum_{\yA} %m!\,
  ...
  \qquad 1\le m \le n,
\end{equation}
where $\px(\yA)$ is the distribution for the full-network activity.
...
$1\le m \le n$. The constraints on the distribution $\px(\yA)$ for
...
\begin{equation}
  ...
  =
   \sum_{\yA}
  \binom{N \yA}{m}\,  \binom{N}{m}^{-1}\, \pf(\yA)
\end{equation}
for any $m$s we wish.
...
important: it corresponds to fully specify the measured frequency
...
\emph{at the sample level} is in this case trivial and meaningless -- it
just gives back the measured frequency distribution. But application of the
method \emph{at the level of the full network}, as done here, is not
trivial.

Using the full frequency distribution of the sample may be a bad idea,
however, because the maximum-entropy distribution becomes a bad
...
the literature:
\begin{itemize}[wide]
\item The first data set, from \textcite[rat 14147]{stensolaetal2012},
