...
%% Last-Updated: 2019-07-25T16:30:40+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
into account individually, and therefore become very complex\mynote{refs}.
It would be useful to have models that operate in between: addressing the
larger brain area whence the sample comes, but without adding too much
complexity and detail about it. Such intermediate models would be useful,
for example, for preliminary investigations, to help us to decide which
hypotheses to discard and which to consider for more complex and costly
studies.

In the present work we propose such an intermediate model. It answers this
question: How much can the \emph{total} activity of a large neuronal
...
The proposed model is based on the straightforward combination of the
...
population size, $\ya/n \in \set{0,1/n,\dotsc,1}$ we have the normalized
total activity or population-averaged activity. From the activities
$\set{\ya_{t}}$ we can count how often the activity levels $\ya=0$,
$\ya=1$, and so on appeared during the recording, obtaining the
distribution of measured relative frequencies $(\yff_{\ya}) \defs \yf$. We
can also consider the sample activity at time bins \emph{outside} of the
recorded range. Such activity is unknown to us, of course.
...
This is our \enquote{full network}.\mynote{how about calling it
  \enquote{the pool}?} The total activity of these $N$ neurons at $t$ is
$\yA_{t}$. The relative frequencies of the various activity levels during
the recording were $(\yFF_{\yA}) \defs \yF$. We don't know the values
$\yA_{t}$ at each $t$, or the frequency distribution $\yF$. We only know
for certain that $\yA_{t} \in \set{0, 1, \dotsc, N}$ and that
$\yA_{t} \ge \ya_{t}$ for obvious reasons. For the time being we assume
that we know $N$; in \sect*** we discuss the consequences of our lack of
precise knowledge about this number.
