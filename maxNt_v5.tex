...
%% Last-Updated: 2019-06-12T16:15:50+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
with the maximum-entropy method. We add two remarks.
First, the distribution given by the method is the zeroth-order
approximation of \emph{four} distinct distributions:
\begin{itemize}
\item the probability distribution for the value of the activity at any
  time bin among those recorded,
\item the probability distribution for the value of the activity at a \emph{new}
  time bin,
\item the most probable time-frequency distribution of the activity during the
  recording,
\item the most probable time-frequency distribution of the activity in a very
  long run of new time bins.
\end{itemize}
So the generic term \enquote{distribution} means one of these four; note
that although these distributions numerically coincide in this basic
approximation, they numerically differ in higher-order approximations.
...
distributions \citep{jaynes1986d_r1996,portamana2009,portamana2017}.

In our case, to apply the method to the total activity of the full
population we need to set some of its expectation values, for some of its
moments for example. Here enters point~\ref{item:sample_step}:
