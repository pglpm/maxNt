...
%% Last-Updated: 2019-06-14T15:23:13+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
Let's introduce the context and notation for our problem.

We have recorded the activities of $n$ neurons for a given amount of time.
These activities are binned into $T$ time bins and binarized to $\set{0,1}$
values. Denote by $\ya_{t}$ the number of active neurons during time bin
$t$, divided by the total number of neurons $n$; we call this the
...
all $n$ neurons were silent during time bin $t$; if $\ya_{t}=1$, all were
active.

\mynote{maybe move this paragraph to the intro?}Present-day technologies
allow the recording of neuronal activity from small brain regions
\mynote{be more specific}. The specific neurons that happen to be recorded
from a region are not and cannot yet be specifically chosen, however, owing
to several factors -- for example, limitations in the precision with which
electrodes are inserted, or in the targeting of neurons by viruses, and so
on. In fact, the set of recorded neurons may vary during very long
recordings or across experimental trials. We assume that there's a small
region, comprising a population of $N$ neurons, for which other sets of
neurons had equal chances of being \enquote{selected} as the sample of $n$
neurons that have actually been recorded. During the recording, the
normalized total activity of these $N$ neurons at time bin $t$ was
$\yA_{t}$; note that $\yA_{t} \in \set{0,1/N,\dotsc,(N-1)/N,1}$ and that
$N\yA_{t} \ge n\ya_{t}$ for obvious reasons.
