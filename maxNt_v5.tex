...
%% Last-Updated: 2019-06-17T10:56:29+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
neurons the \enquote{sample}. Their spikes are binned into $T$ time bins and
binarized to $\set{0,1}$ values in each bin. Denote by $\ya_{t}$ the number
of neurons that fire during time bin $t$, divided by the total number of
neurons $n$; we call this the \emph{normalized total activity} of the
sample, or just its \enquote{total activity} or \enquote{activity} for
...
network we need to fix some of the latter's expectation values, for
example its moments. But we don't have any measured averages for the full
network to equate our expectations to. Here enters
point~\ref{item:sample_step}: the probability calculus gives an exact,
linear relation between the first $m$ moments for the full network and
...
sample from the full network of $N$ neurons, we have this extremely
convenient property: \emph{The first $n$ normalized factorial moments for
  the sample and for the full network are numerically identical}:
\begin{equation}
