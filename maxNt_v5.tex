...
%% Last-Updated: 2019-06-16T21:39:12+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\emph{normalized total activity} or just \enquote{total activity} or
\enquote{activity} for short. Obviously
...
there's a region, comprising a population of $N$ neurons, from which other
sets of neurons had the same probability of being recorded as the set of
$n$ neurons that have actually been recorded. We call this the
\enquote{full population}. During the recording, the normalized total
activity of these $N$ neurons at $t$ was $\yA_{t}$. We don't know the
values $\yA_{t}$ at each $t$; we only know for certain that
$\yA_{t} \in \set{0,1/N,\dotsc,(N-1)/N,1}$ and that $N\yA_{t} \ge n\ya_{t}$
for obvious reasons. For the moment we assume that we know $N$; in \sect***
we discuss the consequences of our lack of precise knowledge about this
number.

Our questions concern general features of the total activity $\yA$ of the
full population during and after the recording. For example: what was its
...
\begin{enumerate}[label=(\alph*)]
  ...
  maximum-entropy method for the full population are, in turn, determined
  via sampling theory from the expectation constraints for the sample.
\end{enumerate}
Let's discuss these points in detail.

Regarding~\ref{item:maxent_step}, we assume you to be familiar with the
...
satisfying specific convex constraints, such as fixed expectation values; and
judge these expectation values to be equal to measured averages. We add two
remarks about this method that are seldom made in the literature. First,
the \enquote{distribution} given by this method is the zeroth-order
...
in higher-order approximations. Second, the maximum-entropy method based on
the Shannon entropy implicitly makes specific assumptions about the
probability of the long-run frequency distributions
\citep{jaynes1986d_r1996,portamana2009,portamana2017}. We discuss these
assumptions in \sect***

In our case, to apply the method for the total activity of the full
population we need to fix some of the latter's expectation values, for
example its moments. But we don't have any measured averages for the full
population to equate our expectations to. Here enters
point~\ref{item:sample_step}: the probability calculus gives an exact,
linear relation between the first $m$ moments for the full population and
the first $m$ for the sample; the ones determine the others and vice versa
at every time bin \citep[for example][\eqns~(16)]{portamanaetal2015}. This
relation is a classical result of sampling theory
...
$m=2$, if $na=4$ neurons spike in a population of $n=5$, we have
...
linearly determine the first $m$ moments and vice versa, so fixing the ones
is equivalent to fixing the others. Now, since the recorded neurons are a
sample from the full population of $N$ neurons, we have this extremely
convenient property: \emph{The first $n$ normalized factorial moments for
  the sample and for the full population are numerically identical}:
\begin{equation}
...
for the full population, by constraining its $m$th factorial moment to be
...
In the next section we apply this method to the data sets from two actual
recordings, and discuss the properties of the resulting distributions.
