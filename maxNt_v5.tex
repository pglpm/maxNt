...
%% Last-Updated: 2019-07-08T11:42:10+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
pairwise correlations are sufficient statistics for a particular sample of
a network, then they \emph{cannot} be sufficient for the full networ, or
for a different sample; and vice versa, if we assume that they are
sufficient statistics for the full network, then we must expect them
\emph{not} to be sufficient for any recorded sample thereof. But questions
about sufficient statistics are implicitly meant to apply to a whole brain
area, not just to some casually recorded sample. Thus, maximum-entropy
methods applied at the sample level paradoxically give an answer opposite
to the one we might think they're giving!

The comparisons of different sufficient statistics are only approximate,
however: we ask only whether our uncertainties conditional on either are
\emph{appreciably} different in some specified sense. It's therefore worth
checking whether the different results we get from a sufficient statistics
for the sample and for the full-network are still very similar when we
consider an altogether different statistics.
\begin{figure}[!p]
\centering
