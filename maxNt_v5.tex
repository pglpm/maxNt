...
%% Last-Updated: 2019-06-16T12:46:55+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Maximum-entropy distributions for a 
  neuronal population\\ from subpopulation data%\\{\large ***}%
}
...
interesting inference is that the most probable time-frequency distribution
for the total activity of the full population, if the latter is assumed to
be larger than 1\,000 neurons, has two very distinct modes, both at low
activities ($0.7\%$ and $2.6\%$ of simultaneously active neurons within
$3\;\mathrm{ms}$, see \fig***). The analogous distribution for second data
set doesn't have two modes but still presents one prominent shoulder in its
one low-activity mode. Note that these inferred features for the full
population aren't observed in the sample, nor are they inferred by the
application of maximum-entropy to the sample alone. \mynote{if we manage
  the Monte Carlo sampling or other approx: say something about the
  deviation from the most probable frequency distribution}
...
The maximum-entropy method \citep{jaynes1957}[much clearer
in][]{jaynes1963}{sivia1996_r2006,hobsonetal1973,jaynes1985b,grandy1980}
has been used for a variety of inferences about the neuronal activity of
various brain areas and about other phenomena of importance to the
...
example second-order time correlations, is sufficient for making other
inferences about the data. It can be considered as an approximation of
probabilistic models based on various assumptions of inferential
sufficiency \citep{jaynes1986d_r1996,portamana2017} -- for example of
empirical moments or of long-run frequencies.

\mynote{Say something more about advantages of such question/answer: for
  example we can make statements about total activity of brain region even
...
Let's introduce some context and notation for our inference problem.

The context we consider is as follows. We have recorded the spiking
activities of $n$ neurons for a given amount of time. These spikes are
binned into $T$ time bins and binarized to $\set{0,1}$ values in each bin.
Denote by $\ya_{t}$ the number of neurons that fire during time bin $t$,
divided by the total number of neurons $n$; we call this the
\emph{normalized total activity} or just \enquote{total activity} for
short. Obviously $\ya_{t} \in\set{0,1/n,\dotsc,(n-1)/n,1}$; if $\ya_{t}=0$,
no neuron spiked during time bin $t$; if $\ya_{t}=1$, all spiked at some
point during time bin $t$. For brevity, let's say \enquote{at $t$} for
\enquote{during time bin $t$}.

\mynote{maybe move this paragraph to the intro?}Present-day technologies
enable the recording of neuronal activity from small brain regions
\mynote{be more specific}. The neurons that are recorded within the region
are not, and cannot yet be, specifically chosen from among the rest, owing
to several limiting factors -- for example, limitations in the precision
with which electrodes are inserted, or with which neurons are targeted by
viruses, and so on. In fact, the set of recorded neurons may even vary
during very long recordings or across experimental trials. We assume that
there's a small region, comprising a population of $N$ neurons, from which
other sets of neurons had the same probability of being recorded as the set
of $n$ neurons that have actually been recorded. During the recording, the
normalized total activity of these $N$ neurons at $t$ was $\yA_{t}$. We
don't know the values $\yA_{t}$; we only know for certain that
$\yA_{t} \in \set{0,1/N,\dotsc,(N-1)/N,1}$ and that $N\yA_{t} \ge n\ya_{t}$
for obvious reasons. For the moment we assume that we know $N$; we'll
discuss later the consequences of our lack of precise knowledge about $N$.

Our questions concern general features of the total activity $\yA$ of the
full population; for example: what was its time-frequency distribution? How
much activity should we expect in a time bin during the recording, or in a
new time bin? The answers to these questions are probabilistic and come in
the form of distributions.

The idea behind the approach presented here is easily summarized:
\begin{enumerate}[label=(\alph*)]
  \item\label{item:maxent_step} we build a distribution for the total
  activity of the full population using the maximum-entropy method;
  \item\label{item:sample_step} the expectation constraints used in the
  method for the full population are, in turn, determined via sampling
  theory from the expectation constraints for the sample.
\end{enumerate}
Let's discuss these points in more detail.

Regarding point~\ref{item:maxent_step}, we assume you to be familiar with
the maximum-entropy method. In short, it amounts to a pair of
prescriptions: choose the distribution having maximum Shannon entropy among
those satisfying specific convex constraints, such as expectation values;
and judge these expectation values to be equal to measured averages. We add
two remarks that are seldom made in the literature. First, the
\enquote{distribution} given by this method is the zeroth-order
approximation of four different distributions:
\begin{itemize}
\item the \emph{probability} distribution for the value of the activity at any
  time bin among those \emph{recorded},
\item the \emph{probability} distribution for the value of the activity at
  a \emph{new} time bin,
\item the most probable \emph{time-frequency} distribution of the activity
  during the \emph{recording},
\item the most probable \emph{time-frequency} distribution of the activity
  in a very long run of \emph{new} time bins.
\end{itemize}
The maximum-entropy distribution is thus an approximate answer to four
conceptually different inferences. Note that although the four
distributions above numerically coincide in this approximation, they
numerically differ in higher-order approximations. Second, the
maximum-entropy method based on the Shannon entropy implicitly makes
specific assumptions about the probability of the long-run frequency
distributions \citep{jaynes1986d_r1996,portamana2009,portamana2017}. We
discuss these assumptions in \sect***

In our case, to apply the method to the total activity of the full
population we need to fix some of its expectation values, for example its
moments. But we don't have any measured averages for the full population to
equate our expectations to. Here enters point~\ref{item:sample_step}: the
probability calculus gives an exact, linear relation between the first $m$
moments for the full population and the first $m$ for the sample; the ones
determine the others and vice versa at every time bin \citep[for
example][\eqns~(16)]{portamanaetal2015}. This is a classical result of
sampling theory \citep[\chaps~I--IV]{whitworth1867_r1965}[\chap~II]{feller1950_r1968}[\chap~3]{jaynes1994_r2003}[see
also][]{whitworth1897}.

Combining this result with the maximum-entropy prescription
\enquote{expectations${}={}$measured averages} for the sample, we have that
the measured averages for the sample determine the moments for the full
population:
\begin{equation*}
...
These two steps are even more straightforward if instead of power moments
we use \emph{normalized factorial moments}. The $m$th normalized factorial
moment is defined as the expectation
\begin{equation}
  \E\condt[\Bigg]{
    \binom{n \ya}{m}\,\binom{n}{m}^{-1}}{\yH}
  \equiv
  \sum_{\ya} %m!\,
  \binom{n \ya}{m}\,  \binom{n}{m}^{-1}\, \pf(\ya \| \yH),
\end{equation}
where $\ya\in\set{0, 1/n, 2/n, \dotsc, (n-1)/n, 1}$ is the normalized total
activity of a population of $n$ neurons, and $\pf(\ya \| \yH)$ is the
probability for activity $\ya$ given assumptions $\yH$. This moment can be
interpreted as the expectation of the number of distinct $m$-tuples of
simultaneously active neurons, normalized by the maximum possible number of
distinct $m$-tuples. For example, with $m=2$, if $na=4$ neurons are active
in a population of $n=5$, we have $\tbinom{4}{2}=6$ distinct pairs of
simultaneously active neurons, and if all five neurons were active we would
have a maximum of $\tbinom{5}{2}=10$ distinct pairs; the normalized number
of pairs is therefore $6/10$. Note that the first $m$ factorial moments
linearly determine the first $m$ moments and vice versa, so fixing the ones is
equivalent to fixing the others. Now assume that these $n$ neurons are a
sample from a full population of $N$ neurons, with possible normalized
activity $\yA \in\set{0,1/N,\dotsc,1}$. We have this extremely convenient
...
We can hence apply the maximum-entropy method to obtain a distribution for
the full population by constraining its $m$th factorial moment to be equal
to the sample's recorded average of $\tbinom{n\ya}{m}\;\tbinom{n}{m}^{-1}$,
for as many $m$ as we please with $1\le m \le n$. If $\ya_{t}$ is the
normalized total activity of the sample during the time bin $t$, and the
recording with $T$ time bins, the constraint on the distribution
$\pf(\yA \| \yH)$ for the full population is
\begin{equation}
