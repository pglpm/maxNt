...
%% Last-Updated: 2019-06-18T19:00:36+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
activities of $n$ neurons for a given amount of time. These neurons are our
\enquote{sample} or \enquote{subnetwork}. Their spikes are binned into $T$
time bins and binarized to $\set{0,1}$ values in each bin. Denote by
$\ya_{t}$ the number of neurons that fire during time bin $t$, divided by
the total number of neurons $n$; this is the \emph{normalized total
  activity} of the sample, or just its \enquote{total activity} or
\enquote{activity} for short. Obviously
$\ya_{t} \in\set{0,1/n,\dotsc,(n-1)/n,1}$; if $\ya_{t}=0$, no neuron spiked
during bin $t$; if $\ya_{t}=1$, all spiked at some point during bin $t$.
For brevity, let's say \enquote{at $t$} for \enquote{during time bin $t$}.
From the activities $\set{\ya_{t}}$ we can count how many times the
activity levels $\ya=0$, $\ya=1/n$, and so on appeared during the
...
recorded within the region are not, and at present cannot be, specifically
chosen from among the rest, owing to several limiting factors -- for
example, limitations in the precision with which electrodes are inserted,
or with which neurons are targeted by viruses, and so on. In fact, the set
...
network of $N$ neurons, from which other sets of neurons had the same
probability of being recorded as the set of $n$ neurons that was actually
recorded. This is our \enquote{full network}\mynote{how about calling it
  \enquote{the pool}?}. During the recording, the normalized total activity
of these $N$ neurons at $t$ was $\yA_{t}$; the relative frequencies of the
various activity levels were $(\yFF_{yA}) \defs \yF$. We don't know the
...
frequency distribution $\yF$ during the recording? How much total activity
$\yA_{t}$ should we expect at a time bin $t$ during the recording? And at a
new time bin? We cannot answer these questions with certainty; we can only
give distributions of probability over their possible answers. The approach
presented here gives such probability distributions.
