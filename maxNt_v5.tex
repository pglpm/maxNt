...
%% Last-Updated: 2019-06-18T20:14:11+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
activities of $n$ neurons for a certain amount of time. These neurons are
our \enquote{sample} or \enquote{subnetwork}. Their spikes are binned into
...
the total number of neurons $n$; this is the \emph{normalized total
  activity} of the sample, or just \enquote{activity} for short. Obviously
$\ya_{t} \in\set{0,1/n,\dotsc,(n-1)/n,1}$; if $\ya_{t}=0$, no neuron spikes
during bin $t$; if $\ya_{t}=1$, all spike at some point during bin $t$. For
brevity, let's say \enquote{at $t$} for \enquote{during time bin $t$}. From
the activities $\set{\ya_{t}}$ we can count how many times the activity
levels $\ya=0$, $\ya=1/n$, and so on appeared during the recording,
obtaining the measured relative-frequency distribution
