...
%% Last-Updated: 2019-07-10T21:31:16+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
the total activity of the full network, if the latter is assumed to be
larger than 1\,000 neurons, has two very distinct modes, both at low
...
uncertainty about the data. It can be considered as an approximation of
...
during bin $t$. For brevity, let's say \enquote{at $t$} for \enquote{during
  time bin $t$}. From the activities $\set{\ya_{t}}$ we can count how many
times the activity levels $\ya=0$, $\ya=1/n$, and so on appeared during the
recording, obtaining the measured relative-frequency distribution
...
enable the recording of neuronal activity from small brain areas
\mynote{be more specific}. For many animal species, the neurons that are
recorded within the area are not -- and at present cannot be --
specifically chosen from among the rest, owing to several limiting factors;
for example, limitations in the precision with which electrodes are
inserted, or with which neurons are targeted by viruses. In fact, the set
of recorded neurons may even change during very long recordings or across
experimental sessions. We assume that there's a area, comprising a
network of $N$ neurons, from which other sets of neurons had or will have
the same probability of being recorded, even in other experimental
sessions, as the set of $n$ neurons that was actually recorded in this
session. This is our \enquote{full network}\mynote{how about calling it
...
levels during the recording were $(\yFF_{\yA}) \defs \yF$. We don't know the
values $\yA_{t}$ at each $t$, or the frequency distribution $\yF$. We only
know for certain that $\yA_{t} \in \set{0,1/N,\dotsc,(N-1)/N,1}$ and that
...
only give distributions of probability over their possible answers. The
...
quantified guesses about the full-network activity. First of all, this seems to be
the primary idea behind recording a sample. Second, it allows us to make
comparisons across experimental sessions; such comparisons would me
difficult or meaningless made at the level of recorded samples, which
generally comprise non-overlapping sets of neurons differing also in size.
...
from four moments (\textcolor{myblue}{\textbf{------}}). The lower plot
...
(\textcolor{myblue}{$\bm{\square}$}), and the distributions obtained by
...
moments (\textcolor{myblue}{$\bm{\blacksquare}$}). The measured frequency
...
\item The application at the sample level leads to deceivingly similar
  ...
  \textcolor{myblue}{$\bm{\square}$}), which can only be distinguished on a
...
and our uncertainty about its possible values is a probability distribution
$\pf(\yF\|\yD\yI)$ conditional on data $\yD$ and prior knowledge $\yI$,
which we approximate with a continuous density
\begin{equation}
  \label{eq:density_F}
  \pf(\yF \| \yD \yI)\;\di\yF.
\end{equation}
In appendix*** we give a summary of how this density is derived. Note that
...
our probability for $\yA_{t}$ is $\yFF_{\yA_{t}}$, out of combinatorial
reasons. But if we are also uncertain about $\yF$, with
probability~\eqref{eq:density_F}, then our probability for $\yA_{t}$ by
marginalization is
\begin{equation}
...
\end{equation}
our probability distribution for $\yA_{t}$ is thus simply the mean
...
\begin{equation}
  ...
  \pf(\yF \| \yD \yI)\;\di\yF \approx \delta(\yF-\yFm)\;\di\yF,
\end{equation}
...
briefly discuss its mathematical features and examine the answers it
provides. Its form is this:
\begin{equation}
  ...
  \exp[-\yL\;\ysh(\yF;\yR)]
\end{equation}
and the three main terms have the following meaning:

$\exp[-\yL\;\ysh(\yF;\yR)]$ expresses our pre-data uncertainty about the
possible frequency distributions. Here
$\ysh(\yF;\yR)\defd \sum_{\yA}\yFF_{\yA}\ln\frac{\yFF_{\yA}}{\yRR_{\yA}}$
is the relative entropy or discrimination information
\citep{kullback1987,jaynes1963,hobson1969,hobsonetal1973}, $\yR$ is a
reference distribution, and $\yL$ a positive parameter. This density is
proportional to the number of ways in which the frequency distribution
$\yF$ can be realized if we assume that the activities appear with
frequencies $\yR$ in the long run, as can be seen using Stirling's
approximation. For $\yR$ we use the uniform distribution, but any
biologically sensible distribution, which gives higher frequencies to low
activities, leads to the same final results. The parameter $\yL$ roughly
corresponds to the number of data points necessary to change our initial
belief, and we set it equal to $50$. \mynote{connection with entropic
  priors
  \citep{rodriguez1991,skilling1998,catichaetal2004,neumann2007,portamana2017}
}

$\Nd(\yd\yF \| 0, \yds)$ expresses our belief, hinging on biological
arguments, that the frequency distribution should be somewhat smooth: if
the activity levels $\yA-1$ and $\yA+1$ appear roughly equally often, we
don't believe that the activity $\yA$ could appear with an extremely
different frequency than theirs. The matrix $\yd$ is the discrete
equivalent of a fourth-order derivative \mynote{ref}; the probability of
its squared integral decreases exponentially with standard deviation
$\yds$. We set this such that the smoothness of the maximum-entropy
distribution is within two standard deviations, thus allowing for more
maxima and minima in $\yF$.

The two densities above together express our pre-data beliefs about the
...
***mode of marginal can be very different from the one in full space
(minka1998)

*** diff position of mode in marginals says that full distr is extremely
skewed – not surprising since the base space is bounded.
...
\begin{figure}[!p]
\centering
\includegraphics[width=\linewidth]{_scripts/MC_plot1000.pdf}%
\\[3em]%
\includegraphics[width=\linewidth]{_scripts/MC_samples1000.pdf}%
\caption{***}
\label{fig:comparisons_marginals}
\end{figure}
% mctests3.nb
...
\begin{figure}[!p]
\centering
\includegraphics[width=\linewidth]{_scripts/MC_plot5000.pdf}%
\\[3em]%
\includegraphics[width=\linewidth]{_scripts/MC_samples5000.pdf}%
\caption{***}
\label{fig:comparisons_marginals}
\end{figure}
% mctests3.nb
