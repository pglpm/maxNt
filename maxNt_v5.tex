...
%% Last-Updated: 2019-07-11T14:53:17+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
uncertainty about the possible frequency distributions. Here
...
necessary to change our initial belief, and we set it equal to $50$. This
...
uncertainty about the measurement results, which can come for example from
...
$\yL$ is large, and $\ycs L$ is small. Its mode in fact will be the
constrained $\yF$ that maximizes the relative entropy $\ysh(\yF;\yR)$. It
should be noted that a pre-data belief different from the entropic
one~\eqref{eq:entropic_prior} will lead to a different mode approximation.
For example, a Dirichlet density would lead to a maximum-entropy
distribution with \emph{Burg's} entropy \citey{burg1975} instead of
Shannon's \citep{jaynes1986d_r1996}.
...
\\[3em]%
\includegraphics[width=\linewidth]{_scripts/MC_marginal1000.pdf}%
\caption{***}
\label{fig:MC1000}
\end{figure}
% mctests3.nb
...
\\[3em]%
\includegraphics[width=\linewidth]{_scripts/MC_marginal5000.pdf}%
\caption{***}
\label{fig:MC5000}
\end{figure}
% mctests3.nb
% mcfd_6mom_unif_afssd32s_cok_tr_N5000_t180_L50_T0.333333333333333_s25800_a329212306.071189
The properties of the belief density~\eqref{eq:density_proportional} are
shown in \fig~\ref{fig:MC1000} for $N=1\,000$ and in \fig~\ref{fig:MC5000}
for $N=5\,000$, considering the first six factorial moments as our data. It
turns out that that six moments lead to practically the same result as
specifying the full frequency distribution for the sample; they are therefore
informationally sufficient.

The top plots in the figures show the mean
$\int \yF \pf(\yF \| \yD \yI)\,\di\yF$
(\textcolor{myblue}{\textbf{------}}), which is also our belief about the
...
(\textcolor{mygreen}{------}) drawn from it. The mean and the
maximum-entropy mode are nevertheless qualitatively similar. In fact, for
$N=1\,000$ the second peak of the mean is even higher than the
corresponding maximum-entropy one. The samples also show that generally two
main frequency peaks of activity are expected, especially for $N=5\,000$.
