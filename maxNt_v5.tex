...
%% Last-Updated: 2019-06-18T16:16:26+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
bin $t$. For brevity, let's say \enquote{at $t$} for \enquote{during time
  bin $t$}. From the activities $\set{\ya_{t}}$ we can count how many times
the activity $\ya=0$, $\ya=1/n$, and so on each appeared during the
...
$\yA_{t}$; the relative frequencies of the various activity levels were
$\yFF(\yA) \defs \yF$. We don't know the values $\yA_{t}$ at each $t$ or
the frequency distribution $\yF$. We only know for certain that
...
should we expect at a time bin during the recording? And at a new time bin?
We cannot answer these questions with certainty; we can only give
