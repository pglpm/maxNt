...
%% Last-Updated: 2019-06-25T17:44:44+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
The maximum-entropy method has been applied in the neurosciences to
questions of sufficient statistics -- for example, whether we approximately
reach the same state of uncertainty about network activity by considering
only its network-averaged pairwise correlations and by considering also
third-order correlations. The method is typically used as follows. We
imagine to be ignorant of the measured frequency distribution for the
network activity, and to know only the measured statistics of interest; two
maximum-entropy distributions are then built, constrained under the two
sets of moments under comparison. Finally, the resulting distributions are
compared with each other and with the measured frequency distribution.

The method introduced in the present work reveals an issue in the recipe
above. The issue doesn't come from the method itself but from a simple,
general fact of probability theory: if a probability distribution has some
...
vice versa (except for trivial cases, such as uniform distributions). This
also holds among marginals of different order (to $n$ neurons or to $n+1$
neurons, for example). This impossibility is known in statistical
...
In the neuroscientific application above, this means for example that if
pairwise correlations are sufficient statistics for a sample of a network,
then they \emph{cannot} be sufficient for the full network or for a sample
with more or fewer neurons; and vice versa, if we assume that they are
sufficient statistics for the full network, then we must expect them
\emph{not} to be sufficient for any recorded sample thereof. It seems to us
that such questions about sufficient statistics are implicitly meant to
apply to a specific brain area, not just to some casually recorded samples
thereof. Thus, maximum-entropy methods applied at the sample level
