...
%% Last-Updated: 2019-06-13T16:07:01+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
and judge these expectations to be equal to measured averages. We add two
remarks that you may not be familiar with. First, the
...
probability calculus gives an exact relation between the first $m$ moments
for the full population and the first $m$ for the sample; the ones
determine the others and vice versa at every time bin. This is a classical
result of sampling theory \citep[for
example][\chaps~I--IV]{whitworth1867_r1965}[\chap~II]{feller1950_r1968}[\chap~3]{jaynes1994_r2003}[see
also][]{whitworth1897}. Combining this result with the maximum-entropy
prescription \enquote{expectations${}={}$measured averages} for the sample,
we have that the measured averages for the sample determine the moments for
the full population:
\begin{equation*}
...
This step is even more straightforward if instead of power moments we use
...
distinct $m$-tuples. Note that the first $m$ factorial moments determine
the first $m$ moments and vice versa, so fixing the ones is equivalent to
fixing the others. Now assume that these $n$ neurons are a sample from a
full population of $N$ neurons, with possible normalized activity
$\yA \in\set{0,1/N,\dotsc,1}$. We have this extremely convenient property:
\emph{The first $n-1$ normalized factorial moments for the sample and for
  the full population are numerically identical}:
\begin{equation}
...
Hence we can apply the maximum-entropy method to obtain a distribution for
the full population by constraining, for any $m$s we please, its $m$th
factorial moment to be equal to the sample's recorded average of
$\tbinom{n\ya}{m}\;\tbinom{n}{m}^{-1}$. If $\ya_{t}$ is the normalized
total activity of the sample during the time bin $t$, during a recording
with $T$ time bins, the constraint on the distribution $\pf(\yA \| \yH)$
for the full population is
\begin{equation}
