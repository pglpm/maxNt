...
%% Last-Updated: 2019-04-08T11:32:41+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Maximum-entropy distribution for a 
...
\begin{equation}
  ...
  \p(\yRv_1, \yRv_2, \dotsc, \yRv_\yNv \| \yHc)
  \quad\text{or}\quad
\p(\yR \| \yHc), \quad \yR \in \set{0,1}^\yNv.
\end{equation}
Our uncertainty about the state of the sample is likewise expressed by
\begin{equation}
  \label{eq:sample_plaus}
  \p(\yrv_1, \yrv_2, \dotsc, \yrv_n \| \yHc) \quad\text{or}\quad
\p(\yr \| \yHc), \quad \yr \in \set{0,1}^n.
\end{equation}
...
\begin{equation}
  ...
  \p(\yR \|  \yH) \equiv
  \sum_{\yRf}\p(\yR \| \yRf, \yH)\,
  \p(\yRf \| \yH) =
  \binom{\yNv}{\yNv\yRf}^{-1} \p(\yRf \| \yH).
\end{equation}
...
\begin{equation}
  \label{eq:marginal}
  \p(\yr \| \yH) = \binom{n}{n\yrs}^{-1} \p(\yrs \| \yH).
\end{equation}
...
$\p(\yrs \|\yRf, \yH) \equiv \ypp(\yrs \|\yRf)$, besides
...
\begin{multline}
  ...
  \E(g\|I)
  \equiv
  \sum_{\yrs} g(\yrs)\,\p(\yrs \| \yH)
  =
  \sum_{\yrs} g(\yrs) \sum_{\yRf} \ypp(\yrs\|\yRf)\,\p(\yRf \| \yH)
  ={}\\
...
\begin{equation}
  ...
  c = \E(g \| \yH) \equiv \sum_{\yRf} \biggl[ \sum_{\yrs} g(\yrs)  \ypp(\yrs\|\yRf) \biggr]\,
  \p(\yRf \| \yH).
\end{equation}
...
\begin{equation}
  ...
  \p(\yrs=a \|\yH) \approx \p(\yRf=a \|\yH),\qquad
\E[g(\yrs) \|\yH] \approx \E[g(\yRf) \|\yH],
\end{equation}
...
\begin{multline}
  ...
  c_k = \E(g_k \|\yHa)\equiv{}\\
\shoveright{  \frac{1}{\yK(\yL)}\sum_{\yrs} \sum_{\yRf} g_k(\yrs) \ypp(\yrs \|\yRf)
\,
  \binom{\yNv}{\yNv \yRf}\,\exp\Bigl[\yL\T
  \sum_{\yrs} \yf(\yrs)\ypp(\yrs\|\yRf)\Bigr],}
  \\ k=1,\dotsc,m.
...
\caption{Linear and log-plots of $\p(\yrs)$ constructed by \me\ at
...
\caption{Linear and log-plots of $\p(\yrs)$ for a sample of $n=200$
