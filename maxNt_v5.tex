...
%% Last-Updated: 2019-06-17T17:13:33+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
for the total activity of the full network, if the latter is assumed to
...
network aren't observed in the sample, nor are they inferred by the
...
activities of $n$ neurons for a given amount of time. Let's call these
neurons the \enquote{sample} or the \enquote{subnetwork}. Their spikes are
...
\emph{normalized total activity} of the sample, or just its \enquote{total
  activity} or \enquote{activity} for short. Obviously
$\ya_{t} \in\set{0,1/n,\dotsc,(n-1)/n,1}$; if $\ya_{t}=0$, no neuron spiked
during time bin $t$; if $\ya_{t}=1$, all spiked at some point during time
bin $t$. For brevity, let's say \enquote{at $t$} for \enquote{during time
  bin $t$}.
...
viruses, and so on. In fact, the set of recorded neurons may even change
during very long recordings or across experimental trials. We assume that
there's a region, comprising a network of $N$ neurons, from which other
sets of neurons had the same probability of being recorded as the set of
$n$ neurons that has actually been recorded. We call this the \enquote{full
  network}\mynote{how about calling it \enquote{the pool}?}. During the
recording, the normalized total activity of these $N$ neurons at $t$ was
$\yA_{t}$. We don't know the values $\yA_{t}$ at each $t$; we only know for
certain that $\yA_{t} \in \set{0,1/N,\dotsc,(N-1)/N,1}$ and that
$N\yA_{t} \ge n\ya_{t}$ for obvious reasons. For the moment we assume that
we know $N$; in \sect*** we discuss the consequences of our lack of precise
knowledge about this number.

Our questions concern general features of the total activity $\yA$ of the
full network during and after the recording. For example: what was its
frequency distribution in time? How much total activity should we expect at
a time bin during the recording, or at a new time bin? We cannot answer
these questions with certainty; we can only give a distribution of
probability over the possible answers. Our approach gives such a
probability distribution.

\bigskip

The idea behind the approach is easily summarized:
\begin{enumerate}[label=(\alph*)]
  ...
  maximum-entropy method for the full network are, in turn, determined
  via sampling theory from the expectation constraints for the sample.
\end{enumerate}
...
satisfying specific convex constraints, such as fixed expectation values;
and judge these expectation values to be equal to measured averages. We add
two remarks about this method that are seldom made in the literature.
First, the \enquote{distribution} given by this method is the zeroth-order
approximation of four different distributions:
\begin{itemize}
\item the most probable \emph{time-frequency} distribution of the total
...
distributions above numerically coincide in this approximation, they differ
in higher-order approximations. We discuss their differences in \sect***.
The maximum-entropy distribution approximates best the first of the four
distributions, so that's the one we'll mainly have in mind, and we'll
denote it by $\Fm(\yA)$.***
Second, the maximum-entropy method based on the Shannon entropy implicitly
makes specific assumptions about the probability of the long-run frequency
...
we need to fix some of the latter's expectation values, for example its
moments. But we don't have any measured averages for the full network to
...
\emph{normalized factorial moments}. The $m$th normalized factorial moment
of the probability distribution for the activity of the recorded neurons is
...
\begin{equation}
  ...
  \qquad 1\le m \le n
\end{equation}
where $\pf(\ya \| \yH)$ is the probability for activity $\ya$ given
assumptions $\yH$; \enquote{$\ya$} may refer to any recorded bin or to a
new bin. This moment can be interpreted as the expectation of the number of
distinct $m$-tuples of simultaneously spiking neurons (within a bin's time
width), normalized by the number of distinct $m$-tuples. For example, with
$m=2$, if $na=4$ neurons spike in a network of $n=5$, we have
$\tbinom{4}{2}=6$ distinct pairs of simultaneously spiking neurons, and the
total number of distinct pairs is $\tbinom{5}{2}=10$. The normalized number
of spiking pairs is therefore $6/10$. Note that the first $m$ factorial
moments linearly determine the first $m$ power moments and vice versa,
because $\tbinom{n\ya}{m}$ is a polynomial in $\ya$ of degree $m$; so
fixing the ones is equivalent to fixing the others. Now we have this
...
$1\le m \le n$. The constraints on the distribution $\pf(\yA \| \yH)$ for
...
The number of moments used with this method depends on the specific
questions and hypotheses that a researcher is exploring; for example,
hypotheses of sufficient statistics, such as the sufficiency of pairwise
correlations for inferential purposes. In the present work we only want to
present the general method without entering into biological questions of
this kind. The particular case in which $n$ moments are constrained
corresponds to fully specify the measured time-frequency distribution of
the sample activity. In this case, our inferences about the full-network
activity are using all available measured frequency data. Using the full
frequency distribution of the sample may be a bad idea, however, because
the maximum-entropy distribution becomes a bad approximation of some of the
four distributions described above. It is preferable to use a moderately
high number of moments smaller than $n$. We explain this point in \sect***.

\bigskip

In the next section we apply the method to the data sets from two actual
recordings, using *** moments, and discuss the properties of the resulting
distributions.
...
The first data set, from \textcite[rat 14147]{stensolaetal2012}, consists
of $n=65$ neurons (27 of which classified as grid cells) from rat Medial
Entorhinal Cortex, recorded for about 20 minutes. Their spikes are
binned into $T=417\,641$ bins of $3\;\textrm{ms}$ width.

The second data set, from
