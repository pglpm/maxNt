...
%% Last-Updated: 2019-11-14T22:48:47+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
correlations versus correlations of higher order \citep[see for
...
correlation sufficiency for the larger network, and vice versa, as will be
explained in detail in \sect~\ref{sec:marginalization}. Therefore,
...
more reliable. We show this point in \sect~\ref{sec:application} for a real
data set.
...
this difference shows that the first two moments alone are \emph{not}
informationally sufficient for this specific data set.

It is instructive to contrast the distributions for the full network with
those obtained applying the maximum-entropy method \emph{at the sample
  level} with the same constraints, as traditionally done. The sample-level
distributions are shown as an inset in the plot for the first data set. We
see that constraining four moments doesn't lead to a distribution much
different from constraining only two. This would suggest that two moments
are informationally almost sufficient. This conclusion would be deceptive:
it obviously cannot be extended to the full network. We discuss this point
more in detail in the next section.


\section{Example application for sufficiency of partial statistics:\\the
  issue with methods at the sample level}
\label{sec:marginalization}

As mentioned in the Introduction (see references there), in the
neurosciences the maximum-entropy method has also been applied to questions
of sufficient statistics. For example: do pairwise correlations in the
neuronal activity provide the same information as the full frequency
distribution of activities? In this section we discuss how our proposed
application bears on this kind of questions. % For concreteness we use an
% example with specific statistics: first and second moments (equivalent to
% mean and correlations), versus first-to-fourth moments. But our discussion
% holds for any sets of statistics.

The question must be made more precise first. We want to assess our degree
of belief about the frequencies of the activities of the sample, or about
the activity of the sample in a new time bin. For this assessment we should
use all measured data we have. But it sometimes happens that dropping part
of the data -- for example, the measured third- and higher-order moments --
leaves our assessment almost unchanged: the dropped data are
\emph{informationally irrelevant}, or almost so. The remaining data -- for
example, first and second moments -- are then \emph{informationally
...
% 322.72, -85.7627, -42.8005, -4.23746
