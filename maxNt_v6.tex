...
%% Last-Updated: 2019-11-20T20:23:20+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
relation between the first $m$ moments for the full network and the first
...
for any belief distribution $\pxxx(\yA_{t})$ for the full-network activity
...
% example, with $m=2$, if $a=4$ neurons spike in a network of $n=5$, we
...
degree $m$. % Specifying one set is therefore
...
analysis: \emph{the first $n$ normalized factorial moments for the sample
  and for the full network are numerically identical}:
\begin{equation}
...
$\E\bigl[\binom{\yA}{m}\bigr]$ of the full-network distribution
...
expectations for the full network are determined by those for the sample
...
$\px(\yA \| M, N)$ for the full network of $N$ neurons by constraining some
...
about the \enquote{cooperativity} or \enquote{interaction} in the network
...
distribution~\eqref{eq:app_maxent_pop_first} \emph{may not have exact
  solutions} if $N$ is strictly larger than $n$; and the possible
discrepancy increases with the number of constraints. The essential reason
for this discrepancy is the approximate character of two assumptions on
which the maximum-entropy method is based and of one assumption specific to
our application: measured moments equal expectations (or equivalently that
the number of time bins $T$ is infinite), the order of the time bins is
irrelevant, and that all full-network subsets with $\yA$ neurons are
equally likely to be generating a total activity $\yA$. We discuss these
assumptions in \sect~\ref{sec:assumptions}.
...
three remarks on the maximum-entropy method that are seldom made in the
...
of four different distributions for the full network, which differ
...
Second, the maximum-entropy method % (based on the \emph{Shannon} relative
...
frequency distribution of activities. The often heard statement that it's
\enquote{the maximally unbiased (or non-committal) distribution} must be
taken with a grain of salt. \citep[\mynote{move to
...
factorial moments of sample and full network.
