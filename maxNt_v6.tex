...
%% Last-Updated: 2019-12-02T14:16:29+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
The limitations of approach here proposed can be appraised by examining the
assumptions and approximations on which it rests. Its assumptions and
approximations become clear within a full-fledged probabilistic inference
problem, which we sketch qualitatively.

The first and most important assumption of this kind of maximum-entropy
application in neuroscience can be expressed in three equivalent ways:
\begin{enumerate*}[(\roman*)]
\item the order of the time bins has no relevance; \item the frequencies of
  the activity during a recording are informationally sufficient for
  inferences about unrecorded times; \item our degree of belief about the
  sequence of activities is invariant under permutations of the time bins,
  that is, exchangeable \citep[\chap~4]{bernardoetal1994_r2000}{dawid2013}.
\end{enumerate*}
...
frequency distribution $(\yFF_{\yA})$ in $T$ time bins is given by a
mixture of terms
\begin{equation}
...
of the same experiment: its constraints represent macroscopically
...
is that $n$ neurons are sampled anew at each time bin.
\begin{equation}
...
Both assumptions above make sense as toy hypotheses for judging the
...
In an exact calculation based on the two assumptions above, the probability
of the sequences of sample and larger-population activities,
$(\ya_{t}, \yA_{t})$, involves the joint frequencies of total activities of
sample and larger population $J_{\ya,\yA}$, multiplied by the number of
time bins $T$:
\begin{equation}
  \label{eq:prob_joint_frequencies}
  \underbracket{\prod_{t} \pxx(\ya_{t}, \yA_{t})}_{\mathclap{\textit{prob. of joint sequence}}} \equiv
  \prod_{\ya,\yA} \pxx(\ya, \yA)^{T\,J_{\ya,\yA}} =
  \prod_{\ya,\yA}\bigl[\underbracket{\vphantom{\underline{g}}\yG_{\ya\yA}\;\nu_{\yA}}_{\mathclap{\textit{sampling relation}}}\bigr]^{T\,J_{\ya,\yA}}.
\end{equation}
The reasoning would be similar to that leading to
\eqn~\eqref{eq:prob_frequencies} of appendix~\ref{sec:measure_suff}.
Conditioning on the frequencies $\yf$ of the sample would correspond to
constraining the marginal $\sum_{\yA}F_{\ya,\yA}=\yff_{\ya}$. The
maximum-entropy method approximates $T$ with infinity, thus treating the
probability above as a delta. The result is that
\begin{enumerate*}[label=(\roman*)]
\item the probability and observed frequency for the total activity $\yA$
  are constrained to be equal,
\item the frequency of sample activity $\ya$ conditional on
  larger-population activity $\yA$ is constrained to equal the
  hypergeometric distribution $\yG_{\ya\yA}$,
  \eqn~\eqref{eq:conditional_hypergeometric}.
\end{enumerate*}
...
The final assumption in the maximum-entropy method regards the form of the
probability $\pxxx(\yA)$ in the expression above.
