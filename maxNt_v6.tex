...
%% Last-Updated: 2019-11-15T15:51:26+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
quantifying the \enquote{cooperativity} \citep{gersteinetal1985} or
\enquote{interaction}
\citep{martignonetal1995,schneidmanetal2006,shlensetal2006} or
\enquote{synchrony} \citep{bohteetal2000,amarietal2003} of neuronal
...
suit all these notions \parentext{for an intuitive understanding of
  sufficiency, which goes back to \cite{fisher1922}, see \eg\
  \cite[\sect~4.5]{bernardoetal1994}; \cite[\chap~8,
  \sect~14.2]{jaynes1994_r2003}; \cite{cifarellietal1982};
...
The idea is as follows. We want to assess our degree of belief about the
...
second moments -- are then \emph{informationally
...
We can check how much a part of the data is irrelevant by simply dropping
it from the conditional of our probabilities, and see if the latter change
appreciably. When our probabilities are built with the maximum-entropy
method, this is done by dropping part of the constraints and see how the
resulting distribution changes. For example, we may only retain the first
two moments (means and correlations) or the first four moments. This
procedure is equivalent to calculating and comparing the probabilities of
our hypotheses about sufficiency, conditional the data, assuming their
pre-data probabilities to be equal. We perform this calculation below.
...
In the neurosciences this check is often done with maximum-entropy
distributions constructed \emph{for the sample}, ignoring the full network
it's sampled from \citep[for
example][]{martignonetal1995,bohteetal2000,schneidmanetal2006,shlensetal2006,barreiroetal2010,ganmoretal2011,granotatedgietal2013}.
The method introduced in the present work, however, reveals an issue with
this application at the sample level. The issue doesn't come from the
method itself but from a general fact of probability theory. If a
probability distribution has some sufficient statistics
...
\citep{maesetal1999}. Mathematically this impossibility translates into a
system of $n$ independent equations in $m$ unknowns with $n>m$
\citep[\sect~3.1]{portamanaetal2015}[\sect~3]{portamanaetal2018b}.
...
recorded sample. Thus, paradoxically, maximum-entropy methods applied
\emph{at the sample level} give an answer opposite to the one we might
