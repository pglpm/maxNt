...
%% Last-Updated: 2019-11-19T08:53:02+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
might be enough for qualitative inferences. As discussed in the next
section, even if the exact number $N$ were known, the maximum-entropy
...
$N$ is the exponential of the relative entropy~\eqref{eq:relentropy_T}:
\begin{equation}
  \label{eq:prob_data_from_N}
  \pf(\yf \| M, N) \approx \exp\bigl\{
  T\,\sh[\yf; \pxx(\ya \| M, N)]\bigr\}.
\end{equation}
If our pre-data probability for $N$ is $p(N)$, then by Bayes's theorem
\begin{equation}
  \label{eq:prob_N_from_data}
  \pf(N \| M, \yf) \propto p(N)\;\exp\bigl\{
  T\,\sh[\yf; \pxx(\ya \| M, N)]\bigr\}.
\end{equation}
...
$N\in \set{1\,000, 2\,000, 5\,000, 10\,000, 20\,000}$. Assuming five
moments $M$ are sufficient,
formula~\eqref{eq:prob_data_from_N} gives
\begin{equation}
...
\end{gathered}
\end{equation}
Our uncertainty is about a scale factor, so it can be represented by equal
pre-data probabilities $1/5$ about these partial orders of magnitude. Thus
from~\eqref{eq:data1_f_from_N} we find 
\begin{equation}
