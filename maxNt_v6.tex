...
%% Last-Updated: 2019-11-13T19:14:02+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
There's also an important methodological reason: the question whether
correlations of some order are informationally sufficient cannot be
properly assessed at the sample level. When the difference in size between
sample and full network is too large, correlation sufficiency for the
sample implies the \emph{lack} of correlation sufficiency for the larger
network, and vice versa. This point is explained in detail in
...
approach in questions of statistical sufficiency \citep[cf. for
...
distribution${}\times N$) for full-network sizes $N=n$, $N=1\,000$,
$N=5\,000$, $N=10\,000$. The case $N=n$ corresponds to applying the
maximum-entropy method at the sample level; it can be observed that with
six moments it reproduces almost exactly the measured frequency
distribution.
\begin{figure}[!p]
\centering
...
questions of sufficient statistics; for example: do pairwise correlations
in the neuronal activity provide the same information as the full frequency
...
Let's make the question more precise first. We want to assess our degree of
belief about the frequencies of the activities of the sample, or about the
activity of the sample in a new time bin. For this assessment we should use
all measured data we have. But it sometimes happens that dropping part of
the data -- for example, the measured third- and higher-order moments --
...
it may hint at peculiar biological  mechanisms or dependencies.
...
it's sampled from. The method introduced in the present work, however,
reveals an issue with this application at the sample level. The issue
doesn't come from the method itself but from a general fact of probability
theory. If a probability distribution has some sufficient statistics
