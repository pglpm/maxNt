...
%% Last-Updated: 2019-11-15T19:22:27+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Maximum-entropy distributions for a 
  neuronal network\\ from subnetwork data {\large\color{myred}[draft]}%\\{\large ***}%
}
...
\begin{abstract}\labelsep 0pt%
  ...
  total activity of a network of neurons, given only some activity data or
  statistics -- for example, empirical moments -- of a \emph{subnetwork}
  thereof. This kind of model is useful because neuronal recordings are
  always limited to a very small sample of a network of neurons.
%
...
We can't say \enquote{the answer is such and such}, but can at best assign
a degree of reasonable belief -- that is, probability -- to every possible
answer. The assessment of this distribution of belief involves experimental
data, such as recordings of neuronal activity from specific brain areas,
and pre-data knowledge about biological conditions and mechanisms. Our
pre-data \dobs\ are often simplified to be mathematically more tractable,
and are therefore called \enquote{models}.

Despite remarkable advances in recording technologies, the best
experimental measurements of instantaneous neuronal activity can still only
record a very small sample of neurons -- hundreds at most -- compared to
the numbers that constitute a functionally distinguished brain region. Many
probabilistic models focus on such samples only, somehow neglecting, in
their assumptions, that the recorded neurons are a sample from a larger
network. This kind of isolation assumptions sometimes escape attention,
being subtly hidden in the mathematics. Some probabilistic models try to
take also unrecorded neurons into account individually, and therefore
become very complex\mynote{Yasser \amp\ Vahid: refs for here?}. It would be
useful to have models that operate in between: addressing the larger brain
area whence the sample comes, but without adding too much complexity and
detail about it. Such intermediate models would be useful, for example, for
preliminary investigations, to help us to decide which hypotheses to
discard and which to consider for more complex and costly studies.

\mynote{[Yasser:] In the present work we aim to address the issue of building probability distributions over a large network using recording from subnetworks by considering the distribution of total population activity as an example. [}
In the present work we propose such an intermediate model. It answers this
question:\mynote{]} How much can the \emph{total} activity of a large neuronal
network have been, given the observation of the activity of a very small
...
\end{enumerate*}
For each data set the model gives the most plausible frequency distribution
with which all levels of total activity of a larger network of 1\,000--10\,000
neurons appeared during the recording. These guessed frequency
distributions are distinctly different from the recorded ones for the
sampled neurons. For the first data set, for example, the frequency
distribution for the full network has two very distinct modes, both at low
activities, %($0.7\%$ and $2.6\%$ of simultaneously active neurons
% within $3\;\mathrm{ms}$,
see \fig~\ref{fig:plots_6mom}, whereas the frequency distribution for the
sample is monotonically decreasing with its maximum at zero activity. The
frequency distribution for the second data set doesn't have two modes but
still presents one prominent shoulder in its low-activity mode. Note that
these guessed features of the full network cannot be inferred by the
application of maximum-entropy \emph{to the sample alone}. Although we don't
formulate any hypotheses on why the larger network could have two distinct
low-activity regimes -- the purpose of the present work is only
methodological -- these results shows that the proposed method can lead to
the formulation or preliminary assessment of interesting hypotheses.
...
There's also an important methodological reason. Maximum-entropy methods
have been used to assess the informational sufficiency of pairwise
correlations and correlations of higher order \citep[see for
...
But when the difference in size between sample and full network is too
large, correlation sufficiency for the sample implies the \emph{lack} of
correlation sufficiency for the larger network, and vice versa. Therefore,
maximum-entropy applications at the sample level can be deceptive for
questions of statistical sufficiency, and a full-network application is
more reliable. We discuss this point in detail in \sect~\ref{sec:marginalization}.
...
How large is the full network addressed by the method proposed here? Its
...
We obviously don't know whether the frequency distribution obtained with
our approach is the \emph{actual} one which the activity levels of the full
network had during the recording; it's only the most plausible. In high
...
the set of all possible frequency distributions, if the full network for
...
% (for a full network of 1\,000 neurons and
...
\section{The approach: maximum-entropy and sampling}
\label{sec:model}
...
$t$: this is the \emph{total activity} of the sample, or just
\enquote{activity} for short. Obviously $\ya_{t} \in\set{0,1,\dotsc,n}$; if
$\ya_{t}=0$, no neuron spikes during bin $t$; if $\ya_{t}=n$, all spike at
some point during bin $t$. For brevity, let's say \enquote{at $t$} for
\enquote{during time bin $t$}. If we divide the total activity by the
population size we have the normalized total activity or
population-averaged activity $\ya/n$, ranging from $0$ to $1$ in $1/n$
steps. From the activities $\set{\ya_{t}}$ we can count how often the
...
bins \emph{outside} of the recorded period. Such activity is unknown to us,
of course.
...
or neurons are targeted by viruses. The set of recorded neurons may even
change slightly across experimental sessions that are very far apart in
time. We assume that there's an area, comprising a network of $N$ neurons,
for which we believe that any other sample of size $n$ could have equally
plausibly been recorded instead of the sample of $n$ neurons that was
actually recorded. We call this larger network the \enquote{full network}.
The total activity of these $N$ neurons at $t$ is $\yA_{t}$. The relative
...
$\yA_{t} \in \set{0, 1, \dotsc, N}$ and that $\yA_{t} \ge \ya_{t}$ for
obvious reasons. For the time being we assume that we know $N$; in
...
during a recording? The approach presented here gives probability
distributions over the possible answers to these questions.
...
Regarding~\ref{item:maxent_step}, we assume familiarity with the
...
distribution about four different quantities. Note that the four
...
\citep{jaynes1986d_r1996,portamana2009,portamana2017}. We discuss these two
remarks further in \sect~\ref{sec:assumptions}. 
...
if $a=4$ neurons spike in a network of $n=5$, we have $\tbinom{4}{2}=6$
...
they linearly determine each other because $\tbinom{\ya}{m}$ is a
polynomial in $\ya$ of degree $m$. Specifying one set is therefore
equivalent to specifying the other set. But the normalized factorial
moments have a convenient property for our analysis
\citep[\sect~2.3]{portamanaetal2015}{potts1953}: \emph{the first $n$
  normalized factorial moments for the sample and for the full network are
...
=    \sum_{\yA=0}^{N} %m!\,
  \binom{\yA}{m}\bigg/  \binom{N}{m}\; \pxxx(\yA),
  % \E\condt[\big]{\tbinom{\ya}{m}\;\tbinom{n}{m}^{-1}}{\yH} =
  % \E\condt[\big]{\tbinom{\yA}{m}\;\tbinom{N}{m}^{-1}}{\yH},
  \qquad 1\le m \le n,
\end{equation}
where $\pxxx(\yA)$ is our distribution of belief about the full-network
activity.

We can therefore apply the maximum-entropy method to obtain a distribution
$\px(\yA)$ for the full network by constraining its $m$th factorial moment
to be equal to the sample's recorded average of
$\tbinom{\ya}{m}/\tbinom{n}{m}$, for as many $m$ as we please with
$1\le m \le n$. In formulae, the constraints on $\px(\yA)$ are
\begin{equation}
...
\underbracket{\frac{1}{T}\sum_{t}\binom{\ya_{t}}{m}\bigg/\binom{n}{m}
  ...
  \binom{\yA}{m}\bigg/  \binom{N}{m} \; \px(\yA)}_{\text{distribution moments}}.
\end{equation}

Finding the constrained maximum-entropy distribution $\px(\yA)$ amounts to
...
on a modern computer without approximations of the partition functions that
appear in the formalism.
...
\item The \textbf{first data set}, from \textcite[rat
  ...
  $3\;\textrm{ms}$ width.
\item The \textbf{second data set}, from
...
% the full-network activity during the recording. But it also approximates
...
approach in questions of statistical sufficiency.
Figure~\ref{fig:plots_6mom} shows the resulting densities (that is,
distribution${}\times N$) for four example values of full-network sizes:
$N=n$, $N=1\,000$, $N=5\,000$, $N=10\,000$. The case $N=n$ corresponds to
applying the maximum-entropy method at the sample level; it can be observed
that with six moments it reproduces almost exactly the measured frequency
distribution. We discuss the case of unknown $N$ in \sect~\ref{sec:N}.
\begin{figure}[!p]
\centering
...
The figure shows that the distribution for the full-network is less flat
than the measured frequency distribution for the sample; their difference
increases with $N$. Most remarkably, for the first data set the
distribution has two distinct low-activity modes. For the second data
the distribution presents a small shoulder on the right of the
mode.% , suggestive of two activity regimes. We frame no hypotheses about the
% biological cause of these two modes (they could stem from the presence of
% different kinds of cells or modules)
These features are clearly not present in the sample or in the
maximum-entropy distribution at the sample level. The application of the
probability calculus thus reveals interesting possible features of the full
network. \textcolor{white}{If you find this you can claim a postcard from
  us.}


\subsection*{Use with moment constraints}
\label{sec:use_moments}
...
statistics, \fig~\ref{fig:plots_2_4mom} shows the full-network
distributions, for $N=10\,000$, obtained constraining the first two moments
(equivalent to constraining means and correlations) and the first four
moments. The frequency distribution of the sample is also shown for
comparison. In both data sets the two-moment constraint leads to a
distribution quite different from the four-moment constraint. For the first
data set in particular, one distribution is bimodal, the other unimodal.
This visual difference shows directly -- without the need to calculate
relative entropies, for example -- that the first two moments alone are
\emph{not} informationally sufficient for this specific data set.

It is instructive to contrast the conclusions about correlation sufficiency
reached through this full-network analysis, with those we would have
reached by applying the maximum-entropy method \emph{at the sample
  level}, as traditionally done. Such a comparison involves an important
methodological caveat, so we discuss it in full in the next section.
...
\enquote{synchrony} \citep[\eg][we're only citing the first papers using
these terms]{bohteetal2000,amarietal2003} of neuronal
activity. In this section we discuss how our proposed
application bears on this kind of quantification. % For concreteness we use an
...
Let us translate this question into more precise terms first, because
\enquote{cooperativity}, \enquote{interaction}, and similar notions are
vague. A translation in terms of \emph{informational sufficiency} seems to
suit all these notions \parentext{for the technical definition and
  intuitive understanding of sufficiency, which goes back to
  \cite{fisher1922}, see \eg\ \cite[\sect~4.5]{bernardoetal1994};
  \cite[\chap~8, \sect~14.2]{jaynes1994_r2003}; \cite{cifarellietal1982};
  \cite{kullbacketal1951}. For technical results and connections with the
  notion of symmetry see \eg\
  \cite{darmois1935,neyman1935,koopman1936,pitman1936,halmosetal1949,bahadur1954,berk1972,lauritzen1974,cifarellietal1980,cifarellietal1981,diaconisetal1981,lauritzen1982_r1988,diaconis1992,furmanczyketal1998,fortinietal2000,nogalesetal2000,kallenberg2005,lauritzen2007,ayetal2015}}.
The idea is as follows. We want to assess our probabilities about the
frequencies of the activities of the sample, or about the activity of the
sample in a new time bin. For this assessment we should use all measured
data and statistics we have. But it can be the case that dropping part of
the data or statistics -- for example, the measured third- and higher-order
moments -- leaves our probabilities almost unchanged. This means that the dropped
statistics are \emph{informationally irrelevant} or almost so. The
remaining statistics -- for example, first and second moments -- are then
\emph{informationally
  sufficient}. % This informational quasi-sufficiency is interesting because
% it may give clues about network organization and dynamics.

We can thus quantify how much a subset of statistics is relevant with
respect to a larger set by keeping only the subset in the conditional of
our probabilities, and checking how much the latter change. When our
probabilities are built with the maximum-entropy method, this means keeping
only part of the constraints (and re-maximizing the entropy). For example,
keeping only the first two moments (means and correlations) out of the
first four moments. This procedure is actually equivalent to comparing the
probabilities of our hypotheses about sufficiency, conditional the
\emph{full} data, assuming their pre-data probabilities to be equal. We
show this equivalence below and perform the calculations for our first data
set.
...
Before applying this idea to neuronal activity, however, it is important to
keep in mind that \emph{informational sufficiency is not preserved under
  sampling}: if a probability distribution has some sufficient statistics,
then its marginals \emph{cannot} have the same sufficient statistics, and
vice versa, except for trivial cases such as uniform probability
distributions. This generally also holds among different marginals: if a
statistics is sufficient for some marginal, then it will not be sufficient
for another marginal. This impossibility is known in statistical mechanics:
marginals of Gibbs states are not Gibbs states \citep{maesetal1999}.
Mathematically this impossibility comes from the Pitman-Koopman theorem
\citep{koopman1936,pitman1936,darmois1935} connecting sufficient statistics
and the exponential family, and translates into the general impossibility
of solving a system of $n$ independent equations in $m$ unknowns with more
equations than unknowns, $n>m$ \citep[\sect~3.1]{portamanaetal2015}.

This fact is important in the analysis of neural activity when it is based
on small samples from brain regions. If means and pairwise correlations
seem informationally sufficient for a particular sample from a brain area,
then they may well be \emph{insufficient} for the full network of neurons
constituting that area. Vice versa, if they are informationally sufficient
for the full network, then they may turn out \emph{not} to be sufficient
(or less so) for any recorded sample thereof. Hence, if we wonder about
\enquote{cooperativity} or \enquote{interaction} of a brain area, we cannot
reliably use maximum-entropy distributions \emph{for a sample only}. Yet
the neuroscientific literature is besprinkled with such sample-level
analyses
\citep[\eg][]{martignonetal1995,bohteetal2000,schneidmanetal2006,shlensetal2006,barreiroetal2010,ganmoretal2011,granotatedgietal2013}.
...
%
% relentropies of sample2mom, marg2mom, sample4mom, marg4mom:
% -322.72, -85.7627, -42.8005, -4.23746 nats =
% -1401.56, -372.462, -185.88, -18.403 db
% correspond to probability ratios of
% 1., 8.11718\times 10^{102}, 3.69524\times 10^{121}, 2.06704\times 10^{138}
%
% difference in relentropies between 4 mom and 2 mom:
% sample level: 279.92 nats = 1215.68 db
% marg level: 81.5252 nats =  354.059 db
% correspond to prob ratios of
% 3.69524\times 10^{121},2.5465\times 10^{35}
%
% comparison_ME_sample_full_levels.nb
%

 This procedure is equivalent to calculating and comparing the
probabilities of our hypotheses about sufficiency, conditional the data, if
their pre-data probability to be equal; so that for example
\begin{equation}
  \label{eq:compare_hypoth_conditional}
  \pf(\text{\small\enquote{first two moments are sufficient}} \| \text{\small data}) \propto
  \pf(\text{\small data} \|
  \text{\small\enquote{first two moments are sufficient}}).
\end{equation}
...
by marginalizing the full-network distribution to the sample size, from two
...
order of magnitude is enough for making qualitative inferences. As
discussed in the next section, even if the exact number were known, the
maximum-entropy distribution ought to be interpreted qualitatively.
...
distribution for the full network depend not only on the measured data in
...
\end{equation}
where $\re(\yF;\yR)\defd \sum_{\yA}\yFF_{\yA}\ln(\yFF_{\yA}/\yRR_{\yA})$ is
...
distribution of population-averaged activities of a network of neurons,
