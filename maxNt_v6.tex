...
%% Last-Updated: 2019-12-02T17:12:16+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
the probability $\pxx(\yF \| \yf )$ for the frequency distribution $\yF$
given the observed frequency distribution for the sample $\yf$. The result
is a mixture of terms involving the joint frequencies of total activities
of sample and larger population $J_{\ya,\yA}$, multiplied by the number of
time bins $T$:
\begin{equation}
  \label{eq:prob_joint_frequencies}
%\pxx[\yF \| \yf ] \propto
 \sum_{\bm{J}} \prod_{\ya,\yA}\bigl(\yG_{\ya\yA}\;\nu_{\yA}\bigr)^{T\,J_{\ya,\yA}}
  \qquad
  \text{with}\quad \tsum_{\yA}J_{\ya,\yA}=\yff_{\ya},
  \quad \tsum_{\ya}J_{\ya,\yA}=\yFF_{\yA},
\end{equation}
with weights $q(\ynu)$. The maximum-entropy method approximates $T$ with
infinity, thus treating the probability above as a delta. The result is
that
\begin{enumerate*}[label=(\roman*)]
\item the frequency distribution $\yF$ for the experiment and the long-run
  one $\ynu$ are constrained to be equal,
\item the frequency of the sample activity $\ya$ conditional on the
...
probability $\pxx(\ynu)$ in the expressions above.
