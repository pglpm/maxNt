...
%% Last-Updated: 2019-12-20T20:13:38+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
highlight some methodological caveats.% For concreteness we use an
...
precise, operational \citep[\cf][\chap~I]{bridgman1927_r1958} notion. Here
we propose the notion of \emph{informational sufficiency}, because
\begin{enumerate*}[label=(\roman*)]
...
\end{enumerate*}
Its meaning and the three points above will now be illustrated. For an
extensive discussion of this notion, probably first suggested by Kullback
\amp\ Leibler \parencite*{kullbacketal1951}, see Bernardo \etal\
...
what is the ratio of their probabilities? By translating our question about
\enquote{cooperativity} into a hypothesis-testing problem about sufficiency,
we can find a well-defined answer determined by the
probability-calculus. % The ratio of
...
Its most important consequence is that, by Bayes's formula, the probability
that some statistics are informationally sufficient can be approximated by
finding the maximum-entropy distribution constrained by those statistics
and then calculating the probability for the whole data under that
distribution. We shall shortly illustrate this kind of calculation for the
first data set.
...
The theorem above makes it very natural to reinterpret studies about
cooperativity, interaction, and similar, based on maximum-entropy, as
studies about informational sufficiency. An important fact must then be
taken into account: \emph{informational sufficiency is not preserved under
  sampling}. If a probability distribution has some sufficient statistics,
then its sample marginals \emph{cannot} have the same sufficient
statistics, and vice versa \citep[\sect~3.1]{portamanaetal2015}; except for
trivial cases such as uniform probability
distributions. % This generally holds also among
...
maximum-entropy distributions. For example, if means and pairwise correlations
seem informationally sufficient for a particular sample from a brain area,
then they are probably not sufficient for the larger population of neurons
constituting that area, and vice versa. If we are interested in the
...
We compare the informational sufficiency between of a set of moments, say
...
Let us illustrate the ideas, the remarks, and the quantity defined so far
with our first data set. Consider for example three sets of constraints
$M_{2}$, $M_{4}$, $M_{5}$, consisting of the first two, four, five moments;
and a larger population of size $N=10\,000$. Figure~\ref{fig:plots_2_4mom}
...
four; but four should almost as likely as five. We shall see that the
...
likely to be informationally sufficient.
...
\end{split}
\end{equation}
So the application at the sample level says that the data are 1\,220 orders
of magnitude more probable conditional on four moments than conditional on
two, and $10^{1.4}=25$ times more probable conditional on five moments than
on four moments. These results are different from the ones obtained from
the larger population, and they would be hard to deduce visually from a
look at the distributions of \fig~\ref{fig:comparisons_marginals}.
...
It is in fact possible to use the measure~\eqref{eq:measure_suffic} and its
probabilistic interpretation~\eqref{eq:bayes_factor} to compare the
probability of the data $\yf$ conditional on the hypothesis $(M_{4},N)$ of
four-moment sufficiency at the larger-population level, $N=10\,000$, versus
the hypothesis $(M_{4}, n)$ of four-moment sufficiency at the sample level,
$N=n=65$. We obtain
\begin{equation}
  \label{eq:diff_suff_n}
