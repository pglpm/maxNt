...
%% Last-Updated: 2019-11-25T17:35:14+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
population. This kind of isolation assumptions sometimes escape attention,
...
\mynote{[Yasser:] In the present work we aim to address the issue of building probability distributions over a large population using recording from subpopulations by considering the distribution of total population activity as an example. [}
...
population have been, given the observation of the activity of a very small
...
with which all levels of total activity of a larger population of 1\,000--10\,000
...
distribution for the full population has two very distinct modes, both at low
...
these guessed features of the full population cannot be inferred by the
application of maximum-entropy \emph{to the sample alone}. Although we don't
formulate any hypotheses on why the larger population could have two distinct
...
But when the difference in size between sample and full population is too
large, correlation sufficiency for the sample implies the \emph{lack} of
correlation sufficiency for the larger population, and vice versa. Therefore,
maximum-entropy applications at the sample level can be deceptive for
questions of statistical sufficiency, and a full-population application is
...
population had during the recording; it's only the most plausible. In high
...
time. These neurons are our \enquote{sample} or \enquote{subpopulation}. Their
...
time. We assume that there's an area, comprising a population of $N$ neurons,
...
actually recorded. We call this larger population the \enquote{full population}.
...
full population during and after the recording, and across sessions under the
...
\item\label{item:sample_step} Using sampling theory we determine the
  ...
  for the total activity $\yA$ of the full population.
...
relation between the first $m$ moments for the full population and the first
...
for any belief distribution $\pxxx(\yA_{t})$ for the full-population activity
...
analysis: \emph{the first $n$ normalized factorial moments for the sample
  and for the full population are numerically identical}:
\begin{equation}
...
\end{split}
\end{equation}
% where $\pxxx(\yA)$ is our distribution of belief about the full-population
% activity, and $\pxx$
...
$\E\bigl[\binom{\yA}{m}\bigr]$ of the full-population distribution
...
expectations for the full population are determined by those for the sample
...
\begin{equation*}
  ...
  \rightarrow
    \text{full-population moments}}_{\textit{sampling theory}}
\end{equation*}
and apply the maximum-entropy method to obtain a distribution
$\px(\yA \| M, N)$ for the full population of $N$ neurons by constraining some
...
full population. The magnitude of this discrepancy can be a signature of the
...
of four different distributions for the full population, which differ
...
% the full-population activity during the recording. But it also approximates
% three other distributions, as discussed in the previous section.

\bigskip

The full-population maximum-entropy distribution is calculated using five
...
(distribution${}\times N$) for three example values of full-population
...
The figure shows that the distribution for the full-population is more
...
(upper panel) the distribution for the full population has two distinct
...
These results show the potential usefulness of this maximum-entropy
approach. Here is a \emph{toy} example of possible uses. We could be
interested in the hypothesis that two distinct cell types or assemblies be
present in the region where the recording was made. Finding a
full-population distribution such as that in the upper panel of
\fig~\ref{fig:plots_5mom} would provide some evidence for this hypothesis.
Let's further imagine that we have reasons for suspecting that a specific
set of the sampled neurons are of one type, and the remaining of the other
type. In the case of data set 1, 27 of the 65 sampled neurons were
identified as grid cells belonging to 3--4 functional modules, see
\cite{dunnetal2015}. Could the two peaks in the distribution of
\fig~\ref{fig:plots_5mom} reflect the activities of grid versus non-grid
cells? We apply the method to these two sets of neurons individually, using
$N=4\,150$ for the grid set and $N=5\,850$ for the non-grid set, to reflect
their proportions in the recorded sample. The results are shown in the
upper panel of \fig~\ref{fig:plots_cellsubsets}. The distribution for the
larger population of grid cells (\textcolor{mygreen}{green triangles})
seems to have one, broad peak, close to the first peak of the
full-population distribution. The distribution for the larger population of
non-grid cells (\textcolor{myred}{red circles}) has two peaks instead,
roughly at the same positions as the full-population distribution
(\textcolor{myblue}{blue curve}) but closer in height. So it would seem
that the population of grid cells is contributing to the first peak of the
full population, but it isn't the sole contributor. We can also assess how
much the distributions for the two sets are independent. If they were
completely independent, the full-population distribution would be given by
their convolution:
\begin{equation}
  \label{eq:convolution}
  P_{\text{full}}(\yA) =  \smashoperator{\sum_{\yA'=0}^{\yA}} P_{\text{grid}}(\yA')\;
  P_{\text{non-grid}}(\yA-\yA'). %, \qquad \yA \in \set{1,\dotsc,10\,000}
\end{equation}
However, the lower panel of
figure~\ref{fig:plots_cellsubsets} shows that such convolution
(\textcolor{black}{black diamonds}) is different from the full-population
distribution (\textcolor{myblue}{blue curve}): the two peaks of the former
are closer in position and height. This means that the population
distributions of grid and non-grid cells are \emph{not} independent:
knowledge of the activity of either set gives us some information about the
activity of the other.

\medskip

The toy analysis just given should  be taken not literally, but just
as an illustration of the method's possible applications. The important point
is that this method is computationally very cheap but can provide useful
...
\fig~\ref{fig:plots_2_4mom} shows the full-population distributions, for
...
brain area, then they may well not be sufficient for the full population of
...
full population of size $N=10\,000$, we obtain the following differences:
\begin{equation}
...
The application at the full-population level therefore leads to different
...
the full-population level, $N=10\,000$, versus the hypothesis $M_{5, n}$ of
...
that we reach by applying the maximum-entropy method to the full population,
...
a full population with $N=10\,000$ and the same constraints, that is, the
...
full-population distribution fits the measured frequencies
...
% But does the application at the full-population level lead to appreciably
...
% maximum-entropy distributions for a full population of $10\,000$ neurons,
...
% by marginalizing the full-population distribution to the sample size, from two
...
% \begin{itemize}
% \item The application to the full-population (upper plot) leads to two
...
% \item The difference between marginals from the full-population distribution
...
% \item The marginals of the application to the full population are closer to
...
% full-population level lead to different results; the latter is not only more
...
\section{The full-population size $N$}
\label{sec:N}
...
on the size of the full population, $N$. The full population can't be the whole
...
moments of the full population and those of the sample. This formula is only
valid if our beliefs about the activity of the sample $\pxx(\ya)$ and that
of the full population $\pxx(\yA)$ are related by a hypergeometric
distribution:
\begin{equation}
...
distribution for the full population depend not only on the measured data in
...
full population constrained by a set of factorial moments. For further details
...
\end{equation}
In our case the units are neurons, the set is the full population, and the
...
factorial moments of sample and full population.
...
for the total activity $S(t)$ of the full population of $N$ neurons that
...
\emph{from empirical statistics of the activity of a \textbf{sample} of
  ...
  activity of the \textbf{full population} of neurons that could have been
...
\textbf{The frequency distribution $\yF$ of the full-population activity
...
\textbf{The long-run frequency distribution $\ynu$ of the full-population
  activity}
...
\textbf{The activity $\yA'$ of the full-population
...
minimum size of the full population for which such modes can be observed. We
then compared the distribution found by marginalizing the full population
...
THAT IF THE FULL POPULATION GETS BIGGER THE DIFFERENCE ALSO GET BIGGER)?…

The rest of the paper is organized as follows. We first describe how to go
from the sample level maximum entropy model to the full population maximum
