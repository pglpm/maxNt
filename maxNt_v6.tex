...
%% Last-Updated: 2020-01-13T15:39:05+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
$(\ya_{t})\equiv (\ya_{1}, \dotso, \ya_{T})$ we can count how often the
activity levels $\ya=0$, $\ya=1$, and so on appeared during the recording,
obtaining the distribution of measured relative frequencies
$(\yff_{\ya}) \equiv (\yff_{0},\dotso, \yff_{n}) \defs \yf$. We can also
...
the recording were
$(\yFF_{\yA}) \equiv (\yFF_{0},\dotso,\yFF_{N}) \defs \yF$. The values
...
Our question is whether some amount of \enquote{cooperation} is more
\enquote{important} than another amount or not. In terms of informational
sufficiency, its definition might suggest to compare the belief
distributions conditional on two different subsets of statistics. But which
measure should be used for such a comparison? and how to motivate it? We
prefer to take a different approach, leading to an automatic answer: let us
thus turn the informational sufficiencies of two different subsets of
...
all neurons which we believe -- with equal degree of belief for each neuron
-- could have been recorded. Then $N$ is the size of that pool. $N$
...
homogeneity of the brain region where the recording is made: if we believe
that it does not matter whether the probe had been inserted in some other
...
Whether this symmetry in our beliefs is justified, or for which sampling
procedures it can be justified, is the fundamental, deep question of
sampling theory, which we cannot discuss here \citep[see \eg\ the
