...
%% Last-Updated: 2019-11-22T15:47:19+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
The full-network maximum-entropy distribution is calculated using five
moments. This number already provides almost as much information as the
full frequency distribution of the sample (see next section).
Figure~\ref{fig:plots_5mom} shows the resulting densities
(distribution${}\times N$) for three example values of full-network sizes:
$N=1\,000$, $N=5\,000$, $N=10\,000$. The frequency distribution of the
sample activity is also shown, and in the plot it would be indistinguishable from
the maximum-entropy distribution for $N=n$, that is, applied at the sample
level. We discuss the case of unknown $N$ in \sect~\ref{sec:N}.
\begin{figure}[!p]
\centering
...
distribution has two distinct low-activity modes. For the second data set
...
These features are clearly not present in the sample frequencies or in the
...
approach. For example, some researchers could be interested in the
hypothesis that two distinct cell types or assemblies be present in the
region where the recording was made. Finding a full-network distribution
such as that in the upper panel of \fig~\ref{fig:plots_5mom} would provide
some evidence for this hypothesis. If the researchers also have reasons to
suspect that a specific set of the sampled neurons are of one type, and the
remaining of the other type, they could further apply the method to the two
sets separately. Finding two distributions with a single peak each would
then provide some evidence that their separation was correct. In the case
of data set 1, 27--29 sampled neurons were identified as grid cells
belonging to 3--4 functional modules, see \cite{dunnetal2015}. The two
peaks in the distribution obtained with our approach could reflect the grid
vs non-grid distinction, or two groups of modules, or a combination of
these cases.
...
\fig~\ref{fig:plots_2_4mom} shows the full-network distributions, for
...
as traditionally done. Such a comparison involves an important
