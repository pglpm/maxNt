...
%% Last-Updated: 2020-04-26T13:23:00+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\firstpublished}{4 November 2015}
...
\paragraphfootnotes\setlength{\footmarkwidth}{1em}
%\twocolumnfootnotes
...
\parbox{0.4\linewidth}%\makebox[0pt][c]%
{\protect\centering E. Torre\\%
\footnotesize\epost{\email{emiliano.torre}{yahoo.com}}}%
\hspace*{\stretch{1}}\\%
...
model for the total activity of a large population of unrecorded neurons,
given only some activity data or statistics recorded from a small sample
thereof. The model addresses only the collective behaviour of the larger
population, thus gaining in simplicity and computational effectiveness at
the expense of more detailed inferential power.

The model is applied to two sets of neuronal data from the literature. It
makes interesting forecasts about the larger population, such as the
probable presence of two low-regime modes in the total activity, which are
not present in the sample data and could not be found with maximum-entropy
models applied only to the sample.

Using the same data sets it is shown how to use the model for questions
about the \enquote{cooperativity}, \enquote{interaction}, or
\enquote{synchrony} of neuronal activity. These questions are translated
into a hypothesis-testing problem about informational sufficiency, and
effectively answered with Bayes's theorem. The answers given by the present
model can be at variance from those given by a maximum-entropy model at the
sample level only. The reason is discussed: informational sufficiency is not
preserved under sampling.

The question of how large the larger population addressed by model should
or can be, and the assumptions on which the model is based, from a Bayesian
point of view, are also discussed.
...
such initial probability distributions are called \enquote{(probabilistic)
  models}; and that is how we intend this word in the present work.
...
that constitute a functionally distinct brain region. Many probabilistic
models focus on such samples only: they neglect that the recorded neurons
are a sample from a larger population. Some probabilistic models try to
...
investigations, to help us decide which hypotheses to discard, which to
consider for a deeper analysis with more complex and costly models, or to
...
\citep[\cf][\sect~3]{portamanaetal2015}. It answers this question: how much was
...
%[see also][for applications to irreversible statistical mechanics]{grandy1980}
to build initial probability distributions has been used in the
neurosciences for several kinds of investigations about the neuronal
activity of various brain areas and about other phenomena of importance % ,
% for example gene and protein interaction
\citep[]{martignonetal1995,bohteetal2000,shlensetal2006,schneidmanetal2006,tkaciketal2006,roudietal2009c,barreiroetal2010,shimazakietal2015,moraetal2015%,lezonetal2006,weigtetal2009
}[for
...
\citey{iso2006,iso2006b,jcgm1993_r2008} standards and Jaynes
...
$T$ time bins, and a binary value 1 is assigned to each bin containing at
least one spike, 0 to the bins without spikes. Call $\ya_{t}$ the number of
neurons that fire during time bin $t$: this is the \emph{total activity} of
the sample, or just \enquote{activity} for short. Obviously
$\ya_{t} \in\set{0,1,\dotsc,n}$; if $\ya_{t}=0$, no neuron spikes during
bin $t$; if $\ya_{t}=n$, all spike (at least once) at some point during bin
$t$, and so on. For brevity we say \enquote{at $t$} for \enquote{during
  time bin
...
% ranging from $0$ to $1$ in $1/n$ steps.
From the sequence of activities
$(\ya_{t})\equiv (\ya_{1}, \dotso, \ya_{T})$ we can count in how many bins
each activity level $\ya=0$, $\ya=1$, \ldots, $\ya=n$ appeared during the
recording. Dividing each number of bins by the total number of bins $T$ we
obtain the distribution of measured relative frequencies
$(\yff_{0},\dotso, \yff_{n}) \equiv (\yff_{\ya}) \defs \yf$. We can also
consider the (unknown) sample activity at time bins \emph{outside} of the
recorded period.
...
sessions that are very far apart in time. We assume that there's a volume,
...
Meanings~8 and~9 of][]{kruskaletal1979c}[note that the
standard][\sect~3.1.14 states \enquote{The notion of representative sample
  is fraught with controversy, with some survey practitioners rejecting the
  term altogether}]{iso2014}.

The total activity of the larger population of $N$ neurons at $t$ is
$\yA_{t}$. The relative frequencies of the various activity levels during
the recording were
$(\yFF_{0},\dotso,\yFF_{N}) \equiv (\yFF_{\yA}) \defs \yF$. The values
$\yA_{t}$ at each $t$ and the frequency distribution $\yF$ are unknown to
us. We only know for certain that $\yA_{t} \in \set{0, 1, \dotsc, N}$, that
$\yA_{t} \ge \ya_{t}$, and that $N-\yA_{t} \ge n-\ya_{t}$ for obvious
reasons. For the time being we assume that we know $N$; in
\sect~\ref{sec:N} we discuss the consequences of our lack of precise
...
\begin{equation}
  ...
  \mathrel{\;\;\underset{\mathclap{\overset{\Bigg\uparrow}{\text{our judgement}}}}{=}\;\;}
...
\end{gathered}
\end{equation}
where the $m'$ parameters
$\yll \defd (\yl_{m}) \equiv(\yl_{1},\dotso,\yl_{m'})$ are determined by
the constraints~\eqref{eq:final_constraint} and can be found by convex
...
increases. The most accurate interpretation is~\ref{item:prob_freq}, and that
...
\item The \textbf{second data set}, from \textcite[supplementary
  material]{rostamietal2016_r2017}, %\texttt{data\_RS.npy}
...
(small \textcolor{myblue}{blue squares}). The frequency density of the sample
...
\caption{Maximum-entropy distributions for the total activity of the larger
  population, using $M={}$five moments and assuming several population sizes $N$.
  ...
  population average). The distributions for large $N$ have very different
  features from the measured frequency distribution, and how their peaks
  become more pronounced as $N$ increases.}
...
\fig~\ref{fig:plots_5mom_b}, the distribution presents a small shoulder
(barely visible) on the right of its mode.
...
In the case of the first data set, indeed, 27 of the 65 sampled cells were
...
\caption{\subcaptionref{fig:plots_cellsubsets_a} Maximum-entropy
  ...
  \subcaptionref{fig:plots_cellsubsets_b} The convolution of the two
  subpopulation distributions is very different from the distribution for
  all cells; this means that the two subpopulation distributions are not
  independent.}
...
(small \textcolor{myblue}{blue squares}) but closer in height. It would thus seem
...
larger-population distribution (small \textcolor{myblue}{blue squares}): the two
...
illustration of the model's possible applications. Instead of a grid cell
\vs\ non-grid cell divisions we could for example have examined the modules
of the cells. In the absence of such auxiliary information about the cells,
one could in principle try all possible partitions of the recorded cells
into two groups, and check whether some partition leads to independent
distributions (computationally very expensive in practice); and so on.

The important point is that this model is computationally very cheap and
yet it can provide useful insights, even if just qualitative ones, and even
suggest new hypotheses.
...
level. %We also highlight some methodological caveats.% For concreteness we use an
...
\citep[\cf][\chap~I]{bridgman1927_r1958} notion. Here we propose to base
this kind of analysis on the notion
...
data and statistics we have collected. This belief distribution, however,
can be such that it remains unchanged if we discard part of the data or
statistics -- for example, the third- and higher-order empirical
moments. % --
% but it does change if we discard more.
...
\emph{informationally sufficient}.%  The qualification \enquote{minimally}
...
It seems to us that the informational sufficiency of a set of $m$ moments
captures the idea of the \enquote{importance} of the almost-simultaneous
activity of $m$ or fewer neurons: such simultaneous activity is
\enquote{unimportant} if its knowledge does not alter our forecasts. We are
thus speaking of informational importance, which may or may not imply or be
implied by biophysical importance -- \enquote{correlation is no proof of
  causation} \citep[\eg][]{simon1954,freedman2006}. But it lends itself to a
precise probabilistic analysis, with consequences determined by
mathematical theorems, as we shall now see.

Our question is whether some degree of \enquote{cooperation} is more
\enquote{important} than some other degree, or not. In terms of
informational sufficiency, its definition might suggest to compare the
belief distributions conditional on two different subsets of statistics.
But which measure should be used for such a comparison? and how to motivate
it? We prefer to take a different approach, leading to an automatic answer:
let us turn the informational sufficiencies of two different subsets of
statistics into two \emph{hypotheses}. We can then ask: which hypothesis is
more probable, given the data? Equivalently, what is the ratio of their
...
well-defined answer determined by the probability-calculus. % The ratio of
% the two probabilities gives us a natural measure of the
% For example, do
% the data say that the first two empirical moments are probably enough to
% determine our beliefs? or is it more probable that the first four are
% needed instead?
We shall see that this approach is indeed more powerful than a simple
...
% Thus, maximum-entropy distributions automatically appear (as
% approximations) from the probability-calculus when we test hypotheses
% about informational sufficiency.
...
% For probabilities built with the maximum-entropy method, this means
% comparing those constrained on the subset and on the full set. This
% procedure is actually equivalent to comparing the probabilities of the two
% hypotheses about sufficiency, conditional on the \emph{full} data, assuming
% their pre-data probabilities to be equal. We show this equivalence below
% and perform the calculations for our first data set. \medskip
...
(\eg\ uniform probability distributions). % This generally holds also among
...
therein]{maesetal1999}. % Mathematically this impossibility comes from the
% Pitman-Koopman theorem mentioned above, and translates into the general
% impossibility of solving a system of independent equations with more
...
We can compare the informational sufficiency of a set of moments, say
...
therein]{kassetal1995}. %[\chaps~V, VI, A]{jeffreys1939_r1983}.
...
about sufficiency and sampling by using our data sets. We want to compare,
for example, three sets of constraints $M_{2}$, $M_{4}$, $M_{5}$,
consisting of the first two, four, five moments, for a larger population of
size $N=10\,000$. Figure~\ref{fig:plots_2_4mom} shows the resulting
distributions for the two data sets.
\begin{figure}[!p]
\centering
\subcaption[]{\label{fig:plots_2_4mom_a}}%  
\includegraphics[width=\linewidth]{_scripts/newpaper2_stensola_threemom_distributions10000_priorc.pdf}%
\\[1em]%
\subcaption[]{\label{fig:plots_2_4mom_b}}%  
\includegraphics[width=\linewidth]{_scripts/newpaper2_riehle_threemom_distributions10000_prior2.pdf}%
\\[1em]%
\caption{Maximum-entropy distributions for the total activity of the larger
  population, based on the first two, four, and five moments, and a
  larger-population size $N=10\,000$. The two- and four-moment
  distributions are very different, suggesting that two moments are not
  informationally sufficient. The four- and five-moment distributions are
  almost indistinguishable, suggesting that four and five moments are
...
\subcaptionref{fig:plots_2_4mom_a}, they even differ in the number of
...
%** sample level: 279.92 nats = 403.839 bit = 1215.68 dB
%** marg level: 81.525 nats =  117.616 bit = 354.06 dB
% difference in relentropies between 5 mom and 4 mom:
%** sample level: 3.30882 nats = 4.77362 bit = 14.37 dB
%** marg level: 0.0366623 nats = 0.0543836 bit = 0.159223 dB
% correspond to prob ratios of
% sample: 3.69524\times 10^{121}, 27.3529
% marg: 2.5469\times 10^{35}, 1.03734
% 
% comparison N vs n:
% 2 mom: 236.958 nats = 341.858 bit = 1029.09 dB, 8.11758\times 10^{102}
% 4 mom: 38.5632 nats = 55.635 bit = 167.478 dB, 5.59496*10^16
% 5 mom: 35.2911 nats = 50.9157 bit = 153.267 dB, 2.12185*10^15
%
...
\begin{equation}
  ...
  \begin{split}
  \ydi_{N}(M_{4},M_{2}) &= 118\;\bit \equiv 35.4\;\hart,
\\    \ydi_{N}(M_{5},M_{4}) &= 0.0544\;\bit \equiv 0.0159\;\hart,
  \end{split}
\end{equation}
...
see][]{good1985%,good1950,good1969
...
\begin{equation}
  \label{eq:diff_suff_n}
  \begin{split}
  \ydi_{n}(M_{4},M_{2}) &= 404\;\bit \equiv 122\;\hart,
\\    \ydi_{n}(M_{5},M_{4}) &= 4.77\;\bit \equiv 1.44\;\hart.
\end{split}
\end{equation}
...
% \\[3em]%
\includegraphics[width=0.92\linewidth]{_scripts/trunct2_comparison_marginals.pdf}%
\caption{Maximum-entropy distributions constructed at the sample level,
  using the first two, four, and five moments; and sample marginal of the
  distribution constructed with five moments for a larger population with
  $N=10\,000$ (dotted yellow in \ref{fig:plots_2_4mom_a}). The
  distributions are multiplied by $T$, obtaining absolute frequencies; note
  that activities with frequency lower than 1 cannot be observed in $T$
  bins. The distributions at the sample level are only distinguishable on a
  logarithmic scale. The five-moment distribution constructed at the
  larger-population level fits the frequency data better than the one at
  the sample level.}
...
five-moment sufficiency at the larger-population level, $N=10\,000$, versus
the hypothesis $(M_{5}, n)$ of five-moment sufficiency at the sample level,
$N=n=65$. We obtain
\begin{equation}
  \label{eq:diff_suff_n}
  \ydi[(M_{5}, N), (M_{5}, n)] = 50.9\;\bit \equiv 15.3\;\hart.
\end{equation}
...
\fig~\ref{fig:comparisons_marginals}. We see that the five-moment
distribution constructed at the larger-population level
(\textcolor{myyellow}{yellow stars}) fits the measured frequency
distribution (black triangles) more closely than the corresponding
sample-level distribution (\textcolor{myyellow}{yellow lozenges}).



\bigskip

% We close this section with some references. The notion of informational or
% predictive sufficiency, probably first presented by Kullback \amp\ Leibler
% \parencite*{kullbacketal1951}, is discussed in Bernardo \etal\
% \parencite*[\sect~4.5]{bernardoetal1994}, who explain its connections with
% Fisher's\parencite*{fisher1922} parametric sufficiency. Jaynes
% \parencite*[\chap~8 and \sect~14.2] shows the connections with information
% theory. The mathematical relation between sufficiency and the exponential
% family comes from the Pitman-Koopman
% theorem \citep{koopman1936,pitman1936,darmois1935} for continuous variables.
...
% theorem\citep{kullbacketal1951}[the
...
The calculations presented in the preceding sections depend on the size $N$
of the larger population, which of course cannot be the whole brain. How
large should or can $N$ be in our formulae?

The answer comes by examining the origin of our belief distribution for the
activity of the sample \emph{conditional} on the activity of the larger
population. It is given by the hypergeometric
distribution~\eqref{eq:conditional_hypergeometric}, reprinted here:
\begin{equation}
...
conditional probability, characteristic of \enquote{drawing without
  replacement}
\citep[\eg][\chap~3]{jaynes1994_r2003}, % [\sect~4.8.3]{ross1976_r2010}[\sect~II.6]{feller1950_r1968}
...
all neurons which we believe -- with an \emph{equal} degree of belief for
each neuron -- could have been recorded. Then $N$ is the size of that pool.
$N$ therefore depends on factors such as: the shape, dimensions, and
technical specifications of the recording probe; the inaccuracy in the
insertion of the probe, leading for example to slightly different insertion
points or angles; the density of neurons around the probe. But it also
depends on the homogeneity of the brain region where the recording is made:
if we believe that it does not matter whether the probe had been inserted
in some other point of the same brain region, then
formula~\eqref{eq:conditional_hypergeometric} is appropriate. $N$ can
therefore only be assessed case by case.

Whether this equality in our degrees of belief is justified, or for which
sampling procedures it can be justified, is the fundamental, deep question
of sampling theory, which we cannot discuss here \citep[see \eg\ the
discussions and references
in][]{ericson1969,smith1976}. % {kruskaletal1979c,kruskaletal1980}
It is of course possible to probabilistically assess, case by case, whether
this symmetry assumption is appropriate. But we must also be aware that
such assessment in turn rests on analogous symmetry assumptions at a higher
level, and so on. The probability-calculus, just like the truth-calculus,
cannot yield conclusions without premisses
\citep{hailperin2011}[p.~182]{johnson1924}.

The plots of \fig~\ref{fig:plots_5mom} nevertheless suggest that the order
of magnitude of $N$ ought to be enough for qualitative inferences. As
...
\begin{equation}
  ...
  \pf(\yf \| M, N) \approx \binom{T}{T\yf}\;\prod_{a}\pxx(\ya \| M, N)^{T\,\yff_{a}}
...
uncertainty spans slightly more than one order of magnitude, from $N=1\,000$
to $N=20\,000$. Divide this range roughly into thirds of order of
magnitude, considering the values
$N\in \set{10^{3}=1\,000, 10^{3+1/3}\approx 2\,000, 10^{3+2/3}\approx
  5\,000, 10^{4}=10\,000, 10^{4+1/3}\approx 20\,000}$. Assuming the set $M$
...
% a set of moments, or about the marginal sample distribution --
that take into account our uncertainty about $N$. We must make sure to
avoid circularities: for example, we cannot assume a set of moments
...
approximations all in all. They become clear within a full-fledged
...
is to relate the probability for the frequency distribution $\yF$ for the
activity of the larger population during the recording, to the probability
for the \emph{long-run} frequency distribution. In statistical physics, as
a comparison, the maximum-entropy method is instead used with the
assumption of permutation-invariance across \emph{repetitions} of the same
experiment, because its constraints represent macroscopically
\emph{reproducible} quantities and kinematics, even in non-equilibrium
\citep{jaynes1985b_r1996,%jaynes1957,%tikochinskyetal1984,
  grandy1988,%berryetal1988,
  deroecketal2005_r2006%,touchette2009
}[for experiments \eg][]{guntonetal1983}. The kinetic-Ising models
mentioned in \sect~\ref{sec:intro} are closer to this latter approach.

The second assumption, typical of maximum-entropy in general, concerns the
probability distribution for the long-run frequencies ensuing from the
first assumption \citep[and references therein]{portamana2017}. It is taken
to be either an \enquote{entropic prior}
\citep{skillingetal1984,rodriguez1991,neumann2007}, which takes into
account the multiplicity of long-run frequencies, or a prior for a model
with sufficient statistics, which expresses the sufficiency of the moments
under study, according to the theorem discussed in
\sect~\ref{sec:marginalization}. This assumption is crucial for the
maximum-entropy method based on the Shannon entropy. Alternative prior
densities lead to alternative maximum-entropy methods; for example one
based on the Burg entropy \citep{burg1975} if a Dirichlet density is used
...
prior's peak. The computations of \sects~\ref{sec:application}
and~\ref{sec:marginalization} were insensitive to the choice of the
reference distribution in their significant digits. We used three
...
allows us to factorize the joint probability of the sequence of activities
and apply the sampling relation~\eqref{eq:conditional_hypergeometric} of
...
the ratio $T/L$ to infinity (that is, $T\gg L \gg 1$). These approximations
lead to the identification of the four distributions listed at the end of
...
distribution~\eqref{eq:conditional_hypergeometric} and the \emph{conditional}
frequencies of the sample activity $\ya$ given $\yA$. The goodness of this
approximation decreases as the ratio -- roughly $nN/T$ -- between the
number of possible joint states and the number of bins increases, and also
as observed quantities approach their mathematical bounds, for example some
...
distribution for the frequencies $\yF$, that is, the most probable
recorded frequency.
...
of correlations of some order, as discussed in
\sect~\ref{sec:marginalization}, to be compared with hypotheses about more
realistic time dependences and
clustering. %They may turn out to approximate well more realistic assumptions.

The $T= \infty$ approximation (together with the third assumption above)
can break our model for specific distributions of sample frequencies, if
too many moments are used. For example, take $n=2$ with frequency
distribution $\yf=(0.5,0,0.5)$ for the activities $\ya=0,1,2$; that is, the
sample is silent half of the time and completely active half of the time.
The first two normalized factorial moments have both value $0.5$, and
together they uniquely determine $\yf$. If we now take $N=3$, it can be
proved that \eqn~\eqref{eq:app_maxent_pop_first}, constrained on those two
moments, has no solution. The reason is clear from a sampling perspective:
if there is zero probability of sampling one active neuron, then the larger
population cannot have any active neurons at all, and a fortiori there must
be zero probability of sampling two active ones. But this reasoning holds
only for each single time bin, not for the averages over time bins. The
$T=\infty$ approximation identifies the two instead. Outside of this
approximation there are no contradictions because the conditional
frequencies and conditional probabilities are kept distinct.

The two infinity approximations are at the edge of their validity range, in
our examples. For this reason we interpret the maximum-entropy distribution
as \emph{the most probable frequency distributions}, as mentioned at the
end of \sect~\ref{sec:model}. In high dimensions, however, the features of
the most plausible distribution may not be \emph{typical} of the majority
of plausible distributions. An important question, then, is \emph{which
  features of the maximum-entropy frequency distribution are typical of the
  majority of plausible frequency distributions?} -- especially for
features such as the number, position, and heights of modes; or tail
behaviour. Preliminary studies using a full probability calculation (in
preparation, to be published separately) indicate that several
semi-quantitative features of the maximum-entropy distribution are indeed
typical. For the first data set, for example, most frequency distributions
have a high-frequency regime at a population-averaged activity of around
0.075, as in \fig~\ref{fig:plots_5mom_a}, but their second high-frequency
regime is typically lower and appears for $N>5\,000$. Most frequency
distributions have two or three very close modes within each high-frequency
regime, rather than one. The same preliminary studies also indicate that
these typical features are insensitive to alternative prior choices in the
second assumption above, for example using a Dirichlet prior instead of an
entropic one.

This suggests that the maximum-entropy approximation presented here can be
favourably used to get a qualitative or semi-quantitative idea of what a
fully probabilistic calculation could yield -- but with a far lower
computational cost than the latter. For example, for $N=1\,000$ the
maximum-entropy distribution can be calculated in ten minutes on a laptop,
whereas the full probability calculation takes several days on a
...
limited inferential scope makes the model computationally very cheap and,
most important, it still leads to inferences that are not at all trivial
and could not be made just by looking at the sample and its distributions.
We showed this by applying the model to two real data sets.
...
itself, discussed in \sect~\ref{sec:assumptions}, can either be under test
or relied upon, depending on the case.
...
% be off by more than a few percentages for the population-averaged
% activity and for the frequency density.
This imprecision, however, is compensated by the
...
\begin{acknowledgements}
  ...
  of Excellence scheme, Yasser Roudi group, of the  Research Council of Norway.
  
  We thank Yasser Roudi and Iv\'an Davidovich for very insightful and valuable comments on 
  previous drafts of this manuscript. PGLPM thanks the staff of the NTNU
  library for their always prompt support; Mari, Miri, Emma for
  continuous encouragement and affection; Buster Keaton and Saitama for
  filling life with awe and inspiration; the developers and maintainers of
  \LaTeX, Emacs, AUC\TeX, Open Science Framework, R, Python, Inkscape, Sci-Hub
  for making a free and unfiltered scientific exchange possible.
...
specific time bin), and we sample $n$ of the $N$ units in such a way
(partly uncontrollable experimental procedure) that we are led to believe,
with the same degree, that any of the $N$ units could have been sampled.
...
\citep[\eg][\chap~3]{jaynes1994_r2003}. %[\sect~4.8.3]{ross1976_r2010}[\sect~II.6]{feller1950_r1968}[\sects~2.1, 3.2]{jeffreys1939_r1983}.
...
\citep%[\chaps~I--IV]{whitworth1867_r1965}[\chap~II]{feller1950_r1968}
...
\end{equation}
The probability of observing a specific sequence $(\ya_{t})$ with
frequencies $\yf$ is therefore
\begin{equation}
...
\citep[\chap~4]{bernardoetal1994_r2000}{dawid2013%cifarellietal1982
...
exponential. If we want to condition not on $\yf$ but on a set of measured
averages $C(m) = \sum_{\ya}c_{\ya}(m) \yff_{\ya}$, $m\in M$, such
