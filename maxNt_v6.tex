...
%% Last-Updated: 2019-12-02T15:04:31+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
formula~\eqref{eq:conditional_hypergeometric} at each bin:
\begin{equation}
...
From these two assumptions the probability calculus allows us to calculate
the probability for the frequency distribution $\yF$ given the observed
frequency distribution for the sample $\yf$. The result is a mixture of
terms involving the joint frequencies of total activities of sample and
larger population $J_{\ya,\yA}$, multiplied by the number of time bins $T$:
\begin{equation}
  \label{eq:prob_joint_frequencies}
%\pxx[\yF \| \yf ] \propto
\prod_{\ya,\yA}\bigl(\yG_{\ya\yA}\;\nu_{\yA}\bigr)^{T\,J_{\ya,\yA}},
\end{equation}
with weights $q(\ynu)$, and with the marginal of the joint frequency
constrained Conditioning on the frequencies $\yf$ of the sample
would correspond to constraining the marginal
$\sum_{\yA}F_{\ya,\yA}=\yff_{\ya}$. The maximum-entropy method approximates
$T$ with infinity, thus treating the probability above as a delta. The
result is that
\begin{enumerate*}[label=(\roman*)]
