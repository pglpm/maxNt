...
%% Last-Updated: 2019-11-17T16:12:37+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
hypotheses that a researcher is exploring; for example, hypotheses about
the \enquote{cooperativity} or \enquote{interaction} -- for example,
pairwise or higher-order -- of the network activity. We discuss this kind
of use in
...
\emph{informationally sufficient}. \parentext{For more technical results
...
and the exponential family of distributions, and translates into the
general impossibility of solving a system of $n$ independent equations in
$m$ unknowns with more equations than unknowns, $n>m$
...
brain area, then they may well not be sufficient for the full network of
...
maximum-entropy method is applied to obtain the distribution of the full
network, not of the sample alone.
...
\begin{enumerate}[label=(\roman*)]
\item from each maximum-entropy distributions $\px(\yA \| M)$ for the full
  network, built from each set of constraints $M=M',M''$, calculate the
...
The measure $\ydi(M'',M')$ so defined is positive if $M''$ is \enquote{more
  informationally sufficient} than $M'$.

Why is this a natural measure? Because $\ydi(M'',M')$ is equal to the
log-ratio of the likelihoods of the hypotheses $M''$ and $M'$ in view of
the data $\yf$:
\begin{equation}
...
this in appendix**. So the exponential (same base) of $\ydi(M'',M')$ tells
us how much more probable the data $\yf$ are, in view of $M''$ vs $M'$. If
we also introduce the pre-data probabilities of the two hypotheses we
obtain their probabilities conditional on the data.
...
Consider the three sets of constraints $M_{2}$, $M_{4}$, $M_{5}$ consisting
in the first two, four, five moments. For a full network of size
$N=10\,000$, we obtain the following differences:
\begin{equation}
...
\citep[\sect~C.4]{iso2009}. In words, the measured frequencies of the
sample activity are 35 orders of magnitude more probable under sufficiency
of the first four moments are sufficient, than under sufficiency of the
first two only; but they are about equally probable ($10^{0.016} = 1.04$)
whether we consider the first five moments to be sufficient or just the
first four.
...
on four moments as opposed to two, and $10^{1.4}=25$ times more probable
conditional on five moments as opposed to five.

The application at the full-network level therefore leads to different
...
meaninig~\eqref{eq:bayes_factor} to compare the probability of the data
$\yf$ conditional on the hypothesis $M_{5, N}$ of 5-moment sufficiency at
the full-network level, $N=10\,000$, versus the hypothesis $M_{5, n}$ of
5-moment sufficiency at the sample level, $N=n=65$. We obtain
\begin{equation}
  \label{eq:diff_suff_n}
  \ydi(M_{5, N}, M_{5, n}) = 35\;\nat = 15\;\hart,
\end{equation}
that is, the data are 15 orders of magnitude more likely under the first
hypothesis than under the second.

\bigskip

The conclusions as regards \enquote{cooperativity} or \enquote{interaction}
that we reach by applying the maximum-entropy method to the full network,
...
a full network with $N=10\,000$ and the same constraints, that is, the
...
full-network distribution fits the measured frequencies very well if we
only keep four moments.
...
bin is the sample marginal of the maximum-entropy distribution for the full
network, owing to the excheangeability assumption implicit in the
...
\caption{Log-density plot of the hypergeometric distribution
  $%\pf(\ya \|\yA, \yH)=
\yG_{\ya\yA} \defd  \raisebox{0pt}[0pt][1ex]{}\binom{n}{n\ya}\binom{\yNv-n}{\yNv \yA-n\ya}\binom{\yNv}{\yNv \yA}^{-1}$ for $\yNv=5000$, $n=200$. (Band artifacts may appear in the
...
equalities express the intuitive fact that \emph{our uncertainty about the
  ...
  about other samples}, and vice versa. When $n=\yNv$, $\yG_{\ya\yA}$
...
\begin{multline}
  ...
  \sum_{\yA} \yG^{(\yNv)}_{\ya\yA}
\,
  \tbinom{\yNv}{\yNv \yA}\,\exp\Bigl[{\yll_\yNv}\T
  \tsum_{\ya} \yg_{\ya}\yG^{(\yNv)}_{\ya\yA}\Bigr]\biggr\}
  \;h(\yNv),
\end{multline}
