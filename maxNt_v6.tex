...
%% Last-Updated: 2019-11-27T11:50:53+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\end{equation}
which leads to the equality of factorial
moments~\eqref{eq:equal_m_factorialmoments}, on which our approach rests.
The conditional probability above is characteristic of \enquote{drawing
  without replacement}
\citep[\chap~3]{jaynes1994_r2003}[\sect~4.8.3]{ross1976_r2010}[\sect~II.6]{feller1950_r1968}.
In other words, consider the pool of all neurons which we believe -- with
equal degree for each neuron -- could have been recorded. $N$ is the size
of that pool. $N$ therefore depends on factors such as: the shape,
dimensions, and technical specifications of the recording probe; the
inaccuracy in the insertion of the probe, leading for example to slightly
different insertion points or angles; the density of neurons around the
probe. But it also depends on the homogeneity of the brain region where the
recording was made: if we believe that it doesn't matter whether the probe
had been inserted in some other point of the same brain region, then
formula~\eqref{eq:conditional_hypergeometric} is appropriate. Thus $N$ can
only be assessed case by case.

Whether such beliefs are justified, or for which sampling procedures they
can be justified, is a fundamental, deep question of sampling theory, which
we cannot discuss here \citep[see \eg\ the discussions, reviews, and
references in][]{ericson1969,smith1976,kruskaletal1979c,kruskaletal1980}.
It is of course possible to probabilistically assess, case by case, whether
this assumption is appropriate. It is good to be aware, however, that such
assessment in turn rests on similar assumptions at a higher level, and so on.
