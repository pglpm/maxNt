...
\item \emph{Whether it matters in practice:} We agree with the Reviewer
  ...
  \item
    ...
    \end{minipage}}
  \item \begin{minipage}[t]{0.74\linewidth}Even if the differences were
      negligible, we believe that the results of the paper would matter:
      the point is that we could not have known about the presence or
      absence of differences, if we had not faced the whole problem and
      derived a formula showing that the difference were negligible.
    \end{minipage}
...
% \end{figure}
  \end{enumerate}
\item\emph{Unnecessarily principled:} For our motivation in emphasizing
  principles, we kindly ask the Reviewer to read our reply 1. to Reviewer~2
  above. Probably the writing could be improved on our part to make the
