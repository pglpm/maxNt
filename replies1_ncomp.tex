...
%% Last-Updated: 2020-03-16T20:21:43+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
The main question, on the other hand, is what we mean by \enquote{model}.
There are two main possibilities\ibid: model as a family, or model as a specific
distribution.
...
$\p(M_{1} \land M_{2} \|I)=0$, because we cannot use $M_{1}$'s family
distribution \emph{and} $M_{2}$'s family distribution at the same time.
...
The notion of nested models becomes unimportant, from this perspective.
...
effectively using fewer parameters than $M_{2}$. Correspondingly the
...
is more powerful than another even if it is based on a lower-dimensional
family.
...
data space is normalized. If $M_{2}$ has a typical-data volume larger than
$M_{1}$'s, then it must also assign lower probability to each volume
element in that volume, to ensure normalization. As a result, if the
...
because the data space is the same in both. Hence the difference in the two
...
distributions. The literature is not at all clear. But, in either case, our
calculations show that the approaches based on a population and a sample
thereof can lead to different conclusions -- in accordance with the
non-preservation of sufficiency under sampling -- and they provide a way to
bypass this problem.
