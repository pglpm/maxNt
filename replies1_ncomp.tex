...
%% Last-Updated: 2020-03-17T00:35:35+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\citep{dawid1982,dawid1993}; under exchangeability, this is mathematically
equivalent to the definition (B) below
\citep[\chap~4]{bernardoetal1994_r2000}. This redefinition has several
consequences:

First, two models are different if they have the same family but different
distributions over it. In fact they clearly lead to different probabilities
on the data space. Two such models are also mutually exclusive,
...
Some authors try to define some \enquote{standard} distribution associated
to a family, but we agree with those \citep[\eg][]{copasetal1979}[DeGroot
in][]{lindleyetal1982} who find such definitions ultimately arbitrary. Many
such distributions lead, moreover, to mathematical singularities.
...
typical-set volume above) would correct for the model's power
\citep[\sect~2.2]{mackay1992}. We believe that the correction would
nevertheless be the same in the calculations based on the larger
population, \eqns~(11), and on the sample, \eqns~(12). Hence the difference
in the two approaches -- our main message -- should persist. But it would
be better to check this.
...
And such approximations may be necessary in the precise calculation of the
evidence, as explained in the reply to the previous point. Appendix~C
explains where and why the numerical differences appear. Also, it is
important to know whether we are calculating the frequency of past results,
or the probability for a new result, or the most probable frequency of
future results. These three quantities can be numerically very different
when the ratio $\frac{\text{\# samples}}{\text{data-space dimension}}$ is
high, and they are to be used in different ways. Preliminary calculations
from the full Bayesian analysis show that%
\setlength{\intextsep}{-1ex}% with wrapfigure
\setlength{\columnsep}{0ex}% with wrapfigure
\begin{wrapfigure}{r}{0pt} % with wrapfigure
\centering\includegraphics[width=0.5\linewidth]{example_fullbayesian.pdf}
%\caption{caption}\label{fig:comparison_a5}
\end{wrapfigure}% exp_family_maxent.nb
the most probable frequency distribution across recorded bins (item i on
p.~8) and the belief distribution over the total activity at any recorded
bin (item ii on p.~8) are visibly different. See figure on the right: the
first is the red dashed red curve, the second the solid green line.

These are therefore not philosophical matters. It's a matter of
understanding what we are doing and stating it clearly in our
