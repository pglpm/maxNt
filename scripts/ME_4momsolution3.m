...
Clear[MEhom4moments3,MEhom4thmoment,MEhomvmoments,MEhom4momentsprob]; 

MEhom4moments3[m1_, m2_, m3_, m4_, nn_, wp_:MachinePrecision] := 
 Block[{totalprob,
	zzzm, js1, js2, js3, js4, freeenm, varm, soluzm, prob,ran=Range[0,nn],
	nn1=Sqrt[nn], nn2=Sqrt@Binomial[nn,2], nn3=Sqrt@Binomial[nn,3], nn4=Sqrt@Binomial[nn,4], 
	bvalues},
       zzzm = Sum[Binomial[nn, m]*
		  Exp[js1*m/nn1 + js2*Binomial[m,2]/nn2 +
		      js3*Binomial[m,3]/nn2 +
		      js4*Binomial[m,4]/nn4],
       {m, 0, nn}];
  freeenm = Log[zzzm] - js1*m1*nn1 - js2*m2*nn2 -js3*m3*nn3 -js4*m4*nn4;
  soluzm = 
  ...
  totalprob = Total[prob];prob=prob/totalprob;
    bvalues={(ran.prob/nn)/m1-1,
	     ((Binomial[ran,2]/nn2^2).prob)/m2-1,
	     ((Binomial[ran,3]/nn3^2).prob)/m3-1,
	     ((Binomial[ran,4]/nn4^2).prob)/m4-1};
 Print["rel. discrepancies %: ", bvalues*100//N];
 {js1/nn1 /. soluzm[[2]], js2/nn2 /. soluzm[[2]],
  js3/nn3 /. soluzm[[2]], js4/nn4 /. soluzm[[2]],prob,totalprob}]
